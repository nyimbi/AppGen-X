...
from pyAppGen.dsl import dsl_language_experience_gate
...
from pyAppGen.form_designer import custom_designer_family_contract
from pyAppGen.form_designer import custom_designer_family_module_file_manifest
...
def test_package_pbc_catalog_composes_enterprise_apps(runner: CliRunner) -> None:
    ...
    package_index_schema = pbc_package_index_schema()
    assert package_index_schema["format"] == "appgen.pbc-package-index-schema.v1"
    assert "packages" in package_index_schema["required_fields"]
    ...
    assert package_loading["index_discovery"]["format"] == "appgen.pbc-package-index-discovery-report.v1"
    assert package_loading["index_discovery"]["ok"] is True
    assert len(package_loading["index_discovery"]["catalog_patches"]) == 2
...
def test_package_form_designer_audit_covers_rad_style_drop_design(
    ...
    assert set(property_families["required_families"]) == {
    ...
    assert component_families["format"] == "appgen.component-editor-family-contract.v1"
    assert component_families["ok"] is True
    assert set(component_families["required_families"]) == {
        family["family"] for family in component_families["families"] if family["editors"]
    }
    assert all("open_component_editor" in family["operation"] for family in component_families["families"])
    custom_families = custom_designer_family_contract()
    assert custom_families["format"] == "appgen.custom-designer-family-contract.v1"
    assert custom_families["ok"] is True
    assert set(custom_families["required_families"]) == {
    ...
    assert len(inspector_workbench["component_editor_family_artifacts"]) == 6
    ...
    assert binding_workbench["design_runtime_replay"]["ok"] is True
    assert {
        "author_graph",
        "validate_graph",
    ...
    assert data_workbench["publish_transaction_replay"]["ok"] is True
    assert {
        "profile_connections",
    ...
    assert visual_depth["style_resolution"]["ordered_layers"][0] == "base_theme"
    assert "sample_keyframes" in visual_depth["timeline_playback"]["playback_steps"]
    assert "select_fallback" in visual_depth["effect_render"]["render_steps"]
    ...
    assert tuple(phase["phase"] for phase in component_readiness["phases"]) == (
        "analog_coverage",
    ...
    assert all(item["path"].startswith("app/component_tests/") for item in usability["component_test_files"])
    ...
    assert set(mapping_gate["required_type_mappings"]) <= set(mapping_gate["passing_type_mappings"])
    assert set(mapping_gate["required_supported_fields"]) <= set(mapping_gate["passing_supported_fields"])
    ...
    assert drop_gate["passing_proposal"] == drop_gate["required_proposal"]
    assert set(drop_gate["required_properties"]) <= set(drop_gate["passing_properties"])
    assert {"label", "help_text"} <= set(drop_gate["inspector_properties"])
    suggestion_gate = next(gate for gate in audit["gates"] if gate["id"] == "placement_suggestions")
    ...
    assert set(overlap_gate["required_overlap_pair_keys"]) <= set(overlap_gate["passing_overlap_pair_keys"])
    ...
    assert runtime_stream_gate["ok"] is True
    ...
    assert set(package_gate["required_manager_checks"]) <= set(package_gate["passing_manager_checks"])
    assert set(package_gate["required_compatibility_packages"]) <= set(package_gate["passing_compatibility_packages"])
    assert set(package_gate["required_signature_packages"]) <= set(package_gate["passing_signature_packages"])
    assert set(package_gate["required_lockfile_packages"]) <= set(package_gate["passing_lockfile_packages"])
    assert set(package_gate["required_sandbox_packages"]) <= set(package_gate["passing_sandbox_packages"])
    assert set(package_gate["required_dependency_order_packages"]) <= set(
        package_gate["passing_dependency_order_packages"]
    )
    assert set(package_gate["required_conflict_resolutions"]) <= set(package_gate["passing_conflict_resolutions"])
    assert set(package_gate["required_update_phases"]) <= set(package_gate["passing_update_phases"])
    assert set(package_gate["required_update_packages"]) <= set(package_gate["passing_update_packages"])
    assert set(package_gate["required_uninstall_plan_phases"]) <= set(
        package_gate["passing_uninstall_plan_phases"]
    )
    ...
    assert set(visual_depth_gate["required_operation_names"]) <= set(visual_depth_gate["passing_operation_names"])
    assert set(visual_depth_gate["required_readiness_checks"]) <= set(visual_depth_gate["passing_readiness_checks"])
    assert set(visual_depth_gate["required_style_tokens"]) <= set(visual_depth_gate["passing_style_tokens"])
    assert set(visual_depth_gate["required_style_layers"]) <= set(visual_depth_gate["passing_style_layers"])
    assert set(visual_depth_gate["required_timeline_tracks"]) <= set(visual_depth_gate["passing_timeline_tracks"])
    assert set(visual_depth_gate["required_timeline_guards"]) <= set(visual_depth_gate["passing_timeline_guards"])
    assert set(visual_depth_gate["required_fallback_targets"]) <= set(visual_depth_gate["passing_fallback_targets"])
    assert set(visual_depth_gate["required_fallback_effects"]) <= set(visual_depth_gate["passing_fallback_effects"])
    assert set(visual_depth_gate["required_shader_operations"]) <= set(
        visual_depth_gate["passing_shader_operations"]
    )
    assert set(visual_depth_gate["required_shader_node_kinds"]) <= set(
        visual_depth_gate["passing_shader_node_kinds"]
    )
    assert set(visual_depth_gate["required_hit_test_nodes"]) <= set(visual_depth_gate["passing_hit_test_nodes"])
    assert set(visual_depth_gate["required_transform_nodes"]) <= set(visual_depth_gate["passing_transform_nodes"])
    assert set(visual_depth_gate["required_runtime_package_checks"]) <= set(
        visual_depth_gate["passing_runtime_package_checks"]
    )
    assert set(visual_depth_gate["required_runtime_adapters"]) <= set(visual_depth_gate["passing_runtime_adapters"])
    assert set(visual_depth_gate["required_runtime_fallbacks"]) <= set(visual_depth_gate["passing_runtime_fallbacks"])
    ...
    assert {
        item["path"] for item in property_editor_family_module_file_manifest()
    } <= set(smoke["required_artifacts"])
    assert {
    ...
    assert {
        item["path"] for item in component_editor_family_module_test_file_manifest()
    } <= set(smoke["required_artifacts"])
    assert {
        item["path"] for item in custom_designer_family_module_file_manifest()
    } <= set(smoke["required_artifacts"])
    assert {
...
def test_dsl_linter_reports_semantic_feedback(runner: CliRunner, tmp_path) -> None:
    ...
    assert experience["format"] == "appgen.dsl-language-experience-gate.v1"
    ...
    assert {
        ...
        "keyword_limited",
        "multi_source_ready",
    } == {check["outcome"] for check in experience["checks"]}
    assert dsl_language_experience_gate("table Book { title: string }")["ok"] is False
...
def test_appgen_dsl_normalizes_low_code_model_and_generates(tmp_path) -> None:
    ...
    assert "Branding Workbench JSON" in (output_dir / "templates" / "appgen_branding.html").read_text()
    branding_workbench = branding.branding_workbench(
        {"app/branding.py", "app/static/appgen-theme.css", "app/templates/appgen_branding.html"}
    )
    assert branding_workbench["format"] == "appgen.branding-workbench.v1"
    assert branding_workbench["ok"] is True
    assert branding_workbench["decision"] == "approved"
    assert {
        "artifact_coverage",
        "theme_contract",
        "css_variables",
    ...
    assert "/branding/branding-workbench.json" in next(
    ...
    assert capability_status["dsl.language-design"] == "implemented"
    ...
    database_ops_module_files = database_ops.database_ops_module_file_manifest()
    database_ops_module_tests = database_ops.database_ops_module_test_file_manifest()
    assert database_ops_module_files["ok"] is True
    assert database_ops_module_tests["ok"] is True
    assert {item["surface"] for item in database_ops_module_files["modules"]} == {
        "provider_runtime",
        "addon_runtime",
        "migration_runtime",
        "projection_runtime",
    }
    assert {item["surface"] for item in database_ops_module_tests["tests"]} == {
        "provider_runtime",
        "addon_runtime",
        "migration_runtime",
        "projection_runtime",
    }
    for item in database_ops_module_files["modules"]:
        module_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(module_path), doraise=True)
        module = _load_module(module_path, f"generated_database_ops_module_{item['module']}")
        assert module.module_contract()["ok"] is True
    ...
    for item in database_ops_module_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_database_ops_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
    ...
    diagnostics_module_files = diagnostics.diagnostics_module_file_manifest()
    diagnostics_module_tests = diagnostics.diagnostics_module_test_file_manifest()
    assert diagnostics_module_files["format"] == "appgen.diagnostics-module-file-manifest.v1"
    assert diagnostics_module_files["ok"] is True
    assert diagnostics_module_tests["format"] == "appgen.diagnostics-module-test-file-manifest.v1"
    assert diagnostics_module_tests["ok"] is True
    assert {
        "schema_selftest",
        "row_validation",
        "debug_snapshot",
        "remediation_support",
        "api_load_plan",
        "diagnostics_release_workbench",
    } == {item["surface"] for item in diagnostics_module_files["modules"]}
    for item in diagnostics_module_files["modules"]:
        module_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(module_path), doraise=True)
        module = _load_module(module_path, f"generated_diagnostics_module_{item['module']}")
        assert module.module_contract()["ok"] is True
        assert module.diagnostics_manifest_contract()["ok"] is True
        assert module.run_diagnostics_operation()["ok"] is True
        assert module.release_context()["ok"] is True
        assert module.smoke_test()["ok"] is True
    for item in diagnostics_module_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_diagnostics_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
    ...
    api_testing_module_files = api_testing.api_testing_module_file_manifest()
    api_testing_module_tests = api_testing.api_testing_module_test_file_manifest()
    assert api_testing_module_files["format"] == "appgen.api-testing-module-file-manifest.v1"
    assert api_testing_module_files["ok"] is True
    assert api_testing_module_tests["format"] == "appgen.api-testing-module-test-file-manifest.v1"
    assert api_testing_module_tests["ok"] is True
    assert {
        "request_matrix",
        "response_validation",
        "fixture_strategy",
        "ui_smoke",
        "synthetic_monitor",
        "api_testing_release_workbench",
    } == {item["surface"] for item in api_testing_module_files["modules"]}
    for item in api_testing_module_files["modules"]:
        module_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(module_path), doraise=True)
        module = _load_module(module_path, f"generated_api_testing_module_{item['module']}")
        assert module.module_contract()["ok"] is True
        assert module.api_testing_manifest_contract()["ok"] is True
        assert module.run_api_testing_operation()["ok"] is True
        assert module.release_context()["ok"] is True
        assert module.smoke_test()["ok"] is True
    for item in api_testing_module_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_api_testing_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
    ...
    code_review_module_files = code_review.code_review_module_file_manifest()
    code_review_module_tests = code_review.code_review_module_test_file_manifest()
    assert code_review_module_files["format"] == "appgen.code-review-module-file-manifest.v1"
    assert code_review_module_files["ok"] is True
    assert code_review_module_tests["format"] == "appgen.code-review-module-test-file-manifest.v1"
    assert code_review_module_tests["ok"] is True
    assert {
        "schema_review",
        "artifact_review",
        "review_summary",
        "primary_key_review",
        "field_policy_review",
        "code_review_release_workbench",
    } == {item["surface"] for item in code_review_module_files["modules"]}
    for item in code_review_module_files["modules"]:
        module_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(module_path), doraise=True)
        module = _load_module(module_path, f"generated_code_review_module_{item['module']}")
        assert module.module_contract()["ok"] is True
        assert module.code_review_manifest_contract()["ok"] is True
        assert module.run_code_review_operation()["ok"] is True
        assert module.release_context()["ok"] is True
        assert module.smoke_test()["ok"] is True
    for item in code_review_module_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_code_review_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
    ...
    generated_form_designer_paths.update(
        f"app/property_editor_family_modules/{path.name}"
        for path in (output_dir / "property_editor_family_modules").glob("*.py")
        if path.name != "__init__.py"
    )
    generated_form_designer_paths.update(
        f"app/property_editor_family_module_tests/{path.name}"
        for path in (output_dir / "property_editor_family_module_tests").glob("*.py")
        if path.name != "__init__.py"
    )
    generated_form_designer_paths.update(
    ...
    generated_form_designer_paths.update(
        f"app/component_editor_family_modules/{path.name}"
        for path in (output_dir / "component_editor_family_modules").glob("*.py")
        if path.name != "__init__.py"
    )
    generated_form_designer_paths.update(
        f"app/component_editor_family_module_tests/{path.name}"
        for path in (output_dir / "component_editor_family_module_tests").glob("*.py")
        if path.name != "__init__.py"
    )
    generated_form_designer_paths.update(
        f"app/custom_designer_family_modules/{path.name}"
        for path in (output_dir / "custom_designer_family_modules").glob("*.py")
        if path.name != "__init__.py"
    )
    generated_form_designer_paths.update(
        f"app/custom_designer_family_module_tests/{path.name}"
        for path in (output_dir / "custom_designer_family_module_tests").glob("*.py")
        if path.name != "__init__.py"
    )
    generated_form_designer_paths.update(
    ...
    assert generated_bindings["design_runtime_replay"]["ok"] is True
    assert {
        "author_graph",
        "validate_graph",
        "execute_converter_validator_pipeline",
    ...
    assert generated_data_tooling["publish_transaction_replay"]["ok"] is True
    assert {
        "profile_connections",
        "introspect_schema_and_plan_queries",
        "rehearse_schema_and_dataset",
    ...
    assert {item["surface"] for item in deep_data_module_files["modules"]} == {
        "schema_browser",
        "schema_diff",
        "lookup_editor",
        "dataset_designer",
    ...
    assert {item["surface"] for item in deep_data_module_tests["tests"]} == {
        "schema_browser",
        "schema_diff",
        "lookup_editor",
        "dataset_designer",
    ...
    for item in data_module_tests["tests"]:
        ...
        assert module.smoke_test()["ok"] is True
    for item in deep_data_module_files["modules"]:
        module_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(module_path), doraise=True)
        module = _load_module(module_path, f"generated_deep_data_module_{item['module']}")
        assert module.module_contract()["ok"] is True
        assert module.operation_manifest()["ok"] is True
        assert module.runtime_context()["ok"] is True
        assert module.smoke_test()["ok"] is True
    for item in deep_data_module_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
    ...
    assert all("update_component_reference" in workflow["workflow"] for workflow in generated_inspector["event_edit_workflows"])
    ...
    generated_component_families = form_designer.component_editor_family_contract()
    assert generated_component_families["format"] == "appgen.generated-component-editor-family-contract.v1"
    assert generated_component_families["ok"] is True
    assert set(generated_component_families["required_families"]) == {
        family["family"] for family in generated_component_families["families"] if family["editors"]
    }
    generated_custom_families = form_designer.custom_designer_family_contract()
    assert generated_custom_families["format"] == "appgen.generated-custom-designer-family-contract.v1"
    assert generated_custom_families["ok"] is True
    ...
    assert len(generated_inspector["component_editor_family_artifacts"]) == 6
    ...
    for item in generated_handler_source_tests["tests"]:
        ...
        assert module.smoke_test()["ok"] is True
    generated_property_editor_files = form_designer.property_editor_family_module_file_manifest(
        generated_form_designer_paths
    )
    generated_property_editor_tests = form_designer.property_editor_family_module_test_file_manifest(
        generated_form_designer_paths
    )
    assert generated_property_editor_files["ok"] is True
    assert generated_property_editor_tests["ok"] is True
    assert {item["family"] for item in generated_property_editor_files["modules"]} == {
        "string",
        "number",
        "boolean",
        "choice",
    ...
    for item in generated_event_editor_tests["tests"]:
        ...
        assert module.smoke_test()["ok"] is True
    generated_component_editor_files = form_designer.component_editor_family_module_file_manifest(
        generated_form_designer_paths
    )
    generated_component_editor_tests = form_designer.component_editor_family_module_test_file_manifest(
        generated_form_designer_paths
    )
    assert generated_component_editor_files["ok"] is True
    assert generated_component_editor_tests["ok"] is True
    assert {item["family"] for item in generated_component_editor_files["modules"]} == {
        "selection",
        "dialog",
        "transaction",
        "layout",
    ...
    for item in generated_component_editor_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_component_editor_family_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
    generated_custom_designer_files = form_designer.custom_designer_family_module_file_manifest(
        generated_form_designer_paths
    )
    generated_custom_designer_tests = form_designer.custom_designer_family_module_test_file_manifest(
        generated_form_designer_paths
    )
    assert generated_custom_designer_files["ok"] is True
    assert generated_custom_designer_tests["ok"] is True
    assert {item["family"] for item in generated_custom_designer_files["modules"]} == {
        "paint_overlay",
        "verb_menu",
        "selection_handles",
        "smart_tags",
    ...
    for item in generated_custom_designer_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_custom_designer_family_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
    ...
    assert {
        ...
        "phase_order_ready",
        "side_effect_guard_ready",
    } == {check["id"] for check in generated_usability["component_readiness"]["checks"]}
    assert all(
    ...
    assert "language_experience" in {gate["gate"] for gate in generated_authoring_gate["gates"]}
    assert {"dbml", "sql", "ponyorm", "database", "dsl"} <= {
    ...
    erp_template_module_files = erp_templates.erp_template_module_file_manifest()
    erp_template_module_tests = erp_templates.erp_template_module_test_file_manifest()
    assert erp_template_module_files["format"] == "appgen.erp-template-module-file-manifest.v1"
    assert erp_template_module_files["ok"] is True
    assert erp_template_module_tests["format"] == "appgen.erp-template-module-test-file-manifest.v1"
    assert erp_template_module_tests["ok"] is True
    assert {
        "module_catalog",
        "table_blueprints",
        "starter_stacks",
        "domain_coverage",
        "dsl_package",
        "erp_release_workbench",
    } == {item["surface"] for item in erp_template_module_files["modules"]}
    for item in erp_template_module_files["modules"]:
        module_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(module_path), doraise=True)
        module = _load_module(module_path, f"generated_erp_template_module_{item['module']}")
        assert module.module_contract()["ok"] is True
        assert module.erp_template_manifest_contract()["ok"] is True
        assert module.run_erp_template_operation()["ok"] is True
        assert module.release_context()["ok"] is True
        assert module.smoke_test()["ok"] is True
    for item in erp_template_module_tests["tests"]:
        test_path = output_dir / item["path"].replace("app/", "")
        py_compile.compile(str(test_path), doraise=True)
        module = _load_module(test_path, f"generated_erp_template_module_test_{item['module']}")
        assert module.smoke_test()["ok"] is True
