DOC_HEADER = """
# coding: utf-8
# AUTOGENERATED BY py_appgen from %s
...
class {class_name}ModelView(ModelView):
    ...
    show_columns = {rt_cols} + {form_columns}
    show_fieldset =  {rt_fld_set}
    show_exclude_columns = hide_list 
    add_columns = {rt_cols} + {form_columns}
    add_fieldset =  {rt_fld_set}
    add_exclude_columns = hide_list 
    edit_columns = {rt_cols} + {form_columns}
    edit_fieldset =  {rt_fld_set}
    edit_exclude_columns = hide_list #
    list_columns = {list_columns} + {rt_cols}
    list_exclude_columns = [] # {list_columns} #
    default_sort = [('id', True)]
    search_columns = {rt_cols} + {search_columns}
    search_exclude_columns = [] # {search_columns}
...
#      }}
    ...
    def pre_add(self, item):
        normalized = before_save_row('{table_name}', _appgen_model_row(item, self.add_columns))
        _appgen_apply_row(item, normalized)

    def pre_update(self, item):
        normalized = before_save_row('{table_name}', _appgen_model_row(item, self.edit_columns))
        _appgen_apply_row(item, normalized)

    def post_add(self, item):
        after_save_row('{table_name}', _appgen_model_row(item, self.add_columns))
