"""AI agent and chatbot skill contract for the schema_registry PBC."""
...
AGENT_NAME = "SchemaRegistryAgent"
...
def standalone_agent_workspace_contract():
    """Return the assistant workspace contribution for the one-PBC standalone app."""
    skills = agent_skill_manifest()
    chatbot = chatbot_interface_contract()
    return {
        "ok": skills["ok"] and chatbot["ok"],
...
def assistant_session_plan(intent: str, payload: dict | None = None) -> dict:
    """Route one assistant request to a skill, form, or wizard without mutating data."""
    text = str(intent or "").lower()
    if "breaking" in text or "compatibility" in text:
        skill = f"{PBC_KEY}.policy_explanation"
        wizard = "breaking_change_review_wizard"
    elif "release" in text or "projection" in text:
        skill = f"{PBC_KEY}.workbench_navigation"
        wizard = "release_gate_wizard"
    else:
        skill = f"{PBC_KEY}.task_guidance"
        wizard = "subject_onboarding_wizard"
    return {
        "ok": True,
        "pbc": PBC_KEY,
        "intent": intent,
...
def smoke_test():
    ...
    session = assistant_session_plan("Review breaking compatibility risk", {"subject_id": "subject_smoke"})
    return {
        "ok": skills["ok"]
        and chatbot["ok"]
        and document["ok"]
