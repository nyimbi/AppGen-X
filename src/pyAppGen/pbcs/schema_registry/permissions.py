"""Executable permission contract for the schema_registry PBC."""
...
PERMISSION_CONTRACT = schema_registry_permissions_contract()
...
def smoke_test():
    ...
    action = next(iter(ACTION_PERMISSIONS)) if ACTION_PERMISSIONS else None
    permission = ACTION_PERMISSIONS.get(action) if action else None
