...
def capital_markets_trading_ops_command_trade_order(state, payload):
    next_state = _copy(state); record_id = payload.get('id') or payload.get('code') or f'trade_order-1'; record = {'id': record_id, 'tenant': payload.get('tenant', 'default'), 'status': payload.get('status', 'active'), 'payload': dict(payload)}; next_state['records'][record_id] = record; _event(next_state, CAPITAL_MARKETS_TRADING_OPS_EMITTED_EVENT_TYPES[0], record)
...
def capital_markets_trading_ops_build_schema_contract():
    table_contracts = (
        {'table': 'capital_markets_trading_ops_trade_order', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
    ...
    return {'format': 'appgen.capital-markets-trading-ops-owned-schema-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'tables': table_contracts, 'migrations': tuple({'path': f'pbcs/capital_markets_trading_ops/migrations/{i+1:03d}_{table["table"]}.sql', 'operation': 'create_owned_table', 'table': table['table'], 'backend_allowlist': CAPITAL_MARKETS_TRADING_OPS_ALLOWED_DATABASE_BACKENDS} for i, table in enumerate(table_contracts)), 'models': tuple({'class_name': ''.join(part.capitalize() for part in table['table'].split('_')), 'table': table['table'], 'fields': table['fields']} for table in table_contracts), 'datastore_backends': CAPITAL_MARKETS_TRADING_OPS_ALLOWED_DATABASE_BACKENDS, 'database_backends': CAPITAL_MARKETS_TRADING_OPS_ALLOWED_DATABASE_BACKENDS, 'shared_table_access': False, 'owned_tables': CAPITAL_MARKETS_TRADING_OPS_OWNED_TABLES}

def capital_markets_trading_ops_build_service_contract():
    return {'format': 'appgen.capital-markets-trading-ops-service-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'command_methods': ('configure_runtime','set_parameter','register_rule','register_schema_extension','receive_event','command_trade_order','run_advanced_assessment','parse_document_instruction') + DOMAIN_OPERATIONS, 'query_methods': ('query_workbench','build_workbench_view'), 'shared_table_access': False, 'transaction_boundary': 'owned_datastore_plus_outbox', 'event_contract': 'AppGen-X'}
...
def capital_markets_trading_ops_build_api_contract():
 ...
 'GET /capital-markets-trading-ops-workbench'), 'event_contract': 'AppGen-X', 'stream_engine_picker_visible': False, 'owned_tables': CAPITAL_MARKETS_TRADING_OPS_OWNED_TABLES}
...
def capital_markets_trading_ops_build_release_evidence():
    ...
    return {'format': 'appgen.capital-markets-trading-ops-release-evidence.v1', 'ok': True, 'pbc': PBC_KEY, 'checks': checks, 'generated_artifacts': {'migrations': capital_markets_trading_ops_build_schema_contract()['migrations'], 'models': capital_markets_trading_ops_build_schema_contract()['models'], 'events': {'contract': 'AppGen-X', 'emits': CAPITAL_MARKETS_TRADING_OPS_EMITTED_EVENT_TYPES, 'consumes': CAPITAL_MARKETS_TRADING_OPS_CONSUMED_EVENT_TYPES}, 'handlers': ('receive_event',), 'ui': CAPITAL_MARKETS_TRADING_OPS_UI_FRAGMENT_KEYS}, 'blocking_gaps': ()}
...
def capital_markets_trading_ops_build_workbench_view(tenant='default'):
    return {'ok': True, 'pbc': PBC_KEY, 'tenant': tenant, 'route': f'/workbench/pbcs/{PBC_KEY}', 'tables': CAPITAL_MARKETS_TRADING_OPS_BUSINESS_TABLES, 'actions': DOMAIN_OPERATIONS, 'ui_fragments': CAPITAL_MARKETS_TRADING_OPS_UI_FRAGMENT_KEYS, 'side_effects': ()}
...
def capital_markets_trading_ops_runtime_smoke():
    ...
    rule = capital_markets_trading_ops_register_rule(param['state'], {'rule_id': 'smoke', 'scope': 'domain'})
    ...
    command = capital_markets_trading_ops_command_trade_order(dead['state'], {'tenant': 'tenant-smoke', 'code': 'SMOKE'})
