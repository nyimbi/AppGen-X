...
from pyAppGen.pbcs.capital_markets_trading_ops.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'capital_markets_trading_ops'
...
def test_registration_plan_is_side_effect_free():
    assert package_metadata_manifest()['pbc'] == 'capital_markets_trading_ops'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-capital_markets_trading_ops'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-capital_markets_trading_ops'})['dead_letter_table'].endswith('dead_letter_event')
