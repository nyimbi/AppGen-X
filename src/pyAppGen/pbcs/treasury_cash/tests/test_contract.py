"""Generated contract smoke tests for treasury_cash."""
...
def test_generated_schema_service_and_release_evidence():
    ...
    assert SCHEMA_CONTRACT['pbc'] == 'treasury_cash'
...
def test_runtime_owned_tables_have_schema_models_and_migrations():
    ...
    runtime_owned = tuple(treasury_cash_runtime_capabilities()['owned_tables'])
...
def test_appgen_event_tables_follow_runtime_owned_contract():
    ...
    assert events.EVENT_CONTRACT['outbox_table'] == 'treasury_cash_appgen_outbox_event'
    assert events.EVENT_CONTRACT['inbox_table'] == 'treasury_cash_appgen_inbox_event'
    assert events.EVENT_CONTRACT['dead_letter_table'] == 'treasury_cash_dead_letter_event'
    assert set(handlers.handler_manifest()['dead_letter_tables']) == {'treasury_cash_dead_letter_event'}


def test_manifest_and_event_contract():
    from .. import events

    assert PBC_MANIFEST['pbc'] == 'treasury_cash'
    ...
    assert EVENT_CONTRACT['outbox_table'].startswith('treasury_cash_')
    assert EVENT_CONTRACT['inbox_table'].startswith('treasury_cash_')
...
def test_registration_plan_is_side_effect_free():
    ...
    assert register_pbc()['pbc'] == 'treasury_cash'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert smoke['first_result']['dead_letter_table'].startswith('treasury_cash_')
