"""Generated contract smoke tests for federated_iam."""
...
def test_generated_schema_service_and_release_evidence():
    ...
    assert SCHEMA_CONTRACT['pbc'] == 'federated_iam'
    assert SCHEMA_CONTRACT['ok'] is True
    assert SCHEMA_CONTRACT['owned_tables']
    assert len(SCHEMA_CONTRACT['owned_tables']) >= 17
    assert all(table.startswith('federated_iam_') for table in SCHEMA_CONTRACT['owned_tables'])
    assert any(
        field['name'] == 'principal_id'
        for table in SCHEMA_CONTRACT['tables']
        if table['owned_table'] == 'federated_iam_principal'
        for field in table['fields']
    )
    assert any(
        field['name'] == 'trust_score'
        for table in SCHEMA_CONTRACT['tables']
        if table['owned_table'] == 'federated_iam_principal_identity'
        for field in table['fields']
    )
    assert any(
        field['name'] == 'idempotency_key'
        for table in SCHEMA_CONTRACT['tables']
        if table['owned_table'] == 'federated_iam_appgen_outbox_event'
        for field in table['fields']
    )
...
def test_manifest_and_event_contract():
    ...
    assert PBC_MANIFEST['pbc'] == 'federated_iam'
    ...
    assert EVENT_CONTRACT['outbox_table'].startswith('federated_iam_')
    assert EVENT_CONTRACT['inbox_table'].startswith('federated_iam_')
...
def test_registration_plan_is_side_effect_free():
    ...
    assert register_pbc()['pbc'] == 'federated_iam'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert smoke['first_result']['dead_letter_table'].startswith('federated_iam_')
