"""AI agent and chatbot skill contract for the federated_iam PBC."""
...
_SKILL_NAMES = (
    ...
    f"{PBC_KEY}.release_evidence_review",
)


def _owned_tables() -> tuple[str, ...]:
    tables = []
    for table in PBC_MANIFEST.get("tables", ()):  # logical owned tables plus runtime tables
        if str(table).startswith(f"{PBC_KEY}_"):
            tables.append(table)
        else:
            tables.append(f"{PBC_KEY}_{table}")
    return tuple(tables)
...
def agent_skill_manifest() -> dict:
    ...
    route_catalog = routes.route_catalog_contract()
    return {
        "ok": bool(_SKILL_NAMES) and bool(_owned_tables()) and bool(_query_operations()) and route_catalog["ok"],
        "pbc": PBC_KEY,
        "agent": AGENT_NAME,
        "skills": tuple(
            {
...
def document_instruction_plan(document=None, instructions=None):
    ...
    return {
        ...
        "workflow_routes": tuple(item["route"] for item in routes.route_catalog_contract()["routes"] if item["workflow"]),
