"""Owned model metadata for the loyalty_rewards PBC."""
...
OWNED_SCHEMA = {
    ...
    "runtime_tables": SCHEMA_CONTRACT["runtime_table_descriptors"],
...
def model_manifest() -> dict:
    ...
    relationship_targets = tuple(relationship.get("target_table") for relationship in schema.get("relationships", ()))
    cross_pbc_relationships = tuple(target for target in relationship_targets if not str(target).startswith(f"{PBC_KEY}_"))
    return {
        "ok": bool(schema_tables)
        and set(schema.get("runtime_tables", ())).issubset(set(table["owned_table"] for table in schema["runtime_table_descriptors"]))
...
def smoke_test() -> dict:
    ...
    instance = instantiate_model(first_table, {"account_id": "smoke"}) if first_table else {"ok": False}
    return {
