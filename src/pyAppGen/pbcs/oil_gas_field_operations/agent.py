...
def _appgen_agent_owned_tables() -> tuple[str, ...]:
    ...
    return tuple(table if str(table).startswith('oil_gas_field_operations_') else f'oil_gas_field_operations_{table}' for table in tables) or (f'oil_gas_field_operations_record',)


def agent_skill_manifest() -> dict:
    skills = (
        {'name': f'oil_gas_field_operations_task_guidance', 'scope': 'oil_gas_field_operations', 'description': 'Guide users through domain tasks and release-safe workflows.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'oil_gas_field_operations_document_instruction_intake', 'scope': 'oil_gas_field_operations', 'description': 'Convert documents and instructions into governed mutation previews.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'oil_gas_field_operations_crud_datastore_mutation', 'scope': 'oil_gas_field_operations', 'description': 'Prepare owned-datastore CRUD plans with human confirmation for writes.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
    )
    return {'ok': True, 'pbc': 'oil_gas_field_operations', 'skills': skills, 'stream_engine_picker_visible': False, 'side_effects': ()}


def chatbot_interface_contract() -> dict:
    return {'ok': True, 'pbc': 'oil_gas_field_operations', 'entrypoint': '/assistant/pbc/oil_gas_field_operations', 'single_agent_contribution': 'oil_gas_field_operations_skills', 'capabilities': ('task_guidance', 'document_instruction_intake', 'governed_datastore_crud', 'mutation_preview'), 'side_effects': ()}


def document_instruction_plan(document: str, instruction: str, context: dict | None = None) -> dict:
    tables = _appgen_agent_owned_tables()
    return {'ok': True, 'pbc': 'oil_gas_field_operations', 'document_digest': str(abs(hash(document)))[:12], 'instruction': instruction, 'context': dict(context or {}), 'requires_human_confirmation': True, 'candidate_tables': tables, 'crud_preview': {'action': 'create', 'table': tables[0], 'event_contract': 'AppGen-X'}, 'side_effects': ()}


def datastore_crud_plan(action: str, table: str | None = None, payload: dict | None = None) -> dict:
    tables = _appgen_agent_owned_tables()
    target = table or tables[0]
    if not str(target).startswith('oil_gas_field_operations_'):
        return {'ok': False, 'reason': 'foreign_table_rejected', 'table': target, 'side_effects': ()}
    return {'ok': action in {'create', 'read', 'update', 'delete'}, 'pbc': 'oil_gas_field_operations', 'action': action, 'table': target, 'payload': dict(payload or {}), 'requires_confirmation': action in {'create', 'update', 'delete'}, 'event_contract': 'AppGen-X', 'side_effects': ()}


def composed_agent_contribution() -> dict:
    namespace = 'oil_gas_field_operations_skills'
    return {'ok': True, 'pbc': 'oil_gas_field_operations', 'single_agent_skill_namespace': namespace, 'dsl_tools': (namespace, 'oil_gas_field_operations_crud', 'oil_gas_field_operations_documents'), 'side_effects': ()}
