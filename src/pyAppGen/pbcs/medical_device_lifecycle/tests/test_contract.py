from pyAppGen.pbcs.medical_device_lifecycle import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
from pyAppGen.pbcs.medical_device_lifecycle.schema_contract import build_schema_contract
from pyAppGen.pbcs.medical_device_lifecycle.service_contract import build_service_contract
from pyAppGen.pbcs.medical_device_lifecycle.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.medical_device_lifecycle.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.medical_device_lifecycle.handlers import dispatch_event, handler_manifest
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'medical_device_lifecycle'
...
def test_registration_plan_is_side_effect_free():
    assert package_metadata_manifest()['pbc'] == 'medical_device_lifecycle'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-medical_device_lifecycle'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-medical_device_lifecycle'})['dead_letter_table'].endswith('dead_letter_event')
