...
def lending_origination_servicing_standalone_ui_contract():
    ui = lending_origination_servicing_ui_contract()
    full = ui['full_capability_surface']
    return {'ok': ui['ok'] and len(full['forms']) >= 9 and len(full['wizards']) >= 7 and len(full['controls']) >= 7, 'pbc': PBC_KEY, 'single_pbc_app_route': f'/apps/{PBC_KEY}', 'forms': full['forms'], 'wizards': full['wizards'], 'controls': full['controls'], 'assistant_panel': 'LendingOriginationServicingAssistantPanel', 'side_effects': ()}
