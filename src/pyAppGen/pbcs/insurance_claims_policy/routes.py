...
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /insurance-policies', 'POST /claims', 'POST /coverage-validations', 'POST /claim-settlements', 'GET /insurance-workbench'))
