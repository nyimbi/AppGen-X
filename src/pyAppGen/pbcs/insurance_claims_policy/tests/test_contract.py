from pyAppGen.pbcs.insurance_claims_policy import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.insurance_claims_policy.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.insurance_claims_policy.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.insurance_claims_policy.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.insurance_claims_policy.services import service_operation_contracts
from pyAppGen.pbcs.insurance_claims_policy.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.insurance_claims_policy.config import governance_smoke_test
from pyAppGen.pbcs.insurance_claims_policy.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('CustomerUpdated', 'PaymentCaptured', 'FraudRiskScored')[0], 'idempotency_key': 'idem'})['ok'] is True
