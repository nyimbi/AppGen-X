...
def agent_skill_manifest():
    skills = tuple({'name': name, 'scope': PBC_KEY, 'description': f'{name} for {PBC_KEY}', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False} for name in ('insurance_claims_policy_guide_user', 'insurance_claims_policy_read_records', 'insurance_claims_policy_create_record', 'insurance_claims_policy_update_record'))
