"""API route contracts for the grant_fund_accounting PBC."""
PBC_KEY = 'grant_fund_accounting'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /grant-awards', 'POST /fund-restrictions', 'POST /grant-budgets', 'POST /reimbursement-claims', 'GET /grant-fund-workbench'))
