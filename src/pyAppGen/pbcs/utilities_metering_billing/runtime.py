...
# Improve1 utilities metering billing control extension.
from .utilities_metering_billing_control import evaluate_utilities_metering_billing_control, improve1_utilities_metering_billing_control_contract

_UTILITY_CONTROL_BASE_RUNTIME_CAPABILITIES = utilities_metering_billing_runtime_capabilities
_UTILITY_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = utilities_metering_billing_build_release_evidence


def utilities_metering_billing_runtime_capabilities():
    runtime = dict(_UTILITY_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_utilities_metering_billing_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["utilities_metering_billing_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_utilities_metering_billing_control", "improve1_utilities_metering_billing_control_contract")))
...
def utilities_metering_billing_build_release_evidence():
    ...
    artifacts["utilities_metering_billing_control"] = {
