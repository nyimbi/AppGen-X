from pyAppGen.pbcs.hotel_revenue_management import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.hotel_revenue_management.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.hotel_revenue_management.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.hotel_revenue_management.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.hotel_revenue_management.services import service_operation_contracts
from pyAppGen.pbcs.hotel_revenue_management.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.hotel_revenue_management.config import governance_smoke_test
from pyAppGen.pbcs.hotel_revenue_management.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-hotel_revenue_management'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-hotel_revenue_management'})['dead_letter_table'].endswith('dead_letter_event')
