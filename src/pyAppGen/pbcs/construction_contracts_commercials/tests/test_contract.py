from pyAppGen.pbcs.construction_contracts_commercials import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
from pyAppGen.pbcs.construction_contracts_commercials.schema_contract import build_schema_contract
from pyAppGen.pbcs.construction_contracts_commercials.service_contract import build_service_contract
from pyAppGen.pbcs.construction_contracts_commercials.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
...
from pyAppGen.pbcs.construction_contracts_commercials.services import service_operation_contracts
from pyAppGen.pbcs.construction_contracts_commercials.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.construction_contracts_commercials.config import governance_smoke_test
from pyAppGen.pbcs.construction_contracts_commercials.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-construction_contracts_commercials'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-construction_contracts_commercials'})['dead_letter_table'].endswith('dead_letter_event')
