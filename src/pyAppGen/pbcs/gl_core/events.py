...
_PBC_KEY = "gl_core"
_OUTBOX_TABLE = "gl_core_appgen_outbox_event"
_INBOX_TABLE = "gl_core_appgen_inbox_event"
_DEAD_LETTER_TABLE = "gl_core_dead_letter_event"

_EMITTED_PAYLOAD_FIELDS = ("event_id", "occurred_at", "pbc", "tenant", "correlation_id", "data")
_CONSUMED_PAYLOAD_FIELDS = ("event_id", "occurred_at", "source_pbc", "tenant", "correlation_id", "data")

EVENT_CONTRACT = {
    "contract": "appgen_event_contract",
    "runtime_profile_visibility": "read_only_platform_metadata",
    "adapter": "appgen_event_adapter",
    "topic": GL_CORE_REQUIRED_EVENT_TOPIC,
    "inbox_topic": GL_CORE_REQUIRED_EVENT_TOPIC,
    "outbox_table": _OUTBOX_TABLE,
    "inbox_table": _INBOX_TABLE,
    "dead_letter_table": _DEAD_LETTER_TABLE,
    "emitted": tuple(
        {
            "event_type": event_type,
            "schema": f"{_PBC_KEY}.{event_type.lower()}.emitted.v1",
            "topic": GL_CORE_REQUIRED_EVENT_TOPIC,
            "outbox_table": _OUTBOX_TABLE,
            "payload_fields": _EMITTED_PAYLOAD_FIELDS,
        }
        for event_type in GL_CORE_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": f"{_PBC_KEY}.{event_type.lower()}.consumed.v1",
            "topic": GL_CORE_REQUIRED_EVENT_TOPIC,
            "inbox_table": _INBOX_TABLE,
            "payload_fields": _CONSUMED_PAYLOAD_FIELDS,
        }
        for event_type in GL_CORE_CONSUMED_EVENT_TYPES
    ),
    "retry_policy": {"name": "gl_core_default_retry", "max_attempts": 5, "backoff": "exponential"},
    "idempotency": {"key_fields": ("event_type", "event_id", "handler"), "storage": _INBOX_TABLE},
}
...
def validate_event_contract() -> dict:
    ...
    invalid_tables = tuple(
        ...
        if not table.startswith(f"{_PBC_KEY}_")
    )
    invalid_emitted = tuple(
        event["event_type"]
        for event in EMITTED_EVENTS
        if event.get("topic") != GL_CORE_REQUIRED_EVENT_TOPIC
...
def build_event_envelope(event_type, payload=None, *, direction="emitted", event_id="smoke-event"):
    ...
    envelope["tenant"] = supplied.get("tenant", "tenant_demo")
    envelope["correlation_id"] = supplied.get("correlation_id", envelope["event_id"])
    if direction == "emitted":
        envelope["pbc"] = supplied.get("pbc", _PBC_KEY)
    else:
