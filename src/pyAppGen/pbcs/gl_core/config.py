"""Executable configuration, parameter, and rule contract for the gl_core PBC."""

from __future__ import annotations

import hashlib

from .runtime import GL_CORE_ALLOWED_DATABASE_BACKENDS
from .runtime import GL_CORE_REQUIRED_EVENT_TOPIC


PBC_KEY = "gl_core"
CONFIG_SCHEMA = (
    {"key": "GL_CORE_DATABASE_URL", "required": True, "source": "environment", "maps_to": "database_backend"},
    {"key": "GL_CORE_EVENT_TOPIC", "required": True, "source": "environment", "maps_to": "event_topic", "fixed": GL_CORE_REQUIRED_EVENT_TOPIC},
    {"key": "GL_CORE_RETRY_LIMIT", "required": False, "source": "environment", "maps_to": "retry_limit", "default": 3},
    {"key": "GL_CORE_DEFAULT_CURRENCY", "required": False, "source": "environment", "maps_to": "default_currency", "default": "USD"},
    {"key": "GL_CORE_DEFAULT_TIMEZONE", "required": False, "source": "environment", "maps_to": "default_timezone", "default": "UTC"},
)
PARAMETER_SCHEMA = (
    {"key": "approval_threshold", "type": "number", "default": 1000.0, "min": 0.0, "max": 100000000.0, "scope": "posting"},
    {"key": "materiality_threshold", "type": "number", "default": 0.05, "min": 0.0, "max": 1.0, "scope": "policy"},
    {"key": "close_tolerance", "type": "number", "default": 0.0, "min": 0.0, "max": 1000000.0, "scope": "close"},
    {"key": "revaluation_threshold", "type": "number", "default": 5000.0, "min": 0.0, "max": 100000000.0, "scope": "currency"},
    {"key": "retention_days", "type": "integer", "default": 2555, "min": 1, "max": 3650, "scope": "audit"},
    {"key": "workbench_limit", "type": "integer", "default": 100, "min": 1, "max": 500, "scope": "ui"},
)
RULE_SCHEMA = (
    {"rule_id": "gl_core.balance_required", "condition": "balanced_lines", "effect": "allow_when_true", "scope": "journal_posting"},
    {"rule_id": "gl_core.materiality_gate", "condition": "materiality_within_threshold", "effect": "require_approval_when_false", "scope": "journal_posting"},
    {"rule_id": "gl_core.close_window_open", "condition": "period_open", "effect": "allow_when_true", "scope": "close"},
    {"rule_id": "gl_core.appgen_event_contract_required", "condition": "appgen_event_contract_required", "effect": "allow_when_true", "scope": "platform"},
    {"rule_id": "gl_core.reconciliation_score_gate", "condition": "reconciliation_score_threshold", "effect": "route_for_review_when_false", "scope": "reconciliation"},
)


def configuration_manifest():
    """Return required configuration keys, bounded parameters, and executable rules."""
    return {
        "ok": bool(CONFIG_SCHEMA) and bool(PARAMETER_SCHEMA) and bool(RULE_SCHEMA),
        "pbc": PBC_KEY,
        "schema": CONFIG_SCHEMA,
        "required_keys": required_keys(),
        "allowed_database_backends": GL_CORE_ALLOWED_DATABASE_BACKENDS,
        "parameter_schema": PARAMETER_SCHEMA,
        "rule_schema": RULE_SCHEMA,
        "required_event_topic": GL_CORE_REQUIRED_EVENT_TOPIC,
        "side_effects": (),
    }


def required_keys():
    """Return configuration keys that must be supplied by an installer."""
    return tuple(item["key"] for item in CONFIG_SCHEMA if item.get("required"))


def _backend_from_values(values: dict) -> str:
    url = str(values.get("GL_CORE_DATABASE_URL", "postgresql://configured"))
    if "://" in url:
        return url.split("://", 1)[0].lower()
    return str(values.get("database_backend", "postgresql")).lower()


def validate_configuration(values=None):
    """Validate supplied configuration values without reading process state."""
    supplied = dict(values or {
        "GL_CORE_DATABASE_URL": "postgresql://configured",
        "GL_CORE_EVENT_TOPIC": GL_CORE_REQUIRED_EVENT_TOPIC,
        "GL_CORE_RETRY_LIMIT": 3,
        "GL_CORE_DEFAULT_CURRENCY": "USD",
        "GL_CORE_DEFAULT_TIMEZONE": "UTC",
    })
    missing = tuple(key for key in required_keys() if not supplied.get(key))
    known = {item["key"] for item in CONFIG_SCHEMA}
    unknown = tuple(sorted(key for key in supplied if key not in known))
    backend = _backend_from_values(supplied)
    invalid_backend = backend not in GL_CORE_ALLOWED_DATABASE_BACKENDS
    invalid_topic = supplied.get("GL_CORE_EVENT_TOPIC") != GL_CORE_REQUIRED_EVENT_TOPIC
    return {
        "ok": not missing and not unknown and not invalid_backend and not invalid_topic,
        "pbc": PBC_KEY,
        "missing": missing,
        "unknown": unknown,
        "backend": backend,
        "invalid_backend": invalid_backend,
        "invalid_topic": invalid_topic,
        "required_keys": required_keys(),
        "side_effects": (),
    }


def parameter_manifest():
    """Return bounded runtime and domain parameters understood by this PBC."""
    return {
        "ok": bool(PARAMETER_SCHEMA),
        "pbc": PBC_KEY,
        "parameters": PARAMETER_SCHEMA,
        "side_effects": (),
    }


def set_parameter(current_parameters=None, key="approval_threshold", value=None):
    """Apply one bounded parameter change without mutating caller state."""
    schema = next((item for item in PARAMETER_SCHEMA if item["key"] == key), None)
    if schema is None:
        return {"ok": False, "accepted": False, "reason": "unknown_parameter", "key": key, "side_effects": ()}
    candidate = schema["default"] if value is None else value
    if schema["type"] == "integer" and (not isinstance(candidate, int) or isinstance(candidate, bool)):
        return {"ok": False, "accepted": False, "reason": "invalid_type", "key": key, "side_effects": ()}
    if schema["type"] == "number" and (not isinstance(candidate, (int, float)) or isinstance(candidate, bool)):
        return {"ok": False, "accepted": False, "reason": "invalid_type", "key": key, "side_effects": ()}
    if candidate < schema["min"] or candidate > schema["max"]:
        return {"ok": False, "accepted": False, "reason": "out_of_bounds", "key": key, "side_effects": ()}
    updated = dict(current_parameters or {})
    updated[key] = candidate
    return {
        "ok": True,
        "accepted": True,
        "pbc": PBC_KEY,
        "key": key,
        "value": candidate,
        "parameters": updated,
        "parameter_scope": schema.get("scope"),
        "side_effects": (),
    }


def rule_manifest():
    """Return declarative platform and domain rules supported by this PBC."""
    return {
        "ok": bool(RULE_SCHEMA),
        "pbc": PBC_KEY,
        "rules": RULE_SCHEMA,
        "side_effects": (),
    }


def compile_rule(rule):
    """Compile a rule into a deterministic side-effect-free rule contract."""
    candidate = dict(rule)
    if "stream_engine" in candidate or "stream_processor" in candidate:
        return {"ok": False, "compiled": False, "reason": "stream_engine_picker_disallowed", "side_effects": ()}
    required = {"rule_id", "condition", "effect", "scope"}
    missing = tuple(sorted(field for field in required if field not in candidate))
    if missing:
        return {"ok": False, "compiled": False, "reason": "missing_fields", "missing": missing, "side_effects": ()}
    known_conditions = {item["condition"] for item in RULE_SCHEMA}
    if candidate.get("condition") not in known_conditions:
        return {"ok": False, "compiled": False, "reason": "unknown_condition", "side_effects": ()}
    raw = f"{PBC_KEY}:{tuple(sorted(candidate.items()))}"
    compiled_hash = hashlib.sha256(raw.encode("utf-8")).hexdigest()
    return {
        "ok": True,
        "compiled": True,
        "pbc": PBC_KEY,
        "rule": candidate,
        "compiled_hash": compiled_hash,
        "side_effects": (),
    }


def evaluate_rule(compiled_rule, context=None):
    """Evaluate one compiled rule against supplied context."""
    if not compiled_rule.get("compiled"):
        return {"ok": False, "allowed": False, "reason": "rule_not_compiled", "side_effects": ()}
    supplied = dict(context or {})
    rule = compiled_rule["rule"]
    condition = rule["condition"]
    if condition == "balanced_lines":
        allowed = bool(supplied.get("balanced", True))
    elif condition == "materiality_within_threshold":
        amount = float(supplied.get("amount", 0.0))
        threshold = float(supplied.get("approval_threshold", 1000.0))
        allowed = amount <= threshold
    elif condition == "period_open":
        allowed = supplied.get("period_status", "open") == "open"
    elif condition == "appgen_event_contract_required":
        allowed = supplied.get("event_contract", "AppGen-X") == "AppGen-X"
    elif condition == "reconciliation_score_threshold":
        allowed = float(supplied.get("score", 1.0)) >= float(supplied.get("minimum_score", 0.8))
    else:
        allowed = False
    return {
        "ok": True,
        "allowed": allowed,
        "pbc": PBC_KEY,
        "rule_id": rule.get("rule_id"),
        "condition": condition,
        "scope": rule.get("scope"),
        "side_effects": (),
    }


def governance_smoke_test():
    """Exercise configuration, parameter, and domain rule behavior together."""
    configuration = validate_configuration()
    parameter = set_parameter({}, "approval_threshold", 1000.0)
    compiled_rule = compile_rule(RULE_SCHEMA[0])
    rule_decision = evaluate_rule(compiled_rule, {"balanced": True})
    compiled_close_rule = compile_rule(RULE_SCHEMA[2])
    close_rule_decision = evaluate_rule(compiled_close_rule, {"period_status": "open"})
    return {
        "ok": configuration["ok"]
        and parameter["ok"]
        and compiled_rule["ok"]
        and rule_decision["ok"]
        and rule_decision["allowed"]
        and compiled_close_rule["ok"]
        and close_rule_decision["ok"]
        and close_rule_decision["allowed"],
        "configuration": configuration,
        "parameter": parameter,
        "compiled_rule": compiled_rule,
        "rule_decision": rule_decision,
        "compiled_close_rule": compiled_close_rule,
        "close_rule_decision": close_rule_decision,
        "side_effects": (),
    }


def smoke_test():
    """Exercise configuration, rules, and parameters using synthetic values."""
    governance = governance_smoke_test()
    return {
        **governance["configuration"],
        "ok": governance["ok"],
        "parameter": governance["parameter"],
        "compiled_rule": governance["compiled_rule"],
        "rule_decision": governance["rule_decision"],
        "compiled_close_rule": governance["compiled_close_rule"],
        "close_rule_decision": governance["close_rule_decision"],
        "side_effects": (),
    }
