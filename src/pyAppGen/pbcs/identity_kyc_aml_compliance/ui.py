...
def identity_kyc_aml_compliance_ui_contract():
    ...
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('IdentityKycAmlComplianceWorkbench',
 'IdentityKycAmlComplianceDetail',
 'IdentityKycAmlComplianceAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('identity_kyc_aml_compliance.read',
 'identity_kyc_aml_compliance.create',
 'identity_kyc_aml_compliance.update',
 'identity_kyc_aml_compliance.approve',
 'identity_kyc_aml_compliance.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def identity_kyc_aml_compliance_render_workbench():
    ui = identity_kyc_aml_compliance_ui_contract(); full = ui['full_capability_surface']
