...
def music_royalties_rights_ui_contract():
 ...
 'MusicRoyaltiesRightsAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('music_royalties_rights.read',
 'music_royalties_rights.create',
 'music_royalties_rights.update',
 'music_royalties_rights.approve',
 'music_royalties_rights.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def music_royalties_rights_render_workbench():
    ui = music_royalties_rights_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': music_royalties_rights_ui_contract()['ok'] and music_royalties_rights_render_workbench()['ok'], 'side_effects': ()}
