"""Service layer for the vendor_supplier_360 PBC."""
...
COMMAND_OPERATIONS = ('command_supplier_profile', 'configure_runtime', 'set_parameter', 'register_rule')
...
def _operation_contract(name, kind):
    return {'operation': name, 'operation_kind': kind, 'owned_tables': OWNED_TABLES[:2] if kind == 'command' else (), 'read_tables': OWNED_TABLES[:2] if kind == 'query' else (), 'emitted_event': ('SupplierQualified', 'SupplierRiskChanged', 'SupplierBankValidated', 'SupplierOnboarded')[0] if kind == 'command' else None, 'transaction_boundary': 'owned_datastore_plus_outbox' if kind == 'command' else 'read_only_projection'}


class VendorSupplier360Service:
...
def service_operation_manifest():
    return {'ok': True, 'pbc': PBC_KEY, 'service_class': 'VendorSupplier360Service', 'command_operations': COMMAND_OPERATIONS, 'query_operations': QUERY_OPERATIONS, 'event_contract': EVENT_CONTRACT, 'side_effects': ()}
...
def smoke_test():
    service = VendorSupplier360Service()
...
_BaseVendorSupplier360Service = VendorSupplier360Service

class VendorSupplier360Service(_BaseVendorSupplier360Service):
    def __getattr__(self, name):
