...
# Improve1 renewables asset operations control release extension.
...
_RENEWABLES_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = build_release_evidence
_RENEWABLES_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE = validate_release_evidence


def build_release_evidence() -> dict:
    evidence = dict(_RENEWABLES_CONTROL_BASE_BUILD_RELEASE_EVIDENCE())
    control = _improve1_renewables_asset_operations_control_contract()
    checks = tuple(evidence.get("checks", ())) + ({"id": "improve1_renewables_asset_operations_control", "ok": control["ok"]},)
    evidence.update({
        "ok": bool(evidence.get("ok")) and control["ok"],
        "checks": checks,
        "renewables_asset_operations_control": control,
...
def validate_release_evidence() -> dict:
    validation = dict(_RENEWABLES_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE())
    control = _improve1_renewables_asset_operations_control_contract()
    validation["ok"] = validation.get("ok") is True and control["ok"]
    validation["renewables_asset_operations_control"] = control
