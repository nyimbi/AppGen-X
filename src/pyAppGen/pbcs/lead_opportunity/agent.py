"""AI agent and chatbot skill contract for the lead_opportunity PBC."""
...
AGENT_NAME = 'LeadOpportunityAgent'
...
def chatbot_interface_contract():
    ...
    return {
        ...
        'capabilities': (
            ...
            'sales_forms_wizards_and_controls',
...
def document_instruction_plan(document=None, instructions=None):
    ...
    sales_plan = document_instruction_lead_opportunity_plan(document_text, instruction_text)
    return {
        'ok': bool(document_text or instruction_text) and sales_plan['ok'],
        'pbc': PBC_KEY,
        ...
        'target_table': sales_plan['target_table'],
        'proposed_operation': sales_plan['proposed_operation'],
...
def composed_agent_contribution():
    ...
    standalone_app = single_pbc_lead_opportunity_app_contract()
    return {
...
def smoke_test():
    ...
    skills = agent_skill_manifest()
    chatbot = chatbot_interface_contract()
    document = document_instruction_plan('proposal request', 'create quote handoff')
    ...
    return {
        ...
        and chatbot['ok']
        and document['ok']
        and document['target_table'].startswith('lead_opportunity_')
        and read_plan['ok']
        and create_plan['ok']
        and contribution['ok']
