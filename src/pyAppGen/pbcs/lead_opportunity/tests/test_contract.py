"""Generated contract smoke tests for lead_opportunity."""
...
def test_manifest_and_event_contract():
    ...
    assert EVENT_CONTRACT['outbox_table'].startswith('lead_opportunity_')
    assert EVENT_CONTRACT['inbox_table'].startswith('lead_opportunity_')


def test_registration_plan_is_side_effect_free():
    from .. import register_pbc, registration_plan

    assert register_pbc()['pbc'] == 'lead_opportunity'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert smoke['first_result']['dead_letter_table'].startswith('lead_opportunity_')
