from pyAppGen.pbcs.case_knowledge_management import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.case_knowledge_management.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.case_knowledge_management.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.case_knowledge_management.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.case_knowledge_management.services import service_operation_contracts
from pyAppGen.pbcs.case_knowledge_management.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.case_knowledge_management.config import governance_smoke_test
from pyAppGen.pbcs.case_knowledge_management.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('ServiceTicketOpened', 'CustomerUpdated', 'SearchIndexRefreshed')[0], 'idempotency_key': 'idem'})['ok'] is True
