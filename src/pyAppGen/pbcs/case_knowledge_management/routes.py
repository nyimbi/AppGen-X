"""API route contracts for the case_knowledge_management PBC."""
PBC_KEY = 'case_knowledge_management'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /support-cases', 'POST /knowledge-articles', 'POST /article-approvals', 'POST /case-deflections', 'GET /knowledge-workbench'))
