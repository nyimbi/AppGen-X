...
from pyAppGen.pbcs.public_safety_dispatch.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'CustomerUpdated', 'SupplierQualified')[0], 'idempotency_key': 'idem-public_safety_dispatch'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-public_safety_dispatch'})['dead_letter_table'].endswith('dead_letter_event')
