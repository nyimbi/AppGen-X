...
# Improve1 production control extension.
from .production_control_control import evaluate_production_control, improve1_production_control_contract

_PRODUCTION_CONTROL_BASE_RUNTIME_CAPABILITIES = production_control_runtime_capabilities
_PRODUCTION_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = production_control_build_release_evidence


def production_control_runtime_capabilities() -> dict:
    runtime = dict(_PRODUCTION_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_production_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["production_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_production_control", "improve1_production_control_contract")))
    runtime["improve1_control_owned_tables"] = control["owned_tables"]
    return runtime


def production_control_build_release_evidence() -> dict:
    evidence = dict(_PRODUCTION_CONTROL_BASE_BUILD_RELEASE_EVIDENCE())
    control = improve1_production_control_contract()
    artifacts = dict(evidence.get("generated_artifacts", {}))
    artifacts["production_control"] = {
