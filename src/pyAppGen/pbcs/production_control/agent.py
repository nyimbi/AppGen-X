...
def standalone_agent_workspace_contract():
    ...
    forms=ui.production_control_form_contracts(); wizards=ui.production_control_wizard_contracts(); controls=ui.production_control_control_catalog(); route_manifest=routes.standalone_route_contracts()
...
_original_production_control_document_instruction_plan=document_instruction_plan
def document_instruction_plan(document=None,instructions=None):
    base_plan=_original_production_control_document_instruction_plan(document,instructions); combined=f"{document or ''} {instructions or ''}".lower(); from . import ui
    wizard_candidates=tuple(i['key'] for i in ui.production_control_wizard_contracts()['contracts'] if any(k in combined for k in i.get('keywords',()))) or ('ShopFloorPacketIntakeWizard',)
...
_original_production_control_datastore_crud_plan=datastore_crud_plan
def datastore_crud_plan(action='read',table=None,payload=None):
    plan=_original_production_control_datastore_crud_plan(action,table,payload); selected=plan.get('table'); action=plan.get('action')
