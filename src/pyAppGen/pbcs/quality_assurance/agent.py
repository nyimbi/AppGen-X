"""AI agent and chatbot skill contract for the quality_assurance PBC."""
...
def standalone_agent_workspace_contract():
    ...
    forms = ui.quality_assurance_form_contracts()
    wizards = ui.quality_assurance_wizard_contracts()
    controls = ui.quality_assurance_control_catalog()
    route_manifest = routes.standalone_route_contracts()
    return {'format': 'appgen.quality-assurance-standalone-agent-workspace.v1', 'ok': forms['ok'] and wizards['ok'] and controls['ok'] and route_manifest['ok'], 'pbc': PBC_KEY, 'agent': AGENT_NAME, 'forms': tuple(item['key'] for item in forms['contracts']), 'wizards': tuple(item['key'] for item in wizards['contracts']), 'controls': tuple(item['key'] for item in controls['contracts']), 'routes': route_manifest['routes'], 'side_effects': ()}


_original_quality_assurance_document_instruction_plan = document_instruction_plan

def document_instruction_plan(document=None, instructions=None):
    base_plan = _original_quality_assurance_document_instruction_plan(document, instructions)
    combined = f"{document or ''} {instructions or ''}".lower()
    from . import ui
    wizards = ui.quality_assurance_wizard_contracts()['contracts']
    wizard_candidates = tuple(item['key'] for item in wizards if any(keyword in combined for keyword in item.get('keywords', ()))) or ('InspectionLotIntakeWizard',)
    ...
    return {**base_plan, 'standalone_routes': route_candidates, 'wizard_candidates': wizard_candidates, 'workspace': standalone_agent_workspace_contract()}


_original_quality_assurance_datastore_crud_plan = datastore_crud_plan

def datastore_crud_plan(action='read', table=None, payload=None):
    plan = _original_quality_assurance_datastore_crud_plan(action, table, payload)
    selected_table = plan.get('table')
    normalized_action = plan.get('action')
