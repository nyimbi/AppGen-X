...
def construction_project_controls_ui_contract():
    ...
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('ConstructionProjectControlsWorkbench',
 'ConstructionProjectControlsDetail',
 'ConstructionProjectControlsAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('construction_project_controls.read',
 'construction_project_controls.create',
 'construction_project_controls.update',
 'construction_project_controls.approve',
 'construction_project_controls.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def construction_project_controls_render_workbench():
    ui = construction_project_controls_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': construction_project_controls_ui_contract()['ok'] and construction_project_controls_render_workbench()['ok'], 'side_effects': ()}
