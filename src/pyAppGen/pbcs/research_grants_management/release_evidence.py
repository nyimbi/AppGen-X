from .runtime import research_grants_management_build_release_evidence
from .standalone import standalone_smoke_test

def build_release_evidence():
    return research_grants_management_build_release_evidence()
...
# Improve1 research grants management control release extension.
...
_RESEARCH_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = build_release_evidence
_RESEARCH_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE = validate_release_evidence
...
def validate_release_evidence() -> dict:
    validation = dict(_RESEARCH_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE())
    control = _improve1_research_grants_management_control_contract()
    validation["ok"] = validation.get("ok") is True and control["ok"]
    validation["research_grants_management_control"] = control
