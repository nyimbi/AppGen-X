"""AI agent and chatbot skill contract for the order_routing_optimization PBC."""
...
AGENT_NAME = 'OrderRoutingOptimizationAgent'
...
def chatbot_interface_contract():
    ...
    return {
        ...
        'capabilities': (
            ...
            'routing_forms_wizards_and_controls',
...
def document_instruction_plan(document=None, instructions=None):
    ...
    routing_plan = document_instruction_routing_plan(document_text, instruction_text)
    return {
        'ok': bool(document_text or instruction_text) and routing_plan['ok'],
        'pbc': PBC_KEY,
        ...
        'target_table': routing_plan['target_table'],
        'proposed_operation': routing_plan['proposed_operation'],
...
def composed_agent_contribution():
    ...
    standalone_app = single_pbc_routing_app_contract()
    return {
...
def smoke_test():
    ...
    skills = agent_skill_manifest()
    chatbot = chatbot_interface_contract()
    document = document_instruction_plan('capacity snapshot', 'load ATP capacity for node')
    ...
    return {
        ...
        and chatbot['ok']
        and document['ok']
        and document['target_table'].startswith('order_routing_optimization_')
        and read_plan['ok']
        and create_plan['ok']
        and contribution['ok']
