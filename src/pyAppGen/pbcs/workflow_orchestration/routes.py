...
def _route_to_route(route: dict) -> dict:
    ...
    return {"method": method, "path": f"/api/pbc/{PBC_KEY}{path}", "handler": route.get("command") or route.get("query"), "permission": route["requires_permission"]}
...
ROUTES = tuple(_route_to_route(route) for route in workflow_orchestration_build_api_contract()["routes"])
API_ROUTE_CONTRACTS = tuple(_route_to_contract(route) for route in workflow_orchestration_build_api_contract()["routes"])
