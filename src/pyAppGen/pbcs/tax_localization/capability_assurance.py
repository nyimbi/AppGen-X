"""Executable capability assurance for the tax_localization PBC."""
...
def _runtime_capabilities() -> dict:
    return runtime.tax_localization_runtime_capabilities()
...
def _ui_contract() -> dict:
    return ui.tax_localization_ui_contract()
...
def _feature_evidence(feature: str, runtime_capabilities: dict, schema: dict) -> tuple[str, ...]:
    ...
    if any(hint in tokenized for hint in ("configuration", "rule", "parameter", "approval")):
        evidence.append("configuration_rules_parameters")
    if any(hint in tokenized for hint in ("workbench", "ui", "console", "view", "dashboard", "panel")):
        evidence.append("ui_workbench")
    if any(hint in tokenized for hint in ("event", "outbox", "inbox", "handler", "dead_letter", "retry", "idempotent")):
        evidence.append("appgen_event_contract")
    if any(hint in tokenized for hint in ("master", "schema", "table", "line", "profile", "calendar", "topology", "tax", "filing")):
        evidence.append("owned_schema")
    if any(hint in tokenized for hint in ("command", "workflow", "routing", "forecast", "optimization", "calculation", "validation", "reconciliation")):
...
def table_stakes_capability_manifest() -> dict:
    ...
    repository_check = repository.smoke_test()
    app = app_surface.app_surface_smoke_test()
    boundary_probe = runtime.tax_localization_verify_owned_table_boundary(("foreign_operational_table",))
    ...
    feature_rows = tuple(
        {"kind": "standard", "feature": feature, "evidence": _feature_evidence(feature, runtime_capabilities, schema)}
        for feature in standard_features
    ) + tuple(
        {"kind": "advanced", "feature": feature, "evidence": _feature_evidence(feature, runtime_capabilities, schema)}
...
def validate_table_stakes_capability_coverage() -> dict:
    ...
    invalid_backends = tuple(
        backend
        for backend in tuple(schema.get("datastore_backends") or schema.get("allowed_database_backends") or ())
    ...
    ui_gaps = tuple(key for key in ("configuration_editor", "parameter_editor", "rule_editor") if key in ui_contract and not ui_contract.get(key))
    stream_picker_visible = bool(ui_contract.get("configuration_editor", {}).get("stream_engine_picker_visible")) or bool(event_contract.get("stream_engine_picker_visible"))
    ...
    return {
        ...
        and manifest["repository"].get("ok") is True
        and manifest["single_pbc_app"].get("ok") is True
