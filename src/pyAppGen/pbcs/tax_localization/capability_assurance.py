"""Executable capability assurance for the tax_localization financial-core PBC."""
...
_UI_FEATURE_HINTS = ("workbench", "ui", "console", "view", "dashboard")
...
def table_stakes_capability_manifest() -> dict:
    ...
    return {
        "format": "appgen.financial-pbc-capability-assurance.v1",
        "ok": True,
        "pbc": PBC_KEY,
...
def validate_table_stakes_capability_coverage() -> dict:
    ...
    invalid_tables = tuple(
        ...
        and not (
            PBC_KEY == "ar_credit"
            and table.startswith("ar_")
            and f"ar_credit_{table.removeprefix('ar_')}" in schema_owned_tables
        )
    )
    invalid_backends = tuple(
        backend
        for backend in runtime_capabilities.get("allowed_database_backends", ())
    ...
    return {
        "format": "appgen.financial-pbc-capability-validation.v1",
