...
def _appgen_agent_owned_tables() -> tuple[str, ...]:
    ...
    return tuple(table if str(table).startswith('ar_credit_') else f'ar_credit_{table}' for table in tables) or (f'ar_credit_record',)


def agent_skill_manifest() -> dict:
    skills = (
        {'name': f'ar_credit_task_guidance', 'scope': 'ar_credit', 'description': 'Guide users through domain tasks and release-safe workflows.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'ar_credit_document_instruction_intake', 'scope': 'ar_credit', 'description': 'Convert documents and instructions into governed mutation previews.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'ar_credit_crud_datastore_mutation', 'scope': 'ar_credit', 'description': 'Prepare owned-datastore CRUD plans with human confirmation for writes.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'ar_credit_cash_application_preview', 'scope': 'ar_credit', 'description': 'Preview cash application against open receivables before mutation.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
    )
    return {'ok': True, 'pbc': 'ar_credit', 'skills': skills, 'stream_engine_picker_visible': False, 'side_effects': ()}


def chatbot_interface_contract() -> dict:
    return {'ok': True, 'pbc': 'ar_credit', 'entrypoint': '/assistant/pbc/ar_credit', 'single_agent_contribution': 'ar_credit_skills', 'capabilities': ('task_guidance', 'document_instruction_intake', 'governed_datastore_crud', 'mutation_preview'), 'side_effects': ()}


def document_instruction_plan(document: str, instruction: str, context: dict | None = None) -> dict:
    tables = _appgen_agent_owned_tables()
    return {'ok': True, 'pbc': 'ar_credit', 'document_digest': str(abs(hash(document)))[:12], 'instruction': instruction, 'context': dict(context or {}), 'requires_human_confirmation': True, 'candidate_tables': tables, 'crud_preview': {'action': 'create', 'table': tables[0], 'event_contract': 'AppGen-X'}, 'side_effects': ()}


def datastore_crud_plan(action: str, table: str | None = None, payload: dict | None = None) -> dict:
    tables = _appgen_agent_owned_tables()
    target = table or tables[0]
    if not str(target).startswith('ar_credit_'):
        return {'ok': False, 'reason': 'foreign_table_rejected', 'table': target, 'side_effects': ()}
    return {'ok': action in {'create', 'read', 'update', 'delete'}, 'pbc': 'ar_credit', 'action': action, 'table': target, 'payload': dict(payload or {}), 'requires_confirmation': action in {'create', 'update', 'delete'}, 'event_contract': 'AppGen-X', 'side_effects': ()}


def composed_agent_contribution() -> dict:
    namespace = 'ar_credit_skills'
    return {'ok': True, 'pbc': 'ar_credit', 'single_agent_skill_namespace': namespace, 'dsl_tools': (namespace, 'ar_credit_crud', 'ar_credit_documents'), 'side_effects': ()}
