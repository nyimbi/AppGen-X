"""Contract smoke tests for ar_credit."""
...
def test_generated_schema_service_and_release_evidence():
    ...
    assert SCHEMA_CONTRACT["pbc"] == "ar_credit"
    ...
    assert "repository" in release_manifest["sections"]
    assert "standalone_app" in release_manifest["sections"]
...
def test_manifest_and_event_contract():
    ...
    assert PBC_MANIFEST["pbc"] == "ar_credit"
    ...
    assert EVENT_CONTRACT["outbox_table"].startswith("ar_credit_")
    assert EVENT_CONTRACT["inbox_table"].startswith("ar_credit_")
...
def test_registration_plan_is_side_effect_free():
    ...
    assert register_pbc()["pbc"] == "ar_credit"
...
def test_configuration_permissions_seed_and_standalone_hooks_are_executable():
    from .. import config, permissions, seed_data, standalone
    ...
    workbench_smoke = standalone.workbench_smoke_test()
    ...
    assert standalone_smoke["latest_release"]["evidence"]["ok"] is True
...
def test_ui_forms_wizards_controls_and_workbench_surface_are_executable():
    from .. import controls, forms, seed_data, ui, wizards

    form_smoke = forms.smoke_test()
    wizard_smoke = wizards.smoke_test()
    control_smoke = controls.smoke_test()
    ui_smoke = ui.smoke_test()
    seeded = seed_data.load_demo_state()
    rendered = ui.ar_credit_render_workbench(
        seeded["state"],
        tenant="tenant_demo",
        principal_permissions=tuple(sorted(ui.ar_credit_ui_contract()["action_permissions"].values())),
    )
    standalone = ui.ar_credit_render_standalone_app(
        seeded["state"],
        tenant="tenant_demo",
        principal_permissions=tuple(sorted(ui.ar_credit_ui_contract()["action_permissions"].values())),
    )
    assert form_smoke["ok"] is True
    assert wizard_smoke["ok"] is True
    assert control_smoke["ok"] is True
    ...
    assert not ui_smoke["side_effects"]
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert smoke["first_result"]["dead_letter_table"].startswith("ar_credit_")
...
# AppGen-X canonical source-audit contract tests for ar_credit.
def test_service_and_route_surface_are_executable():
    import importlib

    services = importlib.import_module("pyAppGen.pbcs.ar_credit.services")
    routes = importlib.import_module("pyAppGen.pbcs.ar_credit.routes")
...
def test_configuration_permissions_and_seed_hooks_are_executable():
    ...
    config = importlib.import_module("pyAppGen.pbcs.ar_credit.config")
    permissions = importlib.import_module("pyAppGen.pbcs.ar_credit.permissions")
    seed_data = importlib.import_module("pyAppGen.pbcs.ar_credit.seed_data")
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    events = importlib.import_module("pyAppGen.pbcs.ar_credit.events")
    handlers = importlib.import_module("pyAppGen.pbcs.ar_credit.handlers")
...
def test_release_registration_and_package_metadata_are_executable():
    ...
    package = importlib.import_module("pyAppGen.pbcs.ar_credit")
    release_evidence = importlib.import_module("pyAppGen.pbcs.ar_credit.release_evidence")
