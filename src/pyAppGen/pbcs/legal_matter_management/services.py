"""Service layer for the legal_matter_management PBC."""
...
COMMAND_OPERATIONS = ('command_legal_matter', 'configure_runtime', 'set_parameter', 'register_rule')
QUERY_OPERATIONS = ('query_workbench',)
OWNED_TABLES = ('legal_matter_management_legal_matter', 'legal_matter_management_outside_counsel', 'legal_matter_management_matter_budget', 'legal_matter_management_matter_document', 'legal_matter_management_legal_deadline', 'legal_matter_management_legal_hold', 'legal_matter_management_counsel_invoice', 'legal_matter_management_matter_outcome', 'legal_matter_management_appgen_outbox_event', 'legal_matter_management_appgen_inbox_event', 'legal_matter_management_appgen_dead_letter_event')


def _operation_contract(name, kind):
    return {'operation': name, 'operation_kind': kind, 'owned_tables': OWNED_TABLES[:2] if kind == 'command' else (), 'read_tables': OWNED_TABLES[:2] if kind == 'query' else (), 'emitted_event': ('LegalMatterOpened', 'LegalHoldIssued', 'MatterBudgetApproved', 'CounselInvoiceReviewed')[0] if kind == 'command' else None, 'transaction_boundary': 'owned_datastore_plus_outbox' if kind == 'command' else 'read_only_projection'}


class LegalMatterManagementService:
...
def service_operation_manifest():
    return {'ok': True, 'pbc': PBC_KEY, 'service_class': 'LegalMatterManagementService', 'command_operations': COMMAND_OPERATIONS, 'query_operations': QUERY_OPERATIONS, 'event_contract': EVENT_CONTRACT, 'side_effects': ()}
...
def smoke_test():
    service = LegalMatterManagementService()
...
_BaseLegalMatterManagementService = LegalMatterManagementService

class LegalMatterManagementService(_BaseLegalMatterManagementService):
    def __getattr__(self, name):
