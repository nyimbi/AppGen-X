"""API route contracts for the legal_matter_management PBC."""
PBC_KEY = 'legal_matter_management'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /legal-matters', 'POST /outside-counsel', 'POST /matter-budgets', 'POST /legal-holds', 'GET /legal-matter-workbench'))
