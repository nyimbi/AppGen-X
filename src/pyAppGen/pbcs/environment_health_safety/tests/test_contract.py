...
from pyAppGen.pbcs.environment_health_safety.routes import api_route_contracts, validate_api_route_contracts
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'environment_health_safety'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-environment_health_safety'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-environment_health_safety'})['dead_letter_table'].endswith('dead_letter_event')
