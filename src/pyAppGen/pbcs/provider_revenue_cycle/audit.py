"""Package-local audit helpers for the provider_revenue_cycle standalone PBC."""
...
def run_provider_revenue_cycle_pbc_audit() -> dict:
    standalone_smoke = standalone.standalone_smoke_test()
    service_manifest = services.standalone_service_manifest()
    ui_contract = ui.provider_revenue_cycle_ui_contract()
    agent_manifest = agent.agent_skill_manifest()
    docs = standalone.documentation_presence()
    boundary = runtime.provider_revenue_cycle_verify_owned_table_boundary(("provider_revenue_cycle_patient_account_table",))
    checks = (
        {"id": "standalone_smoke", "ok": standalone_smoke["ok"]},
        {"id": "service_methods_present", "ok": service_manifest["ok"] and len(service_manifest["service_methods"]) >= 18},
        ...
        {"id": "fixed_event_contract", "ok": service_manifest["event_contract"] == "AppGen-X" and service_manifest["event_topic"] == runtime.PROVIDER_REVENUE_CYCLE_REQUIRED_EVENT_TOPIC},
    )
    return {
        "format": "appgen.provider-revenue-cycle-package-audit.v1",
        ...
        "ui": {
            "forms": tuple(item["form_id"] for item in ui_contract.get("forms", ())),
            "wizards": tuple(item["wizard_id"] for item in ui_contract.get("wizards", ())),
            "controls": tuple(item["control_id"] for item in ui_contract.get("controls", ())),
        },
        "agent": {
