"""AppGen-X event contract for the dom PBC."""
    ...
    "emitted": tuple(
        {
            ...
            "schema": f"dom.{event_type.lower()}.emitted.v1",
            "topic": DOM_REQUIRED_EVENT_TOPIC,
            "outbox_table": "dom_appgen_outbox_event",
            "payload_fields": ("event_id", "occurred_at", "pbc", "data"),
        }
        for event_type in DOM_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": f"dom.{event_type.lower()}.consumed.v1",
            "topic": DOM_REQUIRED_EVENT_TOPIC,
            "inbox_table": "dom_appgen_inbox_event",
            "payload_fields": ("event_id", "occurred_at", "source_pbc", "data"),
        }
        for event_type in DOM_CONSUMED_EVENT_TYPES
    ),
...
def build_event_envelope(event_type, payload=None, *, direction="emitted", event_id="smoke-event"):
    ...
    if direction == "emitted":
        envelope["pbc"] = supplied.get("pbc", "dom")
    else:
