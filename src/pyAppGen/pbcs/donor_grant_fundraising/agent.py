...
def _appgen_agent_owned_tables() -> tuple[str, ...]:
    ...
    return tuple(table if str(table).startswith('donor_grant_fundraising_') else f'donor_grant_fundraising_{table}' for table in tables) or (f'donor_grant_fundraising_record',)


def agent_skill_manifest() -> dict:
    skills = (
        {'name': f'donor_grant_fundraising_task_guidance', 'scope': 'donor_grant_fundraising', 'description': 'Guide users through domain tasks and release-safe workflows.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'donor_grant_fundraising_document_instruction_intake', 'scope': 'donor_grant_fundraising', 'description': 'Convert documents and instructions into governed mutation previews.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
        {'name': f'donor_grant_fundraising_crud_datastore_mutation', 'scope': 'donor_grant_fundraising', 'description': 'Prepare owned-datastore CRUD plans with human confirmation for writes.', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False},
    )
    return {'ok': True, 'pbc': 'donor_grant_fundraising', 'skills': skills, 'stream_engine_picker_visible': False, 'side_effects': ()}


def chatbot_interface_contract() -> dict:
    return {'ok': True, 'pbc': 'donor_grant_fundraising', 'entrypoint': '/assistant/pbc/donor_grant_fundraising', 'single_agent_contribution': 'donor_grant_fundraising_skills', 'capabilities': ('task_guidance', 'document_instruction_intake', 'governed_datastore_crud', 'mutation_preview'), 'side_effects': ()}


def document_instruction_plan(document: str, instruction: str, context: dict | None = None) -> dict:
    tables = _appgen_agent_owned_tables()
    return {'ok': True, 'pbc': 'donor_grant_fundraising', 'document_digest': str(abs(hash(document)))[:12], 'instruction': instruction, 'context': dict(context or {}), 'requires_human_confirmation': True, 'candidate_tables': tables, 'crud_preview': {'action': 'create', 'table': tables[0], 'event_contract': 'AppGen-X'}, 'side_effects': ()}


def datastore_crud_plan(action: str, table: str | None = None, payload: dict | None = None) -> dict:
    tables = _appgen_agent_owned_tables()
    target = table or tables[0]
    if not str(target).startswith('donor_grant_fundraising_'):
        return {'ok': False, 'reason': 'foreign_table_rejected', 'table': target, 'side_effects': ()}
    return {'ok': action in {'create', 'read', 'update', 'delete'}, 'pbc': 'donor_grant_fundraising', 'action': action, 'table': target, 'payload': dict(payload or {}), 'requires_confirmation': action in {'create', 'update', 'delete'}, 'event_contract': 'AppGen-X', 'side_effects': ()}


def composed_agent_contribution() -> dict:
    namespace = 'donor_grant_fundraising_skills'
    return {'ok': True, 'pbc': 'donor_grant_fundraising', 'single_agent_skill_namespace': namespace, 'dsl_tools': (namespace, 'donor_grant_fundraising_crud', 'donor_grant_fundraising_documents'), 'side_effects': ()}
