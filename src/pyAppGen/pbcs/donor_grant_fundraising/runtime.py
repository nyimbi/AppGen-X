...
DONOR_GRANT_FUNDRAISING_EMITTED_EVENT_TYPES = ('DonorGrantFundraisingCreated',
...
DONOR_GRANT_FUNDRAISING_STANDARD_FEATURE_KEYS = ('donor_management',
...
DONOR_GRANT_FUNDRAISING_UI_FRAGMENT_KEYS = ('DonorGrantFundraisingWorkbench',
 'DonorGrantFundraisingDetail',
 'DonorGrantFundraisingAssistantPanel')
DONOR_GRANT_FUNDRAISING_BUSINESS_TABLES = ('donor_grant_fundraising_donor',
 'donor_grant_fundraising_campaign',
 'donor_grant_fundraising_pledge',
 'donor_grant_fundraising_gift',
 'donor_grant_fundraising_restriction',
...
def _event(state, event_type, payload):
    state['outbox'].append({'event_type': event_type, 'topic': DONOR_GRANT_FUNDRAISING_REQUIRED_EVENT_TOPIC, 'payload': dict(payload), 'idempotency_key': _digest((event_type, payload))})

def donor_grant_fundraising_configure_runtime(state, config):
    next_state = _copy(state)
    ok = config.get('database_backend') in DONOR_GRANT_FUNDRAISING_ALLOWED_DATABASE_BACKENDS and config.get('event_topic', DONOR_GRANT_FUNDRAISING_REQUIRED_EVENT_TOPIC) == DONOR_GRANT_FUNDRAISING_REQUIRED_EVENT_TOPIC
    next_state['configuration'] = {'ok': ok, **dict(config), 'event_contract': 'AppGen-X', 'stream_engine_picker_visible': False}
    return {'ok': ok, 'state': next_state, 'configuration': next_state['configuration'], 'side_effects': ()}

def donor_grant_fundraising_set_parameter(state, name, value):
...
def donor_grant_fundraising_register_rule(state, rule):
...
def donor_grant_fundraising_register_schema_extension(state, table, fields):
...
def donor_grant_fundraising_receive_event(state, event):
...
def donor_grant_fundraising_command_donor(state, payload):
    next_state = _copy(state); record_id = payload.get('id') or payload.get('code') or f'donor-1'; record = {'id': record_id, 'tenant': payload.get('tenant', 'default'), 'status': payload.get('status', 'active'), 'payload': dict(payload)}; next_state['records'][record_id] = record; _event(next_state, DONOR_GRANT_FUNDRAISING_EMITTED_EVENT_TYPES[0], record)
    return {'ok': True, 'state': next_state, 'record': record, 'side_effects': ()}

def donor_grant_fundraising_query_workbench(state, filters=None):
    return {'ok': True, 'records': tuple(state.get('records', {}).values()), 'filters': dict(filters or {}), 'read_only': True, 'side_effects': ()}

def donor_grant_fundraising_run_advanced_assessment(state, payload=None):
...
def donor_grant_fundraising_parse_document_instruction(document, instruction):
    return {'ok': True, 'candidate_tables': DONOR_GRANT_FUNDRAISING_BUSINESS_TABLES[:3], 'instruction': instruction, 'document_digest': _digest(document), 'requires_human_confirmation': True, 'side_effects': ()}

def donor_grant_fundraising_build_schema_contract():
    table_contracts = (
        {'table': 'donor_grant_fundraising_donor', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_campaign', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_pledge', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_gift', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        ...
        {'table': 'donor_grant_fundraising_donor_grant_fundraising_control_assertion', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_donor_grant_fundraising_governed_model', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_appgen_outbox_event', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_appgen_inbox_event', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'donor_grant_fundraising_appgen_dead_letter_event', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
    )
    return {'format': 'appgen.donor-grant-fundraising-owned-schema-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'tables': table_contracts, 'migrations': tuple({'path': f'pbcs/donor_grant_fundraising/migrations/{i+1:03d}_{table["table"]}.sql', 'operation': 'create_owned_table', 'table': table['table'], 'backend_allowlist': DONOR_GRANT_FUNDRAISING_ALLOWED_DATABASE_BACKENDS} for i, table in enumerate(table_contracts)), 'models': tuple({'class_name': ''.join(part.capitalize() for part in table['table'].split('_')), 'table': table['table'], 'fields': table['fields']} for table in table_contracts), 'datastore_backends': DONOR_GRANT_FUNDRAISING_ALLOWED_DATABASE_BACKENDS, 'database_backends': DONOR_GRANT_FUNDRAISING_ALLOWED_DATABASE_BACKENDS, 'shared_table_access': False, 'owned_tables': DONOR_GRANT_FUNDRAISING_OWNED_TABLES}

def donor_grant_fundraising_build_service_contract():
    return {'format': 'appgen.donor-grant-fundraising-service-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'command_methods': ('configure_runtime','set_parameter','register_rule','register_schema_extension','receive_event','command_donor','run_advanced_assessment','parse_document_instruction') + DOMAIN_OPERATIONS, 'query_methods': ('query_workbench','build_workbench_view'), 'shared_table_access': False, 'transaction_boundary': 'owned_datastore_plus_outbox', 'event_contract': 'AppGen-X'}

def donor_grant_fundraising_build_api_contract():
    return {'format': 'appgen.donor-grant-fundraising-api-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'routes': ('POST /donors',
 'POST /campaigns',
 'POST /pledges',
 'POST /gifts',
 'POST /restrictions',
...
def donor_grant_fundraising_permissions_contract():
 ...
 'donor_grant_fundraising.admin'), 'roles': ('operator','approver','auditor'), 'side_effects': ()}

def donor_grant_fundraising_build_workbench_view(tenant='default'):
    return {'ok': True, 'pbc': PBC_KEY, 'tenant': tenant, 'route': f'/workbench/pbcs/{PBC_KEY}', 'tables': DONOR_GRANT_FUNDRAISING_BUSINESS_TABLES, 'actions': DOMAIN_OPERATIONS, 'ui_fragments': DONOR_GRANT_FUNDRAISING_UI_FRAGMENT_KEYS, 'side_effects': ()}

def donor_grant_fundraising_verify_owned_table_boundary(references=()):
    invalid = tuple(ref for ref in references if isinstance(ref, str) and ref.endswith('_table') and not ref.startswith(f'{PBC_KEY}_'))
    return {'ok': not invalid, 'pbc': PBC_KEY, 'invalid_references': invalid, 'allowed_tables': DONOR_GRANT_FUNDRAISING_OWNED_TABLES, 'shared_table_access': False}

def donor_grant_fundraising_runtime_capabilities():
...
def donor_grant_fundraising_runtime_smoke():
    state = donor_grant_fundraising_empty_state()
    cfg = donor_grant_fundraising_configure_runtime(state, {
        'database_backend': 'postgresql',
        'event_topic': DONOR_GRANT_FUNDRAISING_REQUIRED_EVENT_TOPIC,
    })
    ...
    received = donor_grant_fundraising_receive_event(rule['state'], event)
