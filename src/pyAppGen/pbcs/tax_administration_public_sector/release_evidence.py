from .runtime import tax_administration_public_sector_build_release_evidence
from .standalone import standalone_smoke_test

def build_release_evidence():
    return tax_administration_public_sector_build_release_evidence()
...
# Improve1 tax administration public sector control release extension.
...
_TAX_ADMIN_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = build_release_evidence
_TAX_ADMIN_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE = validate_release_evidence
...
def validate_release_evidence() -> dict:
    validation = dict(_TAX_ADMIN_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE())
    control = _improve1_tax_administration_public_sector_control_contract()
    validation["ok"] = validation.get("ok") is True and control["ok"]
    validation["tax_administration_public_sector_control"] = control
