from pyAppGen.pbcs.insurance_underwriting import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.insurance_underwriting.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
...
from pyAppGen.pbcs.insurance_underwriting.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'insurance_underwriting'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-insurance_underwriting'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-insurance_underwriting'})['dead_letter_table'].endswith('dead_letter_event')
