...
def insurance_underwriting_ui_contract():
    surface = domain_capability_surface_contract()
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('InsuranceUnderwritingWorkbench',
 'InsuranceUnderwritingDetail',
 'InsuranceUnderwritingAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('insurance_underwriting.read',
...
def insurance_underwriting_render_workbench():
    ui = insurance_underwriting_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': insurance_underwriting_ui_contract()['ok'] and insurance_underwriting_render_workbench()['ok'], 'side_effects': ()}
