...
def claims_adjudication_healthcare_ui_contract():
    ...
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('ClaimsAdjudicationHealthcareWorkbench',
 'ClaimsAdjudicationHealthcareDetail',
 'ClaimsAdjudicationHealthcareAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('claims_adjudication_healthcare.read',
 'claims_adjudication_healthcare.create',
 'claims_adjudication_healthcare.update',
 'claims_adjudication_healthcare.approve',
 'claims_adjudication_healthcare.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def claims_adjudication_healthcare_render_workbench():
    ui = claims_adjudication_healthcare_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': claims_adjudication_healthcare_ui_contract()['ok'] and claims_adjudication_healthcare_render_workbench()['ok'], 'side_effects': ()}
