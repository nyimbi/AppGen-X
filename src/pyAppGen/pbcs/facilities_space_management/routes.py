"""API route contracts for the facilities_space_management PBC."""
PBC_KEY = 'facilities_space_management'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /facility-sites', 'POST /buildings', 'POST /spaces', 'POST /space-reservations', 'GET /facilities-workbench'))
