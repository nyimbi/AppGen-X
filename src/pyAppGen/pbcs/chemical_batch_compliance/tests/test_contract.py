from pyAppGen.pbcs.chemical_batch_compliance import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.chemical_batch_compliance.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.chemical_batch_compliance.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.chemical_batch_compliance.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.chemical_batch_compliance.services import service_operation_contracts
from pyAppGen.pbcs.chemical_batch_compliance.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.chemical_batch_compliance.config import governance_smoke_test
from pyAppGen.pbcs.chemical_batch_compliance.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'chemical_batch_compliance'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-chemical_batch_compliance'})['dead_letter_table'].endswith('dead_letter_event')
