from pyAppGen.pbcs.energy_grid_operations import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.energy_grid_operations.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.energy_grid_operations.services import service_operation_contracts
from pyAppGen.pbcs.energy_grid_operations.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.energy_grid_operations.config import governance_smoke_test
from pyAppGen.pbcs.energy_grid_operations.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'energy_grid_operations'
...
def test_registration_plan_is_side_effect_free():
    assert package_metadata_manifest()['pbc'] == 'energy_grid_operations'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-energy_grid_operations'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-energy_grid_operations'})['dead_letter_table'].endswith('dead_letter_event')
