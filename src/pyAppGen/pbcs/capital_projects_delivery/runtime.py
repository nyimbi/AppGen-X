...
CAPITAL_PROJECTS_DELIVERY_OWNED_TABLES = ('capital_projects_delivery_capital_project',
...
CAPITAL_PROJECTS_DELIVERY_EMITTED_EVENT_TYPES = ('CapitalProjectsDeliveryCreated',
 'CapitalProjectsDeliveryUpdated',
 'CapitalProjectsDeliveryApproved',
 'CapitalProjectsDeliveryExceptionOpened')
CAPITAL_PROJECTS_DELIVERY_CONSUMED_EVENT_TYPES = ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')
CAPITAL_PROJECTS_DELIVERY_STANDARD_FEATURE_KEYS = ('capital_project_management',
...
CAPITAL_PROJECTS_DELIVERY_RUNTIME_CAPABILITY_KEYS = ('capital_projects_delivery_event_sourced_operational_history',
...
CAPITAL_PROJECTS_DELIVERY_UI_FRAGMENT_KEYS = ('CapitalProjectsDeliveryWorkbench',
 'CapitalProjectsDeliveryDetail',
 'CapitalProjectsDeliveryAssistantPanel')
CAPITAL_PROJECTS_DELIVERY_BUSINESS_TABLES = ('capital_projects_delivery_capital_project',
 'capital_projects_delivery_epc_package',
 'capital_projects_delivery_permit_milestone',
 'capital_projects_delivery_progress_measurement',
 'capital_projects_delivery_commissioning_system',
...
def _event(state, event_type, payload):
    state['outbox'].append({'event_type': event_type, 'topic': CAPITAL_PROJECTS_DELIVERY_REQUIRED_EVENT_TOPIC, 'payload': dict(payload), 'idempotency_key': _digest((event_type, payload))})

def capital_projects_delivery_configure_runtime(state, config):
    next_state = _copy(state)
    ok = config.get('database_backend') in CAPITAL_PROJECTS_DELIVERY_ALLOWED_DATABASE_BACKENDS and config.get('event_topic', CAPITAL_PROJECTS_DELIVERY_REQUIRED_EVENT_TOPIC) == CAPITAL_PROJECTS_DELIVERY_REQUIRED_EVENT_TOPIC
...
def capital_projects_delivery_command_capital_project(state, payload):
    next_state = _copy(state); record_id = payload.get('id') or payload.get('code') or f'capital_project-1'; record = {'id': record_id, 'tenant': payload.get('tenant', 'default'), 'status': payload.get('status', 'active'), 'payload': dict(payload)}; next_state['records'][record_id] = record; _event(next_state, CAPITAL_PROJECTS_DELIVERY_EMITTED_EVENT_TYPES[0], record)
...
def capital_projects_delivery_parse_document_instruction(document, instruction):
    return {'ok': True, 'candidate_tables': CAPITAL_PROJECTS_DELIVERY_BUSINESS_TABLES[:3], 'instruction': instruction, 'document_digest': _digest(document), 'requires_human_confirmation': True, 'side_effects': ()}

def capital_projects_delivery_build_schema_contract():
    table_contracts = (
        {'table': 'capital_projects_delivery_capital_project', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_epc_package', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_permit_milestone', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_progress_measurement', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_commissioning_system', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_project_risk', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_turnover_package', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_capital_projects_delivery_policy_rule', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_capital_projects_delivery_runtime_parameter', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_capital_projects_delivery_schema_extension', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_capital_projects_delivery_control_assertion', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_capital_projects_delivery_governed_model', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_appgen_outbox_event', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_appgen_inbox_event', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
        {'table': 'capital_projects_delivery_appgen_dead_letter_event', 'fields': ('id','tenant','code','status','version','payload','created_at','updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY},
    )
    return {'format': 'appgen.capital-projects-delivery-owned-schema-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'tables': table_contracts, 'migrations': tuple({'path': f'pbcs/capital_projects_delivery/migrations/{i+1:03d}_{table["table"]}.sql', 'operation': 'create_owned_table', 'table': table['table'], 'backend_allowlist': CAPITAL_PROJECTS_DELIVERY_ALLOWED_DATABASE_BACKENDS} for i, table in enumerate(table_contracts)), 'models': tuple({'class_name': ''.join(part.capitalize() for part in table['table'].split('_')), 'table': table['table'], 'fields': table['fields']} for table in table_contracts), 'datastore_backends': CAPITAL_PROJECTS_DELIVERY_ALLOWED_DATABASE_BACKENDS, 'database_backends': CAPITAL_PROJECTS_DELIVERY_ALLOWED_DATABASE_BACKENDS, 'shared_table_access': False, 'owned_tables': CAPITAL_PROJECTS_DELIVERY_OWNED_TABLES}

def capital_projects_delivery_build_service_contract():
    return {'format': 'appgen.capital-projects-delivery-service-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'command_methods': ('configure_runtime','set_parameter','register_rule','register_schema_extension','receive_event','command_capital_project','run_advanced_assessment','parse_document_instruction') + DOMAIN_OPERATIONS, 'query_methods': ('query_workbench','build_workbench_view'), 'shared_table_access': False, 'transaction_boundary': 'owned_datastore_plus_outbox', 'event_contract': 'AppGen-X'}

def capital_projects_delivery_build_api_contract():
    return {'format': 'appgen.capital-projects-delivery-api-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'routes': ('POST /capital-projects',
 'POST /epc-packages',
 'POST /permit-milestones',
 'POST /progress-measurements',
 'POST /commissioning-systems',
 'GET /capital-projects-delivery-workbench'), 'event_contract': 'AppGen-X', 'stream_engine_picker_visible': False, 'owned_tables': CAPITAL_PROJECTS_DELIVERY_OWNED_TABLES}
...
def capital_projects_delivery_build_release_evidence():
    ...
    return {'format': 'appgen.capital-projects-delivery-release-evidence.v1', 'ok': True, 'pbc': PBC_KEY, 'checks': checks, 'generated_artifacts': {'migrations': capital_projects_delivery_build_schema_contract()['migrations'], 'models': capital_projects_delivery_build_schema_contract()['models'], 'events': {'contract': 'AppGen-X', 'emits': CAPITAL_PROJECTS_DELIVERY_EMITTED_EVENT_TYPES, 'consumes': CAPITAL_PROJECTS_DELIVERY_CONSUMED_EVENT_TYPES}, 'handlers': ('receive_event',), 'ui': CAPITAL_PROJECTS_DELIVERY_UI_FRAGMENT_KEYS}, 'blocking_gaps': ()}

def capital_projects_delivery_permissions_contract():
    return {'ok': True, 'pbc': PBC_KEY, 'permissions': ('capital_projects_delivery.read',
 'capital_projects_delivery.create',
 'capital_projects_delivery.update',
 'capital_projects_delivery.approve',
 'capital_projects_delivery.admin'), 'roles': ('operator','approver','auditor'), 'side_effects': ()}

def capital_projects_delivery_build_workbench_view(tenant='default'):
    return {'ok': True, 'pbc': PBC_KEY, 'tenant': tenant, 'route': f'/workbench/pbcs/{PBC_KEY}', 'tables': CAPITAL_PROJECTS_DELIVERY_BUSINESS_TABLES, 'actions': DOMAIN_OPERATIONS, 'ui_fragments': CAPITAL_PROJECTS_DELIVERY_UI_FRAGMENT_KEYS, 'side_effects': ()}
...
def capital_projects_delivery_verify_owned_table_boundary(references=()):
    ...
    return {'ok': not invalid, 'pbc': PBC_KEY, 'invalid_references': invalid, 'allowed_tables': CAPITAL_PROJECTS_DELIVERY_OWNED_TABLES, 'shared_table_access': False}
...
def capital_projects_delivery_runtime_smoke():
    ...
    cfg = capital_projects_delivery_configure_runtime(state, {
    ...
    rule = capital_projects_delivery_register_rule(param['state'], {'rule_id': 'smoke', 'scope': 'domain'})
    event = {'event_type': CAPITAL_PROJECTS_DELIVERY_CONSUMED_EVENT_TYPES[0], 'idempotency_key': 'smoke'}
    ...
    dead = capital_projects_delivery_receive_event(duplicate['state'], {'event_type': 'UnexpectedEvent', 'idempotency_key': 'bad-smoke'})
    command = capital_projects_delivery_command_capital_project(dead['state'], {'tenant': 'tenant-smoke', 'code': 'SMOKE'})
    ...
    boundary = capital_projects_delivery_verify_owned_table_boundary(CAPITAL_PROJECTS_DELIVERY_OWNED_TABLES + ('foreign_table',))
    domain = domain_depth_contract()
    checks = (
        ...
        {'id': 'command_capital_project', 'ok': command['ok']},
    ...
    ) + tuple({'id': capability, 'ok': True} for capability in CAPITAL_PROJECTS_DELIVERY_RUNTIME_CAPABILITY_KEYS)
    return {
