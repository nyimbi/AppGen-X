...
def capital_projects_delivery_ui_contract():
    ...
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('CapitalProjectsDeliveryWorkbench',
 'CapitalProjectsDeliveryDetail',
 'CapitalProjectsDeliveryAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('capital_projects_delivery.read',
 'capital_projects_delivery.create',
 'capital_projects_delivery.update',
 'capital_projects_delivery.approve',
 'capital_projects_delivery.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def capital_projects_delivery_render_workbench():
    ui = capital_projects_delivery_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': capital_projects_delivery_ui_contract()['ok'] and capital_projects_delivery_render_workbench()['ok'], 'side_effects': ()}
