from pyAppGen.pbcs.real_estate_property_management import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.real_estate_property_management.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
...
from pyAppGen.pbcs.real_estate_property_management.services import service_operation_contracts
from pyAppGen.pbcs.real_estate_property_management.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.real_estate_property_management.config import governance_smoke_test
from pyAppGen.pbcs.real_estate_property_management.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'real_estate_property_management'
...
def test_registration_plan_is_side_effect_free():
    assert package_metadata_manifest()['pbc'] == 'real_estate_property_management'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'CustomerUpdated', 'SupplierQualified')[0], 'idempotency_key': 'idem-real_estate_property_management'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-real_estate_property_management'})['dead_letter_table'].endswith('dead_letter_event')
