from pyAppGen.pbcs.banking_core_accounts import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.banking_core_accounts.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.banking_core_accounts.events import event_contract_manifest, validate_event_contract
...
from pyAppGen.pbcs.banking_core_accounts.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.banking_core_accounts.config import governance_smoke_test
from pyAppGen.pbcs.banking_core_accounts.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'banking_core_accounts'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-banking_core_accounts'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-banking_core_accounts'})['dead_letter_table'].endswith('dead_letter_event')
