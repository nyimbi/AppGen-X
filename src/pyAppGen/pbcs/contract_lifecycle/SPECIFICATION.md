# Contract Lifecycle Management PBC Specification

## Stable Identity

The `contract_lifecycle` pbc is a package-owned business capability in the `relationship` mesh. It owns the implementation directory `src/pyAppGen/pbcs/contract_lifecycle` and exposes a stable manifest, side-effect-free registration, discovery metadata, release evidence, and package-local tests. The PBC description is: Contract authoring, clauses, obligations, approvals, renewals, risk scoring, documents, and transaction events.

## Owned Boundary

The package owns its datastore boundary and does not mutate shared or foreign tables. All owned table names are generated under the `contract_lifecycle_` prefix in schema contracts, models, migrations, service operation contracts, event handlers, UI workbench projections, and agent CRUD plans. Cross-PBC collaboration is represented through APIs, AppGen-X events, and read-only projections rather than shared table writes.
...
The PBC contributes first-class agent skills into the composed application single assistant under the `contract_lifecycle_skills` namespace. The chatbot helps users accomplish tasks, accepts documents and instructions, proposes governed datastore CRUD mutations, rejects foreign table mutation, requires human confirmation for writes, and emits AppGen-X event plans. Skills are expressible in the DSL through composed assistant tool names.
