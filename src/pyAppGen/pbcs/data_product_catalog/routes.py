...
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /data-products', 'POST /data-contracts', 'POST /quality-slas', 'POST /access-requests', 'GET /data-product-catalog-workbench'))
