"""Executable runtime capability proofs for the api_gateway_mesh PBC."""
...
def test_contracts_enforce_owned_boundary_appgen_eventing_and_backend_allowlist():
    ...
    assert f"{PBC_KEY}_dead_letter_event" in str(schema)
...
def test_configuration_rules_parameters_and_ui_are_executable():
    ...
    configured = _call(
        ...
        state,
        {
            ...
            "event_topic": getattr(runtime, "API_GATEWAY_MESH_REQUIRED_EVENT_TOPIC"),
            "retry_limit": 3,
        },
    )
    parameter = _call("set_parameter", configured["state"], "retry_budget", 4)
