...
def test_generated_schema_service_and_release_evidence():
    ...
    assert all(table.startswith('api_gateway_mesh_') for table in SCHEMA_CONTRACT['owned_tables'])
    assert any(
        field['name'] == 'service_id'
        for table in SCHEMA_CONTRACT['tables']
        if table['owned_table'] == 'api_gateway_mesh_service_registration'
        for field in table['fields']
    )
    assert any(
        field['name'] == 'route_id'
        for table in SCHEMA_CONTRACT['tables']
        if table['owned_table'] == 'api_gateway_mesh_service_route'
        for field in table['fields']
    )
    assert any(
        field['name'] == 'idempotency_key'
        for table in SCHEMA_CONTRACT['tables']
        if table['owned_table'] == 'api_gateway_mesh_appgen_outbox_event'
        for field in table['fields']
    )
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert smoke['first_result']['dead_letter_table'].startswith('api_gateway_mesh_')
