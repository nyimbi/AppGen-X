"""AppGen-X event contract for the api_gateway_mesh PBC."""
...
def _event_schema_name(event_type: str, direction: str) -> str:
    return f"api_gateway_mesh.{event_type.lower()}.{direction}.v1"
...
EVENT_CONTRACT = {
    ...
    "inbox_topic": f"{API_GATEWAY_MESH_REQUIRED_EVENT_TOPIC}.inbox",
    ...
    "emitted": tuple(
        {
            ...
            "schema": _event_schema_name(event_type, "emitted"),
            "topic": API_GATEWAY_MESH_REQUIRED_EVENT_TOPIC,
            "outbox_table": "api_gateway_mesh_appgen_outbox_event",
            "payload_fields": ("event_id", "occurred_at", "pbc", "data"),
        }
        for event_type in API_GATEWAY_MESH_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": _event_schema_name(event_type, "consumed"),
            "topic": f"{API_GATEWAY_MESH_REQUIRED_EVENT_TOPIC}.inbox",
            "inbox_table": "api_gateway_mesh_appgen_inbox_event",
    ...
    "idempotency": {"key_fields": ("event_type", "event_id", "handler"), "storage": "api_gateway_mesh_appgen_inbox_event"},
}
...
def validate_event_contract() -> dict:
    ...
    return {
        ...
        and manifest["topic"] == API_GATEWAY_MESH_REQUIRED_EVENT_TOPIC
...
def build_event_envelope(event_type, payload=None, *, direction="emitted", event_id="smoke-event"):
    ...
    if direction == "emitted":
        envelope["pbc"] = supplied.get("pbc", "api_gateway_mesh")
    else:
