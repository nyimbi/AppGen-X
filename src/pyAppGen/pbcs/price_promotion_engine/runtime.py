...
# Improve1 pricing control extension.
from .pricing_control import evaluate_pricing_control, improve1_pricing_control_contract

_PRICING_CONTROL_BASE_RUNTIME_CAPABILITIES = price_promotion_engine_runtime_capabilities
_PRICING_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = price_promotion_engine_build_release_evidence


def price_promotion_engine_runtime_capabilities() -> dict:
    runtime = dict(_PRICING_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_pricing_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["pricing_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_pricing_control", "improve1_pricing_control_contract")))
...
def price_promotion_engine_build_release_evidence() -> dict:
    ...
    checks = tuple(evidence.get("checks", ())) + ({"id": "improve1_pricing_control", "ok": control["ok"]},)
    evidence.update({
        "ok": bool(evidence.get("ok")) and control["ok"],
        "checks": checks,
        "generated_artifacts": artifacts,
        "pricing_control": control,
