from pyAppGen.pbcs.smart_city_mobility_operations import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.smart_city_mobility_operations.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.smart_city_mobility_operations.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.smart_city_mobility_operations.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.smart_city_mobility_operations.services import service_operation_contracts
from pyAppGen.pbcs.smart_city_mobility_operations.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.smart_city_mobility_operations.config import governance_smoke_test
from pyAppGen.pbcs.smart_city_mobility_operations.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'AuditEventSealed', 'OperationalKpiChanged')[0], 'idempotency_key': 'idem-smart_city_mobility_operations'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-smart_city_mobility_operations'})['dead_letter_table'].endswith('dead_letter_event')
