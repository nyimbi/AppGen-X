...
# Improve1 streaming analytics control extension.
from .streaming_analytics_control import evaluate_streaming_analytics_control, improve1_streaming_analytics_control_contract

_STREAMING_CONTROL_BASE_RUNTIME_CAPABILITIES = streaming_analytics_runtime_capabilities
_STREAMING_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = streaming_analytics_build_release_evidence


def streaming_analytics_runtime_capabilities() -> dict:
    runtime = dict(_STREAMING_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_streaming_analytics_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["streaming_analytics_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_streaming_analytics_control", "improve1_streaming_analytics_control_contract")))
...
def streaming_analytics_build_release_evidence() -> dict:
    evidence = dict(_STREAMING_CONTROL_BASE_BUILD_RELEASE_EVIDENCE())
    control = improve1_streaming_analytics_control_contract()
    artifacts = dict(evidence.get("generated_artifacts", {}))
    artifacts["streaming_analytics_control"] = {
    ...
    checks = tuple(evidence.get("checks", ())) + ({"id": "improve1_streaming_analytics_control", "ok": control["ok"]},)
    evidence.update({
        "ok": bool(evidence.get("ok")) and control["ok"],
        "checks": checks,
        "generated_artifacts": artifacts,
        "streaming_analytics_control": control,
