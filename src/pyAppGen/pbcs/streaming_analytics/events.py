...
EVENT_CONTRACT = {
    ...
    "inbox_topic": f"{STREAMING_ANALYTICS_REQUIRED_EVENT_TOPIC}.inbox",
    ...
    "emitted": tuple(
        ...
        for event_type in STREAMING_ANALYTICS_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": f"{PBC_KEY}.{event_type}.consumed.v1",
            "topic": f"{STREAMING_ANALYTICS_REQUIRED_EVENT_TOPIC}.inbox",
            "inbox_table": STREAMING_ANALYTICS_RUNTIME_TABLES[1],
            "payload_fields": ("event_id", "occurred_at", "tenant", "source_pbc", "data"),
        }
        for event_type in STREAMING_ANALYTICS_CONSUMED_EVENT_TYPES
    ),
    "retry_policy": {"name": "streaming_analytics_default_retry", "max_attempts": 5, "backoff": "exponential"},
    "idempotency": {"key_fields": ("event_type", "event_id", "handler"), "storage": STREAMING_ANALYTICS_RUNTIME_TABLES[1]},
}
...
def event_contract_manifest() -> dict:
    ...
    return {
        ...
        and EVENT_CONTRACT["topic"] == STREAMING_ANALYTICS_REQUIRED_EVENT_TOPIC
        and EVENT_CONTRACT["dead_letter_table"] == STREAMING_ANALYTICS_RUNTIME_TABLES[2]
