...
def agri_supply_chain_traceability_command_farm_lot(state, payload):
    next_state = _copy(state); record_id = payload.get('id') or payload.get('code') or f'farm_lot-1'; record = {'id': record_id, 'tenant': payload.get('tenant', 'default'), 'status': payload.get('status', 'active'), 'payload': dict(payload)}; next_state['records'][record_id] = record; _event(next_state, AGRI_SUPPLY_CHAIN_TRACEABILITY_EMITTED_EVENT_TYPES[0], record)
...
def agri_supply_chain_traceability_build_service_contract():
    return {'format': 'appgen.agri-supply-chain-traceability-service-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'command_methods': ('configure_runtime','set_parameter','register_rule','register_schema_extension','receive_event','command_farm_lot','run_advanced_assessment','parse_document_instruction') + DOMAIN_OPERATIONS, 'query_methods': ('query_workbench','build_workbench_view'), 'shared_table_access': False, 'transaction_boundary': 'owned_datastore_plus_outbox', 'event_contract': 'AppGen-X'}
...
def agri_supply_chain_traceability_build_workbench_view(tenant='default'):
    return {'ok': True, 'pbc': PBC_KEY, 'tenant': tenant, 'route': f'/workbench/pbcs/{PBC_KEY}', 'tables': AGRI_SUPPLY_CHAIN_TRACEABILITY_BUSINESS_TABLES, 'actions': DOMAIN_OPERATIONS, 'ui_fragments': AGRI_SUPPLY_CHAIN_TRACEABILITY_UI_FRAGMENT_KEYS, 'side_effects': ()}
