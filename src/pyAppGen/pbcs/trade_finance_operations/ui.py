...
def trade_finance_operations_ui_contract():
    surface = domain_capability_surface_contract()
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('TradeFinanceOperationsWorkbench',
 'TradeFinanceOperationsDetail',
 'TradeFinanceOperationsAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('trade_finance_operations.read',
 'trade_finance_operations.create',
 'trade_finance_operations.update',
 'trade_finance_operations.approve',
 'trade_finance_operations.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def trade_finance_operations_render_workbench():
    ui = trade_finance_operations_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': trade_finance_operations_ui_contract()['ok'] and trade_finance_operations_render_workbench()['ok'], 'side_effects': ()}
