"""AppGen-X event contract for the cross_border_trade PBC."""
    ...
    + tuple(
        {
            ...
            'schema': f"cross_border_trade.{event_type[0].lower()}{''.join('_' + char.lower() if char.isupper() else char for char in event_type[1:])}.emitted.v1",
...
def build_event_envelope(event_type, payload=None, *, direction='emitted', event_id='smoke-event'):
    ...
    if direction == 'emitted':
        envelope['pbc'] = supplied.get('pbc', 'cross_border_trade')
    else:
