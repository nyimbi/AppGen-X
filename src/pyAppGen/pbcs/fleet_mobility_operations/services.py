"""Service layer for the fleet_mobility_operations PBC."""
...
COMMAND_OPERATIONS = tuple(dict.fromkeys(('command_vehicle','configure_runtime','set_parameter','register_rule') + tuple(DOMAIN_DEPTH_COMMAND_OPERATIONS)))
QUERY_OPERATIONS = ('query_workbench',)
OWNED_TABLES = DOMAIN_DEPTH_OWNED_TABLES

def _operation_contract(name, kind):
    return {'operation': name, 'operation_kind': kind, 'owned_tables': OWNED_TABLES[:2] if kind == 'command' else (), 'read_tables': OWNED_TABLES[:2] if kind == 'query' else (), 'emitted_event': ('FleetMobilityOperationsCreated',
 'FleetMobilityOperationsUpdated',
 'FleetMobilityOperationsApproved',
 'FleetMobilityOperationsExceptionOpened')[0] if kind == 'command' else None, 'transaction_boundary': 'owned_datastore_plus_outbox' if kind == 'command' else 'read_only_projection'}

class FleetMobilityOperationsService:
...
def service_operation_manifest():
    return {'ok': True, 'pbc': PBC_KEY, 'service_class': 'FleetMobilityOperationsService', 'command_operations': COMMAND_OPERATIONS, 'query_operations': QUERY_OPERATIONS, 'event_contract': EVENT_CONTRACT, 'side_effects': ()}
...
def smoke_test():
    service = FleetMobilityOperationsService(); command = getattr(service, COMMAND_OPERATIONS[0])({'tenant': 'tenant-smoke'}); query = getattr(service, QUERY_OPERATIONS[0])({'tenant': 'tenant-smoke'})
