...
def mortgage_servicing_ui_contract():
    ...
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('MortgageServicingWorkbench', 'MortgageServicingDetail', 'MortgageServicingAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('mortgage_servicing.read',
 'mortgage_servicing.create',
 'mortgage_servicing.update',
 'mortgage_servicing.approve',
 'mortgage_servicing.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}
...
def smoke_test():
    return {'ok': mortgage_servicing_ui_contract()['ok'] and mortgage_servicing_render_workbench()['ok'], 'side_effects': ()}
