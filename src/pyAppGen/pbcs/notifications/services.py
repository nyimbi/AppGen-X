...
def _owned_tables(route: dict) -> tuple[str, ...]:
    return tuple(
        table if table.startswith("notifications_") else f"notifications_{table}"
...
def _build_operation_contracts() -> tuple[dict, ...]:
    ...
    for route in api["routes"]:
        ...
        if is_command and not table_scope and operation == "receive_event":
            table_scope = (NOTIFICATIONS_RUNTIME_TABLES[1], NOTIFICATIONS_RUNTIME_TABLES[2])
        contracts.append(
            {
                "operation": operation,
...
def service_operation_contracts() -> dict:
    ...
    runtime_service = notifications_build_service_contract()
    return {
...
def operation_plan(operation_name: str, payload: dict | None = None) -> dict:
    ...
    table_scope = contract["owned_tables"] or contract["read_tables"] or tuple(
        table if table.startswith("notifications_") else f"notifications_{table}"
        for table in notifications_build_service_contract()["runtime_tables"]
    )
    return {
...
def service_operation_manifest() -> dict:
    ...
    return {
        "ok": contracts["ok"],
        "pbc": "notifications",
        "service_class": NotificationsService.__name__,
