"""AppGen-X event contract for the notifications PBC."""
...
EVENT_CONTRACT = {
    ...
    "inbox_topic": f"{NOTIFICATIONS_REQUIRED_EVENT_TOPIC}.inbox",
    ...
    "emitted": tuple(
        {
            ...
            "schema": f"notifications.{event_type}.emitted.v1",
            "topic": NOTIFICATIONS_REQUIRED_EVENT_TOPIC,
            "outbox_table": NOTIFICATIONS_RUNTIME_TABLES[0],
            "payload_fields": ("event_id", "occurred_at", "pbc", "tenant", "data"),
        }
        for event_type in NOTIFICATIONS_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": f"notifications.{event_type}.consumed.v1",
            "topic": f"{NOTIFICATIONS_REQUIRED_EVENT_TOPIC}.inbox",
            "inbox_table": NOTIFICATIONS_RUNTIME_TABLES[1],
            "payload_fields": ("event_id", "occurred_at", "source_pbc", "tenant", "data"),
        }
        for event_type in NOTIFICATIONS_CONSUMED_EVENT_TYPES
    ),
    "retry_policy": {"name": "notifications_default_retry", "max_attempts": 5, "backoff": "exponential"},
    "idempotency": {"key_fields": ("event_type", "event_id", "handler"), "storage": NOTIFICATIONS_RUNTIME_TABLES[1]},
...
def event_contract_manifest() -> dict:
    ...
    return {
        ...
        and len(EMITTED_EVENTS) == len(NOTIFICATIONS_EMITTED_EVENT_TYPES)
        and len(CONSUMED_EVENTS) == len(NOTIFICATIONS_CONSUMED_EVENT_TYPES)
...
def build_event_envelope(
    ...
    *,
    ...
    if direction == "emitted":
        envelope["pbc"] = supplied.get("pbc", "notifications")
    else:
