"""Service layer for the enterprise_risk_controls PBC."""
...
COMMAND_OPERATIONS = ('command_risk_register', 'configure_runtime', 'set_parameter', 'register_rule')
...
def _operation_contract(name, kind):
    return {'operation': name, 'operation_kind': kind, 'owned_tables': OWNED_TABLES[:2] if kind == 'command' else (), 'read_tables': OWNED_TABLES[:2] if kind == 'query' else (), 'emitted_event': ('RiskAssessed', 'ControlTested', 'RemediationOpened', 'ControlAttested')[0] if kind == 'command' else None, 'transaction_boundary': 'owned_datastore_plus_outbox' if kind == 'command' else 'read_only_projection'}
...
def service_operation_manifest():
    return {'ok': True, 'pbc': PBC_KEY, 'service_class': 'EnterpriseRiskControlsService', 'command_operations': COMMAND_OPERATIONS, 'query_operations': QUERY_OPERATIONS, 'event_contract': EVENT_CONTRACT, 'side_effects': ()}
...
_BaseEnterpriseRiskControlsService = EnterpriseRiskControlsService

class EnterpriseRiskControlsService(_BaseEnterpriseRiskControlsService):
    def __getattr__(self, name):
