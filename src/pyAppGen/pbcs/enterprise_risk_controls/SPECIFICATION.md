# Enterprise Risk and Controls PBC Specification

## Stable Identity

The `enterprise_risk_controls` pbc is a package-owned business capability in the `platform` mesh. It owns the implementation directory `src/pyAppGen/pbcs/enterprise_risk_controls` and exposes a stable manifest, side-effect-free registration, discovery metadata, release evidence, and package-local tests. The PBC description is: Risk registers, control libraries, attestations, control testing, issue remediation, evidence, and policy mapping.

## Owned Boundary

The package owns its datastore boundary and does not mutate shared or foreign tables. All owned table names are generated under the `enterprise_risk_controls_` prefix in schema contracts, models, migrations, service operation contracts, event handlers, UI workbench projections, and agent CRUD plans. Cross-PBC collaboration is represented through APIs, AppGen-X events, and read-only projections rather than shared table writes.
