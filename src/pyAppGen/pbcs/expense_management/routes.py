"""API route contracts for the expense_management PBC."""
PBC_KEY = 'expense_management'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /expense-reports', 'POST /expense-lines', 'POST /receipt-documents', 'POST /expense-approvals', 'GET /expense-workbench'))
