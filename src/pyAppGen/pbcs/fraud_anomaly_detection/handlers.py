"""Idempotent event handlers for the fraud_anomaly_detection PBC."""

HANDLER_CONTRACTS = ({'event_type': 'CheckoutCompleted', 'function': 'handle_checkout_completed', 'idempotency_key': 'fraud_anomaly_detection:CheckoutCompleted:{event_id}', 'retry_policy': {'name': 'fraud_anomaly_detection_default_retry', 'max_attempts': 5, 'backoff': 'exponential'}, 'dead_letter_table': 'fraud_anomaly_detection_appgen_dead_letter_event', 'side_effect_boundary': 'owned_tables_or_declared_api_calls'}, {'event_type': 'PaymentCaptured', 'function': 'handle_payment_captured', 'idempotency_key': 'fraud_anomaly_detection:PaymentCaptured:{event_id}', 'retry_policy': {'name': 'fraud_anomaly_detection_default_retry', 'max_attempts': 5, 'backoff': 'exponential'}, 'dead_letter_table': 'fraud_anomaly_detection_appgen_dead_letter_event', 'side_effect_boundary': 'owned_tables_or_declared_api_calls'}, {'event_type': 'AccessPolicyChanged', 'function': 'handle_access_policy_changed', 'idempotency_key': 'fraud_anomaly_detection:AccessPolicyChanged:{event_id}', 'retry_policy': {'name': 'fraud_anomaly_detection_default_retry', 'max_attempts': 5, 'backoff': 'exponential'}, 'dead_letter_table': 'fraud_anomaly_detection_appgen_dead_letter_event', 'side_effect_boundary': 'owned_tables_or_declared_api_calls'})
_PROCESSED_KEYS = set()
