"""AI agent and chatbot skill contract for the fraud_anomaly_detection PBC."""
...
AGENT_NAME = 'FraudAnomalyDetectionAgent'
...
def document_instruction_plan(document=None, instructions=None):
    ...
    app_plan = document_instruction_fraud_anomaly_detection_plan(document_text, instruction_text)
    return {
...
def smoke_test():
    ...
    skills = agent_skill_manifest()
    chatbot = chatbot_interface_contract()
    document = document_instruction_plan('suspicious checkout', 'open analyst case')
    ...
    return {
        ...
        and create_plan['ok']
        and contribution['ok']
        and document['target_table'].startswith('fraud_anomaly_detection_')
