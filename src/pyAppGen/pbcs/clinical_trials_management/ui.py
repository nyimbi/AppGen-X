...
def clinical_trials_management_ui_contract():
    surface = domain_capability_surface_contract()
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('ClinicalTrialsManagementWorkbench',
 'ClinicalTrialsManagementDetail',
 'ClinicalTrialsManagementAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('clinical_trials_management.read',
 'clinical_trials_management.create',
 'clinical_trials_management.update',
 'clinical_trials_management.approve',
 'clinical_trials_management.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def clinical_trials_management_render_workbench():
    ui = clinical_trials_management_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': clinical_trials_management_ui_contract()['ok'] and clinical_trials_management_render_workbench()['ok'], 'side_effects': ()}
