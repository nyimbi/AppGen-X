from pyAppGen.pbcs.clinical_trials_management import implementation_contract, package_discovery_plan, package_metadata_manifest, validate_package_metadata
...
from pyAppGen.pbcs.clinical_trials_management.release_evidence import build_release_evidence, release_readiness_manifest, validate_release_evidence
from pyAppGen.pbcs.clinical_trials_management.events import event_contract_manifest, validate_event_contract
from pyAppGen.pbcs.clinical_trials_management.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.clinical_trials_management.services import service_operation_contracts
from pyAppGen.pbcs.clinical_trials_management.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.clinical_trials_management.config import governance_smoke_test
from pyAppGen.pbcs.clinical_trials_management.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'CustomerUpdated', 'SupplierQualified')[0], 'idempotency_key': 'idem-clinical_trials_management'})['ok'] is True
