...
def agent_skill_manifest():
    skills = tuple({'name': name, 'scope': PBC_KEY, 'description': f'{name} for {PBC_KEY}', 'requires_confirmation_for_mutation': True, 'uses_appgen_event_contract': True, 'stream_engine_picker_visible': False} for name in ('planning_budgeting_forecasting_guide_user', 'planning_budgeting_forecasting_read_records', 'planning_budgeting_forecasting_create_record', 'planning_budgeting_forecasting_update_record'))
