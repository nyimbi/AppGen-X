...
from .app_surface import document_instruction_multi_sided_market_plan, single_pbc_multi_sided_market_app_contract
...
SKILL_NAMES = tuple(f'{PBC_KEY}.{name}' for name in ('task_guidance','document_instruction_intake','governed_create','governed_read','governed_update','governed_delete','exchange_advisor','workbench_navigation','forms_wizards_and_controls'))
...
def chatbot_interface_contract():
    return {'ok': True, 'pbc': PBC_KEY, 'agent': AGENT_NAME, 'entrypoint': f'/assistant/pbc/{PBC_KEY}', 'single_agent_contribution': f'{PBC_KEY}_skills', 'capabilities': ('market_task_guidance','document_and_instruction_intake','governed_datastore_crud','exchange_recommendation','policy_and_permission_explanation','workbench_navigation','forms_wizards_and_controls'), 'professional_controls': ('citation_required_for_document_facts','mutation_preview_before_commit','permission_check_before_mutation','owned_table_boundary_check','audit_event_plan'), 'side_effects': ()}
...
def document_instruction_plan(document=None, instructions=None):
    ...
    app_plan = document_instruction_multi_sided_market_plan(str(document or ''), str(instructions or ''))
    return {'ok': bool(document or instructions) and app_plan['ok'], 'pbc': PBC_KEY, 'document_digest': digest, 'document_actions': DOCUMENT_ACTIONS, 'candidate_tables': _owned_tables(), 'candidate_operations': services.service_operation_manifest().get('command_operations', ()) + services.service_operation_manifest().get('query_operations', ()), 'proposed_operation': app_plan['proposed_operation'], 'target_table': app_plan['target_table'], 'market_plan': app_plan, 'requires_human_confirmation': True, 'side_effects': ()}
...
def composed_agent_contribution():
    ...
    return {'ok': skills['ok'] and chatbot['ok'] and standalone_app['ok'], 'pbc': PBC_KEY, 'agent': AGENT_NAME, 'single_agent_skill_namespace': f'{PBC_KEY}_skills', 'dsl_tools': (f'{PBC_KEY}_skills', f'{PBC_KEY}_documents', f'{PBC_KEY}_crud'), 'skills': tuple(item['name'] for item in skills['skills']), 'chatbot': chatbot, 'standalone_app': standalone_app, 'side_effects': ()}


def smoke_test():
    skills = agent_skill_manifest(); chatbot = chatbot_interface_contract(); document = document_instruction_plan('market listing document', 'create a rental listing'); read_plan = datastore_crud_plan('read'); create_plan = datastore_crud_plan('create', payload={'status': 'draft'}); contribution = composed_agent_contribution(); standalone_app = contribution['standalone_app']
    return {'ok': skills['ok'] and chatbot['ok'] and document['ok'] and document['target_table'].startswith('multi_sided_market_') and read_plan['ok'] and create_plan['ok'] and contribution['ok'] and contribution['standalone_app']['ok'], 'skills': skills, 'chatbot': chatbot, 'standalone_app': standalone_app, 'document': document, 'read_plan': read_plan, 'create_plan': create_plan, 'contribution': contribution, 'side_effects': ()}
