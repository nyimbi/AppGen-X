"""Generated contract smoke tests for predictive_demand."""
...
def test_generated_schema_service_and_release_evidence():
    ...
    assert SERVICE_CONTRACT['pbc'] == 'predictive_demand'
    assert SERVICE_CONTRACT['ok'] is True
    assert SERVICE_CONTRACT.get('shared_table_access') is False
    assert RELEASE_EVIDENCE['pbc'] == 'predictive_demand'
    assert RELEASE_EVIDENCE['ok'] is True


def test_manifest_and_event_contract():
    assert PBC_MANIFEST['pbc'] == 'predictive_demand'
    ...
    assert EVENT_CONTRACT['outbox_table'].startswith('predictive_demand_')
    assert EVENT_CONTRACT['inbox_table'].startswith('predictive_demand_')


def test_registration_plan_is_side_effect_free():
    from .. import register_pbc, registration_plan

    assert register_pbc()['pbc'] == 'predictive_demand'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert smoke['first_result']['dead_letter_table'].startswith('predictive_demand_')
