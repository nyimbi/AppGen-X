"""AI agent and chatbot skill contract for the predictive_demand PBC."""
...
AGENT_NAME = 'PredictiveDemandAgent'
...
def document_instruction_plan(document=None, instructions=None):
    ...
    app_plan = document_instruction_predictive_demand_plan(document_text, instruction_text)
    return {
        ...
        'predictive_demand_plan': app_plan,
...
def smoke_test():
    ...
    skills = agent_skill_manifest()
    chatbot = chatbot_interface_contract()
    document = document_instruction_plan('stockout risk', 'assess shortage coverage')
    ...
    return {
        ...
        and create_plan['ok']
        and contribution['ok']
        and document['target_table'].startswith('predictive_demand_')
