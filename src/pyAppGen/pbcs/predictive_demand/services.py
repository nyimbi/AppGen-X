...
def service_operation_contracts() -> dict:
    ...
    command_contracts = tuple(
        item for item in OPERATION_CONTRACTS if item["operation_kind"] == "command"
    )
    query_contracts = tuple(
        item for item in OPERATION_CONTRACTS if item["operation_kind"] == "query"
    )
    runtime_service = predictive_demand_build_service_contract()
    return {
...
def operation_plan(operation_name: str, payload: dict | None = None) -> dict:
    ...
    contract = next(
        (item for item in OPERATION_CONTRACTS if item["operation"] == operation_name),
        None,
    )
    if contract is None:
        return {
    ...
    return {
        "ok": bool(
            contract["owned_tables"] or contract["read_tables"] or contract["consumed_event"]
        ),
        "pbc": PBC_KEY,
        "operation": operation_name,
...
class PredictiveDemandService:
    ...
    def execute_operation(self, operation_name: str, payload: dict | None = None) -> dict:
        ...
        if plan.get("operation_kind") == "command":
            ...
            result.update(
                {"query": operation_name, "read_only": True, "outbox_table": None, "emits": ()}
            )
        return result
...
def service_operation_manifest() -> dict:
    ...
    return {
        "ok": contracts["ok"],
        "pbc": PBC_KEY,
        "service_class": PredictiveDemandService.__name__,
