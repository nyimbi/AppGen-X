...
COURT_CASE_MANAGEMENT_EMITTED_EVENT_TYPES = ('CourtCaseManagementCreated', 'CourtCaseManagementUpdated', 'CourtCaseManagementApproved', 'CourtCaseManagementExceptionOpened')
COURT_CASE_MANAGEMENT_CONSUMED_EVENT_TYPES = ('PolicyChanged', 'CustomerUpdated', 'SupplierQualified')
COURT_CASE_MANAGEMENT_STANDARD_FEATURE_KEYS = (
...
COURT_CASE_MANAGEMENT_RUNTIME_CAPABILITY_KEYS = (
...
COURT_CASE_MANAGEMENT_BUSINESS_TABLES = COURT_CASE_MANAGEMENT_OWNED_TABLES[:-3]
COURT_CASE_MANAGEMENT_API_ROUTES = (
    'POST /court-cases',
    'POST /filings',
    'POST /evidence',
    'POST /hearings',
...
def court_case_management_command_court_case(state, payload):
    next_state = _copy(state)
    record_id = payload.get('id') or payload.get('code') or 'court_case-1'
    ...
    _event(next_state, COURT_CASE_MANAGEMENT_EMITTED_EVENT_TYPES[0], record)
...
def court_case_management_parse_document_instruction(document, instruction):
    return {'ok': True, 'candidate_tables': COURT_CASE_MANAGEMENT_BUSINESS_TABLES[:5], 'instruction': instruction, 'document_digest': _digest(document), 'requires_human_confirmation': True, 'side_effects': ()}


def court_case_management_build_schema_contract():
    table_contracts = tuple(
        {'table': table, 'fields': ('id', 'tenant', 'code', 'status', 'version', 'payload', 'created_at', 'updated_at'), 'primary_key': ('id',), 'owned_by': PBC_KEY}
        for table in COURT_CASE_MANAGEMENT_OWNED_TABLES
    )
    return {
        'format': 'appgen.court-case-management-owned-schema-contract.v1',
        'ok': True,
        'pbc': PBC_KEY,
        'tables': table_contracts,
        'migrations': tuple({'path': f'pbcs/court_case_management/migrations/{i + 1:03d}_{table["table"]}.sql', 'operation': 'create_owned_table', 'table': table['table'], 'backend_allowlist': COURT_CASE_MANAGEMENT_ALLOWED_DATABASE_BACKENDS} for i, table in enumerate(table_contracts)),
        ...
        'datastore_backends': COURT_CASE_MANAGEMENT_ALLOWED_DATABASE_BACKENDS,
...
def court_case_management_build_service_contract():
    return {
        'format': 'appgen.court-case-management-service-contract.v1',
        'ok': True,
        'pbc': PBC_KEY,
        'command_methods': ('configure_runtime', 'set_parameter', 'register_rule', 'register_schema_extension', 'receive_event', 'command_court_case', 'run_advanced_assessment', 'parse_document_instruction', 'create_court_case', 'receive_filing', 'register_evidence', 'schedule_hearing', 'create_task', 'complete_task', 'draft_order', 'sign_and_enter_order') + tuple(DOMAIN_OPERATIONS),
...
def court_case_management_build_api_contract():
    return {'format': 'appgen.court-case-management-api-contract.v1', 'ok': True, 'pbc': PBC_KEY, 'routes': COURT_CASE_MANAGEMENT_API_ROUTES, 'event_contract': 'AppGen-X', 'stream_engine_picker_visible': False, 'owned_tables': COURT_CASE_MANAGEMENT_OWNED_TABLES}


def court_case_management_build_release_evidence():
    from .standalone import documentation_presence, pbc_generation_smoke_audit, pbc_implementation_release_audit, pbc_source_artifact_contract
    ...
    return {
        ...
        'checks': checks,
        'generated_artifacts': {
            'migrations': court_case_management_build_schema_contract()['migrations'],
            'models': court_case_management_build_schema_contract()['models'],
...
def court_case_management_permissions_contract():
    return {'ok': True, 'pbc': PBC_KEY, 'permissions': ('court_case_management.read', 'court_case_management.create', 'court_case_management.update', 'court_case_management.approve', 'court_case_management.admin'), 'roles': ('operator', 'approver', 'auditor'), 'side_effects': ()}


def court_case_management_build_workbench_view(tenant='default'):
    return {'ok': True, 'pbc': PBC_KEY, 'tenant': tenant, 'route': f'/workbench/pbcs/{PBC_KEY}', 'tables': COURT_CASE_MANAGEMENT_BUSINESS_TABLES, 'actions': DOMAIN_OPERATIONS, 'ui_fragments': COURT_CASE_MANAGEMENT_UI_FRAGMENT_KEYS, 'sections': ('intake', 'docket', 'calendar', 'evidence', 'tasks', 'release_evidence'), 'side_effects': ()}
...
def court_case_management_runtime_smoke():
    ...
    cfg = court_case_management_configure_runtime(state, {'database_backend': 'postgresql', 'event_topic': COURT_CASE_MANAGEMENT_REQUIRED_EVENT_TOPIC})
