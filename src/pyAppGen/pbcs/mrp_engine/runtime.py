...
def mrp_engine_runtime_smoke() -> dict:
    ...
    release_evidence = mrp_engine_build_release_evidence()
    ...
    checks = (
        ...
        {"id": "universal_api_async_streaming", "ok": api["ok"] and schema["ok"] and service["ok"] and release_evidence["ok"] and "PlannedOrderReleased" in api["events"]["emits"]},
...
def mrp_engine_build_schema_contract() -> dict:
    default_fields = ("tenant", "record_id", "source_id", "status", "effective_at", "audit_hash")
    table_fields = {
        "bill_of_material": ("bom_id", "tenant", "parent_item", "site", "status", "current_revision", "audit_hash"),
        "bom_revision": ("revision_id", "bom_id", "revision", "status", "valid_from", "released_by"),
        "bom_component": ("component_id", "bom_id", "revision_id", "component_item", "component_qty", "scrap_percent", "sequence"),
        "bom_alternate": ("alternate_id", "bom_id", "alternate_code", "priority", "status", "effective_from"),
        "bom_substitution_rule": ("substitution_id", "component_item", "substitute_item", "rule_id", "priority", "status"),
        "item_planning_profile": ("profile_id", "item", "site", "item_type", "lot_policy", "lead_time_days", "planner"),
        "item_source_rule": ("source_rule_id", "item", "site", "route", "supplier_ref", "work_center_ref"),
        "material_demand": ("demand_id", "tenant", "item", "site", "quantity", "source", "need_date", "status"),
        "demand_projection_line": ("line_id", "demand_id", "bucket_start", "bucket_end", "quantity", "confidence"),
        "demand_forecast_snapshot": ("forecast_snapshot_id", "item", "site", "horizon_days", "quantity", "confidence"),
        "sales_order_demand_projection": ("order_projection_id", "order_ref", "item", "site", "quantity", "need_date"),
        "safety_stock_policy": ("policy_id", "item", "site", "minimum_qty", "multiplier", "review_cycle"),
        "inventory_projection": ("inventory_id", "tenant", "item", "site", "available_qty", "quality_status", "status"),
        "inventory_lot_projection": ("lot_projection_id", "inventory_id", "lot_ref", "available_qty", "expiry_date"),
        "inventory_reservation_projection": ("reservation_id", "inventory_id", "demand_id", "reserved_qty", "status"),
        "quality_hold_projection": ("hold_projection_id", "item", "site", "lot_ref", "release_status", "event_ref"),
        "capacity_projection": ("capacity_id", "tenant", "site", "resource", "capacity_qty", "bucket_start", "bucket_end"),
        "capacity_bucket": ("bucket_id", "capacity_id", "bucket_start", "bucket_end", "available_capacity"),
        "work_center_capacity_projection": ("work_center_projection_id", "work_center_ref", "site", "available_capacity", "event_ref"),
        "supplier_lead_time_projection": ("supplier_projection_id", "supplier_ref", "item", "site", "lead_time_days", "event_ref"),
        "production_capacity_projection": ("production_projection_id", "line_ref", "site", "available_capacity", "event_ref"),
        "mrp_run": ("run_id", "tenant", "site", "horizon_days", "scenario", "planner", "status", "shortage_total"),
        "mrp_run_item": ("run_item_id", "run_id", "item", "gross_requirement", "net_requirement", "coverage_status"),
        "mrp_run_bucket": ("run_bucket_id", "run_id", "bucket_start", "bucket_end", "shortage_qty", "capacity_gap"),
        "mrp_scenario": ("scenario_id", "run_id", "scenario_name", "baseline", "created_by", "status"),
        "mrp_plan_version": ("plan_version_id", "run_id", "version", "approved_by", "status"),
        "planned_order": ("planned_order_id", "tenant", "run_id", "item", "site", "quantity", "route", "status"),
        "planned_order_component": ("planned_component_id", "planned_order_id", "component_item", "required_qty", "bom_component_id"),
        "planned_purchase_suggestion": ("purchase_suggestion_id", "planned_order_id", "supplier_ref", "lead_time_days", "status"),
        "planned_production_order": ("production_order_id", "planned_order_id", "work_center_ref", "capacity_qty", "status"),
        "planned_transfer_order": ("transfer_order_id", "planned_order_id", "from_site", "to_site", "quantity", "status"),
        "material_shortage": ("shortage_id", "run_id", "item", "site", "shortage_qty", "severity", "status"),
        "shortage_pegging": ("shortage_pegging_id", "shortage_id", "demand_id", "planned_order_id", "quantity"),
        "supply_demand_pegging": ("pegging_id", "run_id", "demand_id", "supply_ref", "quantity"),
        "planning_exception": ("exception_id", "run_id", "exception_type", "severity", "recommendation", "status"),
        "exception_resolution_plan": ("resolution_id", "exception_id", "action", "owner", "due_at", "status"),
        "release_route": ("release_route_id", "route", "target_api", "idempotency_policy", "status"),
        "planning_policy_screening": ("screening_id", "run_id", "policy_id", "decision", "reason"),
        "planning_audit_trace": ("trace_id", "event_id", "previous_hash", "hash", "actor"),
        "supply_availability_proof": ("proof_id", "planned_order_id", "proof_hash", "public_claims", "status"),
        "mrp_federation_projection": ("projection_id", "planned_order_id", "system", "payload_hash", "status"),
        "carbon_planning_window": ("window_id", "site", "window_start", "window_end", "carbon_score"),
        "material_allocation_optimization": ("optimization_id", "run_id", "objective_score", "selected_plan", "status"),
        "capacity_allocation": ("allocation_id", "run_id", "resource", "allocated_capacity", "clearing_priority"),
        "shortage_anomaly_signal": ("signal_id", "run_id", "entropy", "outlier_count", "status"),
        "material_risk_model": ("model_id", "name", "auc", "drift_score", "feature_lineage", "status"),
        "shortage_forecast": ("forecast_id", "item", "site", "forecast_demand", "forecast_shortage", "horizon"),
        "planning_parsed_instruction": ("instruction_id", "run_id", "item", "site", "action", "confidence"),
        "mrp_seed_data": ("seed_id", "seed_type", "key", "value", "status"),
        "mrp_schema_extension": ("extension_id", "table_name", "field_name", "field_type", "status"),
        "mrp_control_assertion": ("assertion_id", "control_name", "result", "evidence_hash", "tested_at"),
        "mrp_governed_model": ("governed_model_id", "model_name", "regulated", "explainability_required", "status"),
        "mrp_rule": ("rule_id", "tenant", "scope", "status", "compiled_hash"),
        "mrp_parameter": ("parameter_id", "name", "value", "effective_at", "status"),
        "mrp_configuration": ("configuration_id", "database_backend", "event_topic", "retry_limit", "status"),
        "mrp_engine_appgen_outbox_event": ("event_id", "event_type", "payload", "idempotency_key", "created_at"),
        "mrp_engine_appgen_inbox_event": ("event_id", "event_type", "payload", "idempotency_key", "attempts"),
        "mrp_engine_dead_letter_event": ("event_id", "event_type", "reason", "payload", "failed_at"),
    }
    relationships = (
        ("bom_revision", "bill_of_material", "bom_id"),
        ("bom_component", "bill_of_material", "bom_id"),
        ("bom_component", "bom_revision", "revision_id"),
        ("bom_alternate", "bill_of_material", "bom_id"),
        ("demand_projection_line", "material_demand", "demand_id"),
        ("sales_order_demand_projection", "material_demand", "order_projection_id"),
        ("inventory_lot_projection", "inventory_projection", "inventory_id"),
        ("inventory_reservation_projection", "inventory_projection", "inventory_id"),
        ("capacity_bucket", "capacity_projection", "capacity_id"),
        ("mrp_run_item", "mrp_run", "run_id"),
        ("mrp_run_bucket", "mrp_run", "run_id"),
        ("mrp_scenario", "mrp_run", "run_id"),
        ("mrp_plan_version", "mrp_run", "run_id"),
        ("planned_order", "mrp_run", "run_id"),
        ("planned_order_component", "planned_order", "planned_order_id"),
        ("planned_order_component", "bom_component", "bom_component_id"),
        ("planned_purchase_suggestion", "planned_order", "planned_order_id"),
        ("planned_production_order", "planned_order", "planned_order_id"),
        ("planned_transfer_order", "planned_order", "planned_order_id"),
        ("material_shortage", "mrp_run", "run_id"),
        ("shortage_pegging", "material_shortage", "shortage_id"),
        ("supply_demand_pegging", "mrp_run", "run_id"),
        ("planning_exception", "mrp_run", "run_id"),
        ("exception_resolution_plan", "planning_exception", "exception_id"),
    )
    allowed_prefixes = (
        "bill_",
        "bom_",
    ...
    tables = tuple(
        ...
        for table in MRP_ENGINE_OWNED_TABLES
    )
    migrations = tuple(
        {
            "path": f"pbcs/mrp_engine/migrations/{position + 1:03d}_{table}.sql",
            "table": table,
            "operation": "create_owned_table",
        }
        for position, table in enumerate(MRP_ENGINE_OWNED_TABLES)
    )
    models = tuple({"path": f"pbcs/mrp_engine/models/{table}.py", "table": table, "class_name": _class_name(table)} for table in MRP_ENGINE_OWNED_TABLES)
    invalid_prefixes = tuple(table for table in MRP_ENGINE_OWNED_TABLES if not table.startswith(allowed_prefixes))
    return {
        "ok": not invalid_prefixes and len(tables) == len(MRP_ENGINE_OWNED_TABLES) and len(migrations) == len(MRP_ENGINE_OWNED_TABLES),
        "format": "appgen.mrp-engine-owned-schema-contract.v1",
        "tables": tables,
        "relationships": relationships,
        "migrations": migrations,
        "models": models,
        "allowed_prefixes": allowed_prefixes,
        "datastore_backends": MRP_ENGINE_ALLOWED_DATABASE_BACKENDS,
        "shared_table_access": False,
        "invalid_prefixes": invalid_prefixes,
    }


def mrp_engine_build_service_contract() -> dict:
    command_methods = (
        "configure_runtime",
        "set_parameter",
        "register_rule",
        ...
        "release_planned_order",
        "route_supply",
        "generate_supply_proof",
        "screen_policy",
        "federate_plan_view",
        "verify_item_identity",
        "run_resilience_drill",
        "rotate_crypto_epoch",
        "schedule_carbon_aware_plan",
        "optimize_material_allocation",
        "allocate_capacity",
        "run_control_tests",
    ...
    query_methods = (
        "build_workbench_view",
        "simulate_planning_policy",
        "forecast_shortage",
        "parse_planning_instruction",
        "score_planning_risk",
        "recommend_exception_resolution",
        "detect_shortage_anomaly",
        "model_stochastic_material_exposure",
        "build_api_contract",
        "build_schema_contract",
        "build_release_evidence",
    )
    return {
        "ok": len(command_methods) >= 25 and not mrp_engine_verify_owned_table_boundary(MRP_ENGINE_OWNED_TABLES)["violations"],
        "format": "appgen.mrp-engine-service-contract.v1",
        "command_methods": command_methods,
        "query_methods": query_methods,
        "mutates_only": MRP_ENGINE_OWNED_TABLES,
        "external_dependencies": {
            "apis": tuple(item for item in _MRP_ENGINE_ALLOWED_DEPENDENCIES if "/" in item),
            "events": MRP_ENGINE_CONSUMED_EVENT_TYPES,
            "projections": tuple(item for item in _MRP_ENGINE_ALLOWED_DEPENDENCIES if "/" not in item),
            "shared_tables": (),
        },
        "idempotent_handlers": ("receive_event", "release_planned_order", "route_supply"),
        "rules_parameters_configuration": ("register_rule", "set_parameter", "configure_runtime"),
    }


def mrp_engine_build_release_evidence() -> dict:
    schema = mrp_engine_build_schema_contract()
    service = mrp_engine_build_service_contract()
    api = mrp_engine_build_api_contract()
    permissions = mrp_engine_permissions_contract()
    checks = (
        {"id": "owned_schema_depth", "ok": len(schema["tables"]) >= 40},
        {"id": "migration_per_owned_table", "ok": len(schema["migrations"]) == len(MRP_ENGINE_OWNED_TABLES)},
        {"id": "service_command_depth", "ok": len(service["command_methods"]) >= 25},
        {"id": "api_event_contract", "ok": api["event_contract"] == "AppGen-X" and api["stream_engine_picker_visible"] is False},
        {"id": "permissions_cover_runtime", "ok": all(action in permissions["action_permissions"] for action in ("create_mrp_run", "release_planned_order", "receive_event"))},
        {"id": "backend_allowlist", "ok": api["database_backends"] == MRP_ENGINE_ALLOWED_DATABASE_BACKENDS},
    ...
    return {
        ...
        "format": "appgen.mrp-engine-release-evidence.v1",
        "checks": checks,
        "blocking_gaps": blocking_gaps,
        "owned_table_count": len(MRP_ENGINE_OWNED_TABLES),
        "service_command_count": len(service["command_methods"]),
        "migration_count": len(schema["migrations"]),
    }
...
def mrp_engine_permissions_contract() -> dict:
    return {
        ...
        "action_permissions": {
            ...
            "route_supply": "mrp_engine.release",
            "screen_policy": "mrp_engine.audit",
            "federate_plan_view": "mrp_engine.read",
            "verify_item_identity": "mrp_engine.audit",
            "run_resilience_drill": "mrp_engine.audit",
            "rotate_crypto_epoch": "mrp_engine.audit",
            "schedule_carbon_aware_plan": "mrp_engine.plan",
            "optimize_material_allocation": "mrp_engine.plan",
            "allocate_capacity": "mrp_engine.plan",
            "register_governed_model": "mrp_engine.configure",
            "recommend_exception_resolution": "mrp_engine.plan",
            "detect_shortage_anomaly": "mrp_engine.audit",
            "model_stochastic_material_exposure": "mrp_engine.audit",
            "parse_planning_instruction": "mrp_engine.plan",
            "score_planning_risk": "mrp_engine.plan",
            "forecast_shortage": "mrp_engine.plan",
            "simulate_planning_policy": "mrp_engine.plan",
            "build_schema_contract": "mrp_engine.audit",
            "build_service_contract": "mrp_engine.audit",
            "build_release_evidence": "mrp_engine.audit",
        },
    }
