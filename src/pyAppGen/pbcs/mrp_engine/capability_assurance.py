"""Executable capability assurance for this people/manufacturing PBC."""
...
_UI_FEATURE_HINTS = ("workbench", "ui", "console", "view", "dashboard", "queue", "studio")
...
def _logical_table_aliases(table: str) -> tuple[str, ...]:
    aliases = [table]
    if table.startswith(PBC_KEY + "_"):
        aliases.append(table.removeprefix(PBC_KEY + "_"))
    for prefix in ("personnel_", "time_", "payroll_", "talent_", "mrp_", "production_", "quality_", "eam_"):
        if table.startswith(prefix):
            aliases.append(table.removeprefix(prefix))
...
def _feature_evidence(feature: str, runtime_capabilities: dict) -> tuple[str, ...]:
    ...
    owned_tables = tuple(dict.fromkeys((*_owned_tables_from_schema(), *runtime_capabilities.get("owned_tables", ()))))
    owned_table_aliases = {alias for table in owned_tables for alias in _logical_table_aliases(table)}
    ...
    if feature in owned_table_aliases or any(feature.endswith(alias) for alias in owned_table_aliases):
...
def _boundary_rejection_probe() -> dict:
    verifier = _runtime_function("verify_owned_table_boundary")
    return verifier(("foreign_shared_operational_table",))
...
def table_stakes_capability_manifest() -> dict:
    ...
    boundary_rejection = _boundary_rejection_probe()
...
def validate_table_stakes_capability_coverage() -> dict:
    ...
    boundary_owned_tables = set(manifest["boundary_rejection"].get("owned_tables", ()))
    boundary_runtime_tables = set(manifest["boundary_rejection"].get("runtime_tables", ()))
    valid_owned_tables = schema_owned_tables | manifest_tables | boundary_owned_tables | boundary_runtime_tables
    invalid_tables = tuple(
        table
        for table in runtime_capabilities.get("owned_tables", ())
        if table not in valid_owned_tables
    )
    ...
    boundary_rejection_gap = manifest["boundary_rejection"].get("ok") is not False or not manifest["boundary_rejection"].get("violations")
    ...
    blocking_gaps = tuple(
        ...
        for gap in (
            ...
            ("boundary_rejection_gap", ("foreign_shared_operational_table",)) if boundary_rejection_gap else None,
        )
        if gap is not None
    )
    return {
        ...
        "boundary_rejection": manifest["boundary_rejection"],
        "side_effects": (),
    }
