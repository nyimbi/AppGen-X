from .runtime import telecom_network_operations_build_release_evidence
from .standalone import standalone_smoke_test

def build_release_evidence():
    return telecom_network_operations_build_release_evidence()
...
# Improve1 telecom network operations control release extension.
...
_TELECOM_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = build_release_evidence
_TELECOM_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE = validate_release_evidence
...
def validate_release_evidence() -> dict:
    validation = dict(_TELECOM_CONTROL_BASE_VALIDATE_RELEASE_EVIDENCE())
    control = _improve1_telecom_network_operations_control_contract()
    validation["ok"] = validation.get("ok") is True and control["ok"]
    validation["telecom_network_operations_control"] = control
