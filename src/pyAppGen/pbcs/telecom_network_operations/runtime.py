...
# Improve1 telecom network operations control extension.
from .telecom_network_operations_control import evaluate_telecom_network_operations_control, improve1_telecom_network_operations_control_contract

_TELECOM_CONTROL_BASE_RUNTIME_CAPABILITIES = telecom_network_operations_runtime_capabilities
_TELECOM_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = telecom_network_operations_build_release_evidence


def telecom_network_operations_runtime_capabilities():
    runtime = dict(_TELECOM_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_telecom_network_operations_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["telecom_network_operations_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_telecom_network_operations_control", "improve1_telecom_network_operations_control_contract")))
...
def telecom_network_operations_build_release_evidence():
    evidence = dict(_TELECOM_CONTROL_BASE_BUILD_RELEASE_EVIDENCE())
    control = improve1_telecom_network_operations_control_contract()
    artifacts = dict(evidence.get("generated_artifacts", {}))
    artifacts["telecom_network_operations_control"] = {
    ...
    checks = tuple(evidence.get("checks", ())) + ({"id": "improve1_telecom_network_operations_control", "ok": control["ok"]},)
    evidence.update({
        "ok": bool(evidence.get("ok")) and control["ok"],
        "checks": checks,
        "generated_artifacts": artifacts,
        "telecom_network_operations_control": control,
