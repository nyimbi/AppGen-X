...
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /success-accounts', 'POST /health-scores', 'POST /onboarding-plans', 'POST /renewal-plans', 'GET /customer-success-workbench'))
