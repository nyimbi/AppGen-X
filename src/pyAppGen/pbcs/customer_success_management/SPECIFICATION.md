# Customer Success Management PBC Specification

## Stable Identity

The `customer_success_management` pbc is a package-owned business capability in the `relationship` mesh. It owns the implementation directory `src/pyAppGen/pbcs/customer_success_management` and exposes a stable manifest, side-effect-free registration, discovery metadata, release evidence, and package-local tests. The PBC description is: Health scores, onboarding plans, adoption telemetry, renewals, expansion signals, playbooks, and churn risk workflows.

## Owned Boundary

The package owns its datastore boundary and does not mutate shared or foreign tables. All owned table names are generated under the `customer_success_management_` prefix in schema contracts, models, migrations, service operation contracts, event handlers, UI workbench projections, and agent CRUD plans. Cross-PBC collaboration is represented through APIs, AppGen-X events, and read-only projections rather than shared table writes.
