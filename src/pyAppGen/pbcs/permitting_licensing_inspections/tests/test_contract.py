...
from pyAppGen.pbcs.permitting_licensing_inspections.service_contract import build_service_contract
...
from pyAppGen.pbcs.permitting_licensing_inspections.handlers import dispatch_event, handler_manifest
from pyAppGen.pbcs.permitting_licensing_inspections.services import service_operation_contracts
from pyAppGen.pbcs.permitting_licensing_inspections.routes import api_route_contracts, validate_api_route_contracts
from pyAppGen.pbcs.permitting_licensing_inspections.config import governance_smoke_test
from pyAppGen.pbcs.permitting_licensing_inspections.agent import agent_skill_manifest, chatbot_interface_contract, document_instruction_plan, datastore_crud_plan
...
def test_manifest_and_event_contract():
    assert implementation_contract()['pbc'] == 'permitting_licensing_inspections'
...
def test_event_handlers_are_idempotent_and_retryable():
    ...
    assert dispatch_event({'event_type': ('PolicyChanged', 'CustomerUpdated', 'SupplierQualified')[0], 'idempotency_key': 'idem-permitting_licensing_inspections'})['ok'] is True
    assert dispatch_event({'event_type': 'Unexpected', 'idempotency_key': 'bad-permitting_licensing_inspections'})['dead_letter_table'].endswith('dead_letter_event')
