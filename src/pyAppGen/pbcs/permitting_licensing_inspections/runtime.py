...
# Improve1 permit control extension.
from .permit_control import evaluate_permit_control, improve1_permit_control_contract

_PERMITTING_BASE_RUNTIME_CAPABILITIES = permitting_licensing_inspections_runtime_capabilities
_PERMITTING_BASE_BUILD_RELEASE_EVIDENCE = permitting_licensing_inspections_build_release_evidence


def permitting_licensing_inspections_runtime_capabilities():
    runtime = dict(_PERMITTING_BASE_RUNTIME_CAPABILITIES())
    control = improve1_permit_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["permit_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_permit_control", "improve1_permit_control_contract")))
    runtime["improve1_control_owned_tables"] = control["owned_tables"]
    return runtime


def permitting_licensing_inspections_build_release_evidence():
    evidence = dict(_PERMITTING_BASE_BUILD_RELEASE_EVIDENCE())
    control = improve1_permit_control_contract()
    artifacts = dict(evidence.get("generated_artifacts", {}))
    artifacts["permit_control"] = {"contract": control["format"], "capability_count": control["capability_count"], "owned_tables": control["owned_tables"], "service_apis": tuple(item["evidence"]["service_api"] for item in control["capabilities"]), "ui_surfaces": tuple(item["evidence"]["ui_surface"] for item in control["capabilities"]), "test": "tests/test_domain_behavior.py"}
    checks = tuple(evidence.get("checks", ())) + ({"id": "improve1_permit_control", "ok": control["ok"]},)
    evidence.update({"ok": bool(evidence.get("ok")) and control["ok"], "checks": checks, "generated_artifacts": artifacts, "permit_control": control, "blocking_gaps": tuple(evidence.get("blocking_gaps", ())) + tuple(control.get("blocking_gaps", ()))})
    return evidence
