...
def permitting_licensing_inspections_ui_contract():
    ...
    return {'ok': True, 'pbc': PBC_KEY, 'fragments': ('PermittingLicensingInspectionsWorkbench',
 'PermittingLicensingInspectionsDetail',
 'PermittingLicensingInspectionsAssistantPanel'), 'configuration_editor': True, 'stream_engine_picker_visible': False, 'action_permissions': ('permitting_licensing_inspections.read',
 'permitting_licensing_inspections.create',
 'permitting_licensing_inspections.update',
 'permitting_licensing_inspections.approve',
 'permitting_licensing_inspections.admin'), 'full_capability_surface': {'operation_actions': DOMAIN_OPERATIONS, 'rule_editors': DOMAIN_RULES, 'parameter_editors': DOMAIN_PARAMETERS, 'advanced_panels': DOMAIN_ADVANCED_CAPABILITIES, 'table_browsers': DOMAIN_OWNED_TABLES, 'edge_case_queues': DOMAIN_EDGE_CASES, 'agent_tools': tuple(f'{PBC_KEY}_skills.{op}' for op in DOMAIN_OPERATIONS), 'navigation_sections': ('overview','operations','edge_case_triage','advanced_intelligence','release_evidence'), 'coverage': surface['coverage']}, 'side_effects': ()}

def permitting_licensing_inspections_render_workbench():
    ui = permitting_licensing_inspections_ui_contract(); full = ui['full_capability_surface']
...
def smoke_test():
    return {'ok': permitting_licensing_inspections_ui_contract()['ok'] and permitting_licensing_inspections_render_workbench()['ok'], 'side_effects': ()}
