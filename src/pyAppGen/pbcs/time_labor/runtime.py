...
# Improve1 time labor control extension.
from .time_labor_control import evaluate_time_labor_control, improve1_time_labor_control_contract

_TIME_CONTROL_BASE_RUNTIME_CAPABILITIES = time_labor_runtime_capabilities
_TIME_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = time_labor_build_release_evidence


def time_labor_runtime_capabilities():
    runtime = dict(_TIME_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_time_labor_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["time_labor_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_time_labor_control", "improve1_time_labor_control_contract")))
    runtime["improve1_control_owned_tables"] = control["owned_tables"]
    runtime["allowed_database_backends"] = control["allowed_database_backends"]
    runtime["event_contract"] = "AppGen-X"
    runtime["stream_engine_picker_visible"] = False
    return runtime


def time_labor_build_release_evidence():
    evidence = dict(_TIME_CONTROL_BASE_BUILD_RELEASE_EVIDENCE())
    control = improve1_time_labor_control_contract()
    artifacts = dict(evidence.get("generated_artifacts", {}))
    artifacts["time_labor_control"] = {
