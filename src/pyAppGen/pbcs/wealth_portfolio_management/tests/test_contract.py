from pyAppGen.pbcs.wealth_portfolio_management import (
    implementation_contract,
    package_discovery_plan,
    package_metadata_manifest,
    validate_package_metadata,
)
from pyAppGen.pbcs.wealth_portfolio_management.agent import (
...
from pyAppGen.pbcs.wealth_portfolio_management.events import (
...
from pyAppGen.pbcs.wealth_portfolio_management.release_evidence import (
    build_release_evidence,
    release_readiness_manifest,
    validate_release_evidence,
)
from pyAppGen.pbcs.wealth_portfolio_management.routes import (
    api_route_contracts,
...
def test_agent_chatbot_skills_are_executable():
    ...
    assert all(item["requires_confirmation_for_mutation"] is True for item in skills["skills"])
    assert chatbot_interface_contract()["ok"] is True
    assert composed_agent_contribution()["ok"] is True
    assert document_instruction_plan("doc", "create portfolio")["ok"] is True
        ...
        dispatch_event(
            {
                ...
                "idempotency_key": "idem-wealth_portfolio_management",
            }
        )["ok"]
        is True
    )
        ...
        dispatch_event(
            {
                ...
                "idempotency_key": "bad-wealth_portfolio_management",
            }
