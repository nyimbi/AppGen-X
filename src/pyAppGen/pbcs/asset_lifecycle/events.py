"""AppGen-X event contract for the asset_lifecycle PBC."""
...
def _emitted_events() -> tuple[dict, ...]:
    return tuple(
        {
            "event_type": event_type,
            "schema": f"asset_lifecycle.{event_type.lower()}.emitted.v1",
        ...
        for event_type in ASSET_LIFECYCLE_EMITTED_EVENT_TYPES
    )


def _consumed_events() -> tuple[dict, ...]:
    return tuple(
        {
            "event_type": event_type,
            "schema": f"asset_lifecycle.{event_type.lower()}.consumed.v1",
            "topic": ASSET_LIFECYCLE_REQUIRED_EVENT_TOPIC,
            "inbox_table": "asset_lifecycle_appgen_inbox_event",
            "payload_fields": ("event_id", "occurred_at", "source_pbc", "data"),
        }
        for event_type in ASSET_LIFECYCLE_CONSUMED_EVENT_TYPES
    )
...
def build_event_envelope(event_type, payload=None, *, direction="emitted", event_id="smoke-event"):
    ...
    if direction == "emitted":
        envelope["pbc"] = supplied.get("pbc", "asset_lifecycle")
    else:
