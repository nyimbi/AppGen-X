"""AppGen-X event contract for the enterprise_search_vector PBC."""
...
EVENT_CONTRACT = {
    ...
    "inbox_topic": f"{ENTERPRISE_SEARCH_VECTOR_REQUIRED_EVENT_TOPIC}.inbox",
    ...
    "emitted": tuple(
        {
            ...
            "topic": ENTERPRISE_SEARCH_VECTOR_REQUIRED_EVENT_TOPIC,
            "outbox_table": ENTERPRISE_SEARCH_VECTOR_RUNTIME_TABLES[0],
            "payload_fields": ("event_id", "occurred_at", "tenant", "pbc", "data"),
        }
        for event_type in ENTERPRISE_SEARCH_VECTOR_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": f"{PBC_KEY}.{event_type}.consumed.v1",
            "topic": f"{ENTERPRISE_SEARCH_VECTOR_REQUIRED_EVENT_TOPIC}.inbox",
            "inbox_table": ENTERPRISE_SEARCH_VECTOR_RUNTIME_TABLES[1],
            "payload_fields": ("event_id", "occurred_at", "tenant", "source_pbc", "data"),
        }
        for event_type in ENTERPRISE_SEARCH_VECTOR_CONSUMED_EVENT_TYPES
    ),
    "retry_policy": {"name": "enterprise_search_vector_default_retry", "max_attempts": 5, "backoff": "exponential"},
    "idempotency": {"key_fields": ("event_type", "event_id", "handler"), "storage": ENTERPRISE_SEARCH_VECTOR_RUNTIME_TABLES[1]},
}
...
def event_contract_manifest() -> dict:
    ...
    return {
        ...
        and EVENT_CONTRACT["topic"] == ENTERPRISE_SEARCH_VECTOR_REQUIRED_EVENT_TOPIC
        and EVENT_CONTRACT["dead_letter_table"] == ENTERPRISE_SEARCH_VECTOR_RUNTIME_TABLES[2]
...
def validate_event_contract() -> dict:
    ...
    required_emitted_fields = {"event_id", "occurred_at", "tenant", "pbc", "data"}
    required_consumed_fields = {"event_id", "occurred_at", "tenant", "source_pbc", "data"}
...
def build_event_envelope(event_type: str, payload: dict | None = None, *, direction: str = "emitted", event_id: str = "smoke-event") -> dict:
    ...
    envelope["tenant"] = supplied.get("tenant", "tenant_smoke")
