"""Idempotent event handlers for the enterprise_search_vector PBC."""

HANDLER_CONTRACTS = ({'event_type': 'ProductPublished', 'function': 'handle_product_published', 'idempotency_key': 'enterprise_search_vector:ProductPublished:{event_id}', 'retry_policy': {'name': 'enterprise_search_vector_default_retry', 'max_attempts': 5, 'backoff': 'exponential'}, 'dead_letter_table': 'enterprise_search_vector_appgen_dead_letter_event', 'side_effect_boundary': 'owned_tables_or_declared_api_calls'}, {'event_type': 'CustomerUpdated', 'function': 'handle_customer_updated', 'idempotency_key': 'enterprise_search_vector:CustomerUpdated:{event_id}', 'retry_policy': {'name': 'enterprise_search_vector_default_retry', 'max_attempts': 5, 'backoff': 'exponential'}, 'dead_letter_table': 'enterprise_search_vector_appgen_dead_letter_event', 'side_effect_boundary': 'owned_tables_or_declared_api_calls'}, {'event_type': 'AuditEventSealed', 'function': 'handle_audit_event_sealed', 'idempotency_key': 'enterprise_search_vector:AuditEventSealed:{event_id}', 'retry_policy': {'name': 'enterprise_search_vector_default_retry', 'max_attempts': 5, 'backoff': 'exponential'}, 'dead_letter_table': 'enterprise_search_vector_appgen_dead_letter_event', 'side_effect_boundary': 'owned_tables_or_declared_api_calls'})
_PROCESSED_KEYS = set()
