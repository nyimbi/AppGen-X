...
HANDLER_CONTRACTS = tuple(
    {
        ...
        "function": f"handle_{''.join(ch.lower() if ch.isalnum() else '_' for ch in event_type).strip('_')}",
        "idempotency_key": f"enterprise_search_vector:{event_type}:{{event_id}}",
...
def handler_manifest() -> dict:
    ...
    return {
        ...
        and all(item["dead_letter_table"] == ENTERPRISE_SEARCH_VECTOR_RUNTIME_TABLES[2] for item in HANDLER_CONTRACTS),
...
def dispatch_event(event: dict) -> dict:
    ...
    return {
        ...
        "inbox_table": handler["inbox_table"],
    }
...
def smoke_test() -> dict:
    ...
    return {
        ...
        and first_result.get("dead_letter_table") == ENTERPRISE_SEARCH_VECTOR_RUNTIME_TABLES[2],
        "manifest": manifest,
