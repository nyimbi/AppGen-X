# Revenue Recognition PBC Specification

## Stable Identity

The `revenue_recognition` pbc is a package-owned business capability in the `finops` mesh. It owns the implementation directory `src/pyAppGen/pbcs/revenue_recognition` and exposes a stable manifest, side-effect-free registration, discovery metadata, release evidence, and package-local tests. The PBC description is: Performance obligations, contract modifications, allocation rules, revenue schedules, compliance evidence, and GL posting events.

## Owned Boundary

The package owns its datastore boundary and does not mutate shared or foreign tables. All owned table names are generated under the `revenue_recognition_` prefix in schema contracts, models, migrations, service operation contracts, event handlers, UI workbench projections, and agent CRUD plans. Cross-PBC collaboration is represented through APIs, AppGen-X events, and read-only projections rather than shared table writes.
...
The PBC contributes first-class agent skills into the composed application single assistant under the `revenue_recognition_skills` namespace. The chatbot helps users accomplish tasks, accepts documents and instructions, proposes governed datastore CRUD mutations, rejects foreign table mutation, requires human confirmation for writes, and emits AppGen-X event plans. Skills are expressible in the DSL through composed assistant tool names.
