...
def _owned_tables_for(route: dict) -> tuple[str, ...]:
    raw_tables = route.get("owned_tables") or ((route["owned_table"],) if route.get("owned_table") else ())
    return tuple(
...
def _route_to_route(route: dict) -> dict:
    ...
    return {
        ...
        "handler": operation,
        "permission": route.get("requires_permission")
        or route.get("permission")
        or personnel_identity_permissions_contract()["action_permissions"].get(operation),
    }
...
ROUTES = tuple(_route_to_route(route) for route in personnel_identity_build_api_contract()["routes"])
API_ROUTE_CONTRACTS = tuple(
    _route_to_contract(route) for route in personnel_identity_build_api_contract()["routes"]
)
