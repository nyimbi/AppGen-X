"""AppGen-X event contract for the composition_engine PBC."""
...
def _schema_name(event_type: str, direction: str) -> str:
    parts = []
    for index, character in enumerate(event_type):
        if index and character.isupper():
            parts.append("_")
        parts.append(character.lower())
    return f"{PBC_KEY}.{''.join(parts)}.{direction}.v1"
    ...
    "emitted": tuple(
        {
            ...
            "schema": _schema_name(event_type, "emitted"),
            "topic": COMPOSITION_ENGINE_REQUIRED_EVENT_TOPIC,
            "outbox_table": COMPOSITION_ENGINE_RUNTIME_TABLES[0],
            "payload_fields": ("event_id", "occurred_at", "pbc", "data"),
        }
        for event_type in COMPOSITION_ENGINE_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": _schema_name(event_type, "consumed"),
            "topic": COMPOSITION_ENGINE_REQUIRED_EVENT_TOPIC,
            "inbox_table": COMPOSITION_ENGINE_RUNTIME_TABLES[1],
            "payload_fields": ("event_id", "occurred_at", "source_pbc", "data"),
        }
        for event_type in COMPOSITION_ENGINE_CONSUMED_EVENT_TYPES
    ),
