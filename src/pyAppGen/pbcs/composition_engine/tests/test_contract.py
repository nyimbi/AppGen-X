"""Contract and surface smoke tests for composition_engine."""

from .. import PBC_MANIFEST
...
def test_generated_schema_service_and_release_evidence():
    ...
    assert SCHEMA_CONTRACT["implementation_artifacts"]["migration_files"] == ("migrations/001_initial.sql",)
    assert {"dsl_artifact", "package_registration_plan", "release_evidence"} <= set(SCHEMA_CONTRACT["owned_tables"])
    assert tuple(table["table"] for table in SCHEMA_CONTRACT["runtime_tables"]) == (
    ...
    assert SERVICE_CONTRACT["pbc"] == "composition_engine"
    assert SERVICE_CONTRACT["ok"] is True
    assert SERVICE_CONTRACT.get("shared_table_access") is False
    assert RELEASE_EVIDENCE["pbc"] == "composition_engine"
    ...
    assert {"schema", "service", "rehearsal", "assistant"} <= set(release_manifest["sections"])
...
def test_manifest_and_event_contract():
    ...
    assert "CompositionControlCenter" in PBC_MANIFEST["ui_fragments"]
    ...
    assert EVENT_CONTRACT["outbox_table"].startswith("composition_engine_")
    assert EVENT_CONTRACT["inbox_table"].startswith("composition_engine_")
...
def test_registration_plan_is_side_effect_free():
    ...
    assert register_pbc()["pbc"] == "composition_engine"
...
def test_service_and_route_surface_are_executable():
    ...
    assert len(route_contracts["contracts"]) >= 20
    assert "create_workspace" in operation_contracts["command_operations"]
    assert {"build_release_evidence", "build_control_center", "assistant_document_preview"} <= set(
        operation_contracts["query_operations"]
    )
...
def test_configuration_permissions_and_seed_hooks_are_executable():
    ...
    reviewer = permissions.role_permissions("composition_reviewer")
    ...
    assert not reviewer["side_effects"]
    assert not seed_smoke["side_effects"]


def test_ui_forms_wizards_controls_and_agent_surface_are_executable():
    from .. import agent, controls, forms, ui, wizards
    ...
    assert ui_smoke["contract"]["forms"]
    assert ui_smoke["contract"]["wizards"]
    assert ui_smoke["contract"]["controls"]
    assert ui_smoke["rendered"]["cards"]
    assert not form_smoke["side_effects"]
    assert not wizard_smoke["side_effects"]
    assert not control_smoke["side_effects"]
...
def test_table_stakes_and_advanced_capability_assurance_is_executable():
    ...
    assert manifest["forms"]["ok"] is True
    assert manifest["wizards"]["ok"] is True
    assert manifest["controls"]["ok"] is True
    assert manifest["agent"]["ok"] is True
