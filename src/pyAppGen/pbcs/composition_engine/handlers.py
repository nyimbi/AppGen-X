...
HANDLER_CONTRACTS = tuple(
    {
        ...
        "function": f"handle_{event_type}",
        "idempotency_key": f"composition_engine:{event_type}:{{event_id}}",
...
def handler_manifest() -> dict:
    ...
    return {
        "ok": bool(HANDLER_CONTRACTS) and len(HANDLER_CONTRACTS) == len(COMPOSITION_ENGINE_CONSUMED_EVENT_TYPES),
...
def smoke_test() -> dict:
    ...
    event = {"event_type": first["event_type"], "event_id": f"smoke-{len(_PROCESSED_KEYS)}", "payload": {"smoke": True}}
    ...
    return {
        ...
        and first_result.get("dead_letter_table") == COMPOSITION_ENGINE_RUNTIME_TABLES[2],
        "manifest": manifest,
