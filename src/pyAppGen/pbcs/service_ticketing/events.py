"""AppGen-X event contract for the service_ticketing PBC."""
...
EVENT_CONTRACT = {
    ...
    "topic": SERVICE_TICKETING_REQUIRED_EVENT_TOPIC,
    "inbox_topic": f"{SERVICE_TICKETING_REQUIRED_EVENT_TOPIC}.inbox",
    ...
    "emitted": tuple(
        {
            ...
            "schema": f"service_ticketing.{event_type}.emitted.v1",
            "topic": SERVICE_TICKETING_REQUIRED_EVENT_TOPIC,
            "outbox_table": SERVICE_TICKETING_RUNTIME_TABLES[0],
            "payload_fields": ("event_id", "occurred_at", "pbc", "tenant", "data"),
        }
        for event_type in SERVICE_TICKETING_EMITTED_EVENT_TYPES
    ),
    "consumed": tuple(
        {
            "event_type": event_type,
            "schema": f"service_ticketing.{event_type}.consumed.v1",
            "topic": f"{SERVICE_TICKETING_REQUIRED_EVENT_TOPIC}.inbox",
            "inbox_table": SERVICE_TICKETING_RUNTIME_TABLES[1],
            "payload_fields": ("event_id", "occurred_at", "source_pbc", "tenant", "data"),
        }
        for event_type in SERVICE_TICKETING_CONSUMED_EVENT_TYPES
    ),
    "retry_policy": {"name": "service_ticketing_default_retry", "max_attempts": 5, "backoff": "exponential"},
    "idempotency": {"key_fields": ("event_type", "event_id", "handler"), "storage": SERVICE_TICKETING_RUNTIME_TABLES[1]},
...
def event_contract_manifest() -> dict:
    ...
    return {
        ...
        and len(EMITTED_EVENTS) == len(SERVICE_TICKETING_EMITTED_EVENT_TYPES)
        and len(CONSUMED_EVENTS) == len(SERVICE_TICKETING_CONSUMED_EVENT_TYPES)
...
def build_event_envelope(event_type: str, payload: dict | None = None, *, direction: str = "emitted", event_id: str = "smoke-event") -> dict:
    ...
    if direction == "emitted":
        envelope["pbc"] = supplied.get("pbc", "service_ticketing")
    else:
