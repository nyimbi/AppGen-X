...
def _operation_name(route: dict) -> str:
    return route.get("command") or route.get("query")


def _owned_tables(route: dict) -> tuple[str, ...]:
    return tuple(
        table if table.startswith("service_ticketing_") else f"service_ticketing_{table}"
...
def _build_operation_contracts() -> tuple[dict, ...]:
    ...
    fallback_emitted = api["events"]["emits"][0]
    contracts = []
    for route in api["routes"]:
        ...
        if is_command and not table_scope and operation == "receive_event":
            table_scope = (SERVICE_TICKETING_RUNTIME_TABLES[1], SERVICE_TICKETING_RUNTIME_TABLES[2])
        contracts.append(
            {
                "operation": operation,
...
def service_operation_contracts() -> dict:
    ...
    runtime_service = service_ticketing_build_service_contract()
    return {
...
def operation_plan(operation_name: str, payload: dict | None = None) -> dict:
    ...
    table_scope = contract["owned_tables"] or contract["read_tables"] or tuple(
        f"service_ticketing_{table}" for table in service_ticketing_build_service_contract()["runtime_tables"]
    )
    return {
...
def service_operation_manifest() -> dict:
    ...
    return {
        "ok": contracts["ok"],
        "pbc": "service_ticketing",
        "service_class": ServiceTicketingService.__name__,
