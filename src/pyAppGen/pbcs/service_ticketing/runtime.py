...
# Improve1 service ticketing control extension.
from .service_ticketing_control import evaluate_service_ticketing_control, improve1_service_ticketing_control_contract

_SERVICE_CONTROL_BASE_RUNTIME_CAPABILITIES = service_ticketing_runtime_capabilities
_SERVICE_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = service_ticketing_build_release_evidence


def service_ticketing_runtime_capabilities() -> dict:
    runtime = dict(_SERVICE_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_service_ticketing_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["service_ticketing_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_service_ticketing_control", "improve1_service_ticketing_control_contract")))
...
def service_ticketing_build_release_evidence() -> dict:
    evidence = dict(_SERVICE_CONTROL_BASE_BUILD_RELEASE_EVIDENCE())
    control = improve1_service_ticketing_control_contract()
    artifacts = dict(evidence.get("generated_artifacts", {}))
    artifacts["service_ticketing_control"] = {
    ...
    checks = tuple(evidence.get("checks", ())) + ({"id": "improve1_service_ticketing_control", "ok": control["ok"]},)
    evidence.update({
        "ok": bool(evidence.get("ok")) and control["ok"],
        "checks": checks,
        "generated_artifacts": artifacts,
        "service_ticketing_control": control,
