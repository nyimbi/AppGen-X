"""API route contracts for the travel_management PBC."""
PBC_KEY = 'travel_management'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /travel-requests', 'POST /travel-bookings', 'POST /itineraries', 'POST /policy-checks', 'GET /travel-workbench'))
