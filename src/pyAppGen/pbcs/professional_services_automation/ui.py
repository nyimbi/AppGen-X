...
# Improve1 PSA control UI extension.
from .psa_control import improve1_psa_control_contract as _improve1_psa_control_contract

_PSA_CONTROL_BASE_UI_CONTRACT = professional_services_automation_ui_contract
_PSA_CONTROL_BASE_RENDER_WORKBENCH = professional_services_automation_render_workbench


def professional_services_automation_ui_contract() -> dict:
    ui = dict(_PSA_CONTROL_BASE_UI_CONTRACT())
    control = _improve1_psa_control_contract()
    ui.update({
        "ok": ui.get("ok") is True and control["ok"],
        "psa_control_contract": control,
        "psa_control_panels": tuple(item["evidence"]["ui_surface"] for item in control["capabilities"]),
        "psa_control_service_actions": tuple(item["evidence"]["service_api"] for item in control["capabilities"]),
        "stream_engine_picker_visible": False,
    })
    return ui


def professional_services_automation_render_workbench(*args, **kwargs) -> dict:
    workbench = dict(_PSA_CONTROL_BASE_RENDER_WORKBENCH(*args, **kwargs))
    control = _improve1_psa_control_contract()
    workbench.update({
        "ok": workbench.get("ok") is True and control["ok"],
        "psa_control_panels": tuple(item["evidence"]["ui_surface"] for item in control["capabilities"]),
        "psa_control_service_actions": tuple(item["evidence"]["service_api"] for item in control["capabilities"]),
        "psa_control_agent_tools": tuple(f"professional_services_automation.skills.{item['slug']}" for item in control["capabilities"]),
    })
    return workbench
