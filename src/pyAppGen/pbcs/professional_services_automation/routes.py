"""API route contracts for the professional_services_automation PBC."""
PBC_KEY = 'professional_services_automation'
ROUTES = tuple({'method': api.split()[0], 'path': api.split(maxsplit=1)[1], 'operation': api.lower().replace(' ', '_').replace('/', '_'), 'idempotency_key': f'{PBC_KEY}:{api}'} for api in ('POST /engagements', 'POST /statements-of-work', 'POST /staffing', 'POST /billing-milestones', 'GET /services-automation-workbench'))
