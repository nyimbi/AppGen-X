...
# Improve1 PSA control extension.
from .psa_control import evaluate_psa_control, improve1_psa_control_contract

_PSA_CONTROL_BASE_RUNTIME_CAPABILITIES = professional_services_automation_runtime_capabilities
_PSA_CONTROL_BASE_BUILD_RELEASE_EVIDENCE = professional_services_automation_build_release_evidence


def professional_services_automation_runtime_capabilities():
    runtime = dict(_PSA_CONTROL_BASE_RUNTIME_CAPABILITIES())
    control = improve1_psa_control_contract()
    runtime["ok"] = bool(runtime.get("ok")) and control["ok"]
    runtime["psa_control"] = control
    runtime["operations"] = tuple(dict.fromkeys(tuple(runtime.get("operations", ())) + ("evaluate_psa_control", "improve1_psa_control_contract")))
