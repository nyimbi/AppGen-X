...
def map_oracle_datatypes(oracle_type):
    ...
    if oracle_type.startswith('NUMBER') or oracle_type.startswith('BINARY_FLOAT') or oracle_type.startswith('BINARY_DOUBLE'):
        return 'Numeric'
    elif oracle_type.startswith('VARCHAR2') or oracle_type.startswith('NVARCHAR2') or oracle_type.startswith('CHAR') or oracle_type.startswith('NCHAR') or oracle_type.startswith('CLOB') or oracle_type.startswith('NCLOB'):
        return 'String'
    elif oracle_type.startswith('DATE') or oracle_type.startswith('TIMESTAMP') or oracle_type.startswith('TIMESTAMP WITH TIME ZONE') or oracle_type.startswith('TIMESTAMP WITH LOCAL TIME ZONE'):
        return 'DateTime'
...
def map_sqlite_datatypes(sqlite_type):
    ...
    """
    if sqlite_type.startswith('integer') or sqlite_type.startswith('tinyint') or sqlite_type.startswith('smallint') or sqlite_type.startswith('mediumint') or sqlite_type.startswith('int') or sqlite_type.startswith('bigint'):
        return 'Integer'
    elif sqlite_type.startswith('real') or sqlite_type.startswith('float') or sqlite_type.startswith('double') or sqlite_type.startswith('decimal'):
        return 'Numeric'
    elif sqlite_type.startswith('char') or sqlite_type.startswith('varchar') or sqlite_type.startswith('text') or sqlite_type.startswith('clob'):
        return 'String'
    ...
    def insert_data(conn, tables):
        ...
        for table in tables:
            ...
            for relationship in relationships:
                ...
                s = "SELECT id FROM appColumns WHERE table_id={} AND name='{}'".format(table_id,relationship['col1'].name )
                print('parent_column_id',  s)
                parent_column_id = cursor.execute(s).fetchone()
                print('parent_column_id:', parent_column_id)

                s = "SELECT id FROM appColumns WHERE table_id={} AND name='{}'".format(child_table_id,relationship['col2'].name )
                print(s)
