...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Expanded LSP code-action CLI coverage to the full required quick-fix family. `appgen.lsp-code-action-apply-audit.v1` now includes `create_event_contract`, and `appgen.lsp-code-action-cli-audit.v1` exercises missing table/field/calculated field, operation/flow, lookup/relationship, typo, secret, invalid picker, event contract, PBC manifest, agent permission, package, and smoke-test fixes through `appgen lsp --apply-code-action`. | Pending focused verification and commit. |
