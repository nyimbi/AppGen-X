...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced semantic drift blocking gap counts in text output. `appgen drift` now prints `blocking_gaps=...`, so CI and agent logs expose the same gate posture as `appgen.semantic-drift-audit.v1` without parsing JSON. | Pending focused verification. |
