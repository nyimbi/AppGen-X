...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened diagnostics text renderer evidence. `appgen.diagnostics-text-renderer.v1` now reports summary-line, required-code-line, covered-fixture-line, covered-code-line, missing-code-line, and blocking-gap-line counts so diagnostic text logs prove catalog coverage, fixture coverage, missing diagnostic coverage, and blocking fixture gaps. | Pending verification: py_compile, focused diagnostics text renderer pytest, direct diagnostics text renderer probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
