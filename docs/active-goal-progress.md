...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added PBC publish text renderer evidence without entering active PBC implementation paths. `appgen.pbc-publish-text-renderer.v1` proves publish logs expose catalog target mode, side-effect-free and write-performed flags, plus per-check publication status without parsing JSON. | Pending focused verification. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
