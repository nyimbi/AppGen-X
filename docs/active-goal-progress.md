...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Added top-level DSL language-quality gating to the tooling audit. `appgen.tooling-audit.v1` now embeds `appgen.dsl-language-quality.v1`, and phase 0 exit criteria require grammar/parser sync, required enterprise grammar rules, keyword budget, authoring aliases, and progressive learning-path evidence before tooling completion can pass. | Pending verification: py_compile, focused tooling implementation-phase and tooling-audit pytest, direct language-quality/tooling-audit probes, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
