...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Tightened the test-strategy CLI audit to require generator drift evidence. `appgen.test-strategy-cli-audit.v1` now fails unless `appgen drift` reports CLI, LSP, Studio, graph, generator, and release-verifier surfaces and includes `appgen.generate-report.v1` as generator evidence. | Pending focused verification and commit. |
