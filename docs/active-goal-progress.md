...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened Studio semantic-service payload-depth evidence. `appgen.studio-semantic-service-audit.v1` now proves panels carry component entries, form views, database tables, workflows, PBC composition entries, package/deployment counts, diagnostics, graph kinds, and natural-language patch content instead of passing on empty format envelopes. | Pending focused verification. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
