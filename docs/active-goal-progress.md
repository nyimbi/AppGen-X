...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Complete text renderer count evidence across the remaining non-PBC renderer contracts. Parser-golden, component-publish, graph/explain, migration-plan, release-verifier, LSP service, LSP code-action, and tooling-audit text renderer contracts now report required-fragment, missing-fragment, output-line, and marker-line counts. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
