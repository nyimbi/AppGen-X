...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen CLI audit breadth counts across non-PBC tooling contracts. Lint directory, validate/generate, graph, graph-suite, migration, package/verify, LSP code-action, and LSP rename CLI audits now report case/scenario, passing, target/backend/action/manifest/handoff, and coverage counts so release evidence proves breadth instead of only boolean success. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
