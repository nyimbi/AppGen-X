...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen documentation contract-format integrity. `appgen.tooling-doc-anchor-audit.v1` now checks every `appgen.*.v1` contract named in `docs/tooling.md` against runtime and focused test references, not only heading anchors. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
