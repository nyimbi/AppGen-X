...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added view-section depth to LSP document symbols. The semantic symbol table and LSP outline now include `view_section` children under views alongside component bindings and handlers, making the docs/tooling.md hierarchical outline requirement executable for form sections, dropped components, and event wiring. | Pending focused document-symbol pytest, direct outline probe, Python compile, diff check, and restricted legacy-name scan excluding active PBC paths. |
