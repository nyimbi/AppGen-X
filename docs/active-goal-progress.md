...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened release verifier text renderer evidence. `appgen.release-verifier-text-renderer.v1` now reports release-line, graph-line, target-status-line, passing/failing target-line, blocking-gap-line, and artifact-line counts so package handoff logs prove target status, graph-suite context, gaps, and written artifact paths remain visible. | Pending verification: py_compile, focused release verifier text renderer pytest, direct release text renderer audit probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
