...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen parser-golden construct coverage text evidence. `appgen.parser-golden-text-renderer.v1` now proves parser-golden text logs include covered construct names as `covered-constructs ...`, not only coverage counts and missing construct names. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
