...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen LSP code-action edit text evidence. `appgen.lsp-code-action-text-renderer.v1` now proves quick-fix text logs include applied DSL edit text as `edit ...`, not only edit counts, alongside title, lint, available-action, failure, and diagnostic markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
