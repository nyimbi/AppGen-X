...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen natural-language planner token-budget text evidence. `appgen.nl-plan-cli-audit.v1` now proves accepted `appgen nl-plan` text output includes individual `token-budget-note ...` markers, not only the note count. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
