...
| --- | --- | --- | --- |
| 2026-05-25 | `current` | Made the event-processing guidance expose an executable developer use card that tells platform users exactly what to use, what to omit, what developers write, what AppGen-X generates, what Studio exposes, what Studio hides, and when to stop branching. | Py compile, focused PBC catalog policy test, scoped diff check, and scoped restricted-name scan passed. |
