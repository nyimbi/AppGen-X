...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Expanded package CLI audit coverage to the full `--target all` handoff family. `appgen.package-verify-cli-audit.v1` now verifies `appgen verify --target all` and `appgen package --target all --out ...` produce release evidence and manifests for web, mobile, desktop, PBC, and deployment targets with target-specific handoff metadata. | Pending focused verification and commit. |
