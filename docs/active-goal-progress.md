...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Split language-server transport and navigation/completion behavior into first-class tooling gates. `appgen.tooling-audit.v1` now includes `lsp_transport_rpc_contracts` and `lsp_navigation_completion_contracts`, and phase 4 requires JSON-RPC/stdio transport evidence plus completion, symbol, navigation, and LSP text renderer evidence. | Pending verification: py_compile, focused implementation-phase pytest, focused LSP RPC/stdio/text pytest, top-level tooling-audit pytest, direct tooling-audit probe for the LSP transport and navigation gates, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
