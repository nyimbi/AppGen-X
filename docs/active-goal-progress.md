...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened formatter write text coverage. `appgen format --write` now has executable text-mode proof that the write summary includes organize state, write request state, write completion state, and write path for an actual mutation. | Pending focused verification. |
