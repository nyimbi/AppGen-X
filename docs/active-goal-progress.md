...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced component publication patch evidence in text output. `appgen component-publish` now prints the catalog patch format and before/after catalog counts so non-PBC component publication logs prove the side-effect-free catalog update plan without JSON parsing. | Pending focused verification. |
