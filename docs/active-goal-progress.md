...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen graph-suite rendering coverage evidence. `appgen.graph-suite-cli-audit.v1` now reports `rendering_formats_by_kind` and `missing_renderings`, proving every required graph kind has JSON, Mermaid, and DOT renderings instead of only reporting a rendering kind count. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
