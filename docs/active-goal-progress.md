...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Aligned lint catalog path failures with the CLI missing-input contract. `appgen lint --catalog missing.json` now fails as a configuration error with exit code 2, while malformed existing catalogs continue to exercise traceback-free internal error handling. | Pending focused verification. |
