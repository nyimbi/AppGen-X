...
| --- | --- | --- | --- |
| 2026-05-24 | `current` | Tightened the source form-designer release audit so its parity workbench gate now requires explicit passing lifecycle, requirement-audit, subsystem, and artifact evidence instead of relying on the aggregate workbench boolean alone. | Py compile, focused generated form-designer parity smoke, scoped diff check, and scoped restricted-name scan passed. |
