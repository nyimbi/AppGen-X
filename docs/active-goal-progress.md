...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Add focused doctor text renderer release evidence. `appgen.doctor-text-renderer.v1` now proves doctor text logs include check status, blocking-gap counts, and embedded `detail_format=...` audit markers without invoking the full doctor CLI or traversing active PBC implementation paths. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
