...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen text renderer count evidence across multiple tooling contracts. The diagnostics, lint, semantic drift, doctor, validate/generate, format, and designer-sync text renderer contracts now report required-fragment, missing-fragment, output-line, and marker-line counts, with doctor preserving explicit check-line and detail-format-line counts. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
