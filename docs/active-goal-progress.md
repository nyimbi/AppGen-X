...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened release-verifier failure text coverage. `appgen verify --target mobile` now has executable CLI text proof that failed target verification still prints release evidence, graph-suite evidence, and concrete blocking gaps such as missing package metadata and smoke launch declarations. | Pending focused verification. |
