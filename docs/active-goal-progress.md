...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added catalog-backed LSP definition navigation. `textDocument/definition` now resolves registered PBC keys and API/event contract tokens to read-only `catalog://pbc/...` locations, completing the catalog navigation path alongside hover and workspace-symbol search. | Pending focused definition pytest, direct definition probes, Python compile, diff check, and restricted legacy-name scan excluding active PBC paths. |
