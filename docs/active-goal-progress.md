...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced CLI alias readiness in doctor checks. `appgen.doctor-report.v1` now includes a `cli_alias_contract` check backed by `appgen.cli-alias-contract.v1`, so environment/readiness audits prove the `appgen`/`apg` command contract. | Pending focused verification. |
