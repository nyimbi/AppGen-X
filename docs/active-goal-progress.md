...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen LSP code-action parity evidence. `appgen.lsp-code-action-apply-audit.v1` and `appgen.lsp-code-action-cli-audit.v1` now expose required, observed, and missing action-id sets, and the top-level tooling check fails if the agent-facing CLI quick-fix surface drifts from the in-process patch contract. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
