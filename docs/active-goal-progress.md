...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened lint text renderer evidence. `appgen.lint-text-renderer.v1` now reports source-file-line, stage-line, migration-line, migration-preview-line, migration-detected-line, diagnostic-line, error-line, and warning-line counts so lint text logs prove source coverage, stage coverage, migration preview evidence, and diagnostic severity visibility. | Pending verification: py_compile, focused lint text renderer pytest, direct lint text renderer probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
