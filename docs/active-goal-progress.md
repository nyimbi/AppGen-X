...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Enriched validate and generate default text summaries. `appgen validate` now reports requested targets, app-declared targets, semantic-model format, checks, and diagnostics; `appgen generate` now reports resolved targets, artifact count, semantic-model format, output directory, manifest path, artifact paths, blocking gaps, and diagnostics. | Pending focused verification. |
