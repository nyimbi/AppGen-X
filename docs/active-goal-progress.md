...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen diagnostics fixture text evidence. `appgen.diagnostics-text-renderer.v1` now proves `appgen diagnostics --audit-fixtures` human logs include explicit `covered-code ...` markers as well as missing-code and blocking-gap markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
