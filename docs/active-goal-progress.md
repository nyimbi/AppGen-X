...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened package and verify text coverage. `appgen package` now has executable proof for concrete release-evidence and target-manifest artifact paths, while `appgen verify` proves graph-suite evidence without emitting artifact lines when no output directory is used. | Pending focused verification. |
