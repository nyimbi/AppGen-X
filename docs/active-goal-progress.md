...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened natural-language planner text coverage. `appgen nl-plan` now has executable text-mode proof for intent, operation count, patch size, generated test count, token-budget note count, operation kinds, lint status, and migration preview status tied to the JSON plan payload. | Pending focused verification. |
