...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added LSP hover depth for PBC metadata and diagnostic explanations. Hover now exposes registered PBC catalog metadata through `appgen.lsp-pbc-hover.v1` and appends structured diagnostic explanation objects for symbols with lint diagnostics, matching the docs/tooling.md hover requirements. | Pending focused hover pytest, direct hover probes, Python compile, diff check, and restricted legacy-name scan excluding active PBC paths. |
