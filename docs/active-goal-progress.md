...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen missing-required-option CLI evidence. `appgen.missing-required-option-exit-audit.v1` now proves `appgen explain` rejects bare invocations that omit the required selector family, in addition to generator output directories, natural-language prompts, and component publication names. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
