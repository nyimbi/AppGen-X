...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen designer-sync DSL diff text evidence. `appgen.designer-sync-text-renderer.v1` now proves designer-sync text logs include DSL diff preview lines as `dsl-diff ...`, not only diff line counts, alongside round-trip, matrix, required-operation, and executed-case markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
