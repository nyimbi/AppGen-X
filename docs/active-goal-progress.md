...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen release verifier graph-suite text evidence. `appgen.release-verifier-text-renderer.v1` now proves verify/package text logs include release graph-suite kinds as `graph-kinds ...` and rendering formats as `graph-formats ...`, not only graph-suite counts. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
