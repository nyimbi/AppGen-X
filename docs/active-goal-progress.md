...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen component publish catalog context text evidence. `appgen.component-publish-text-renderer.v1` now proves human component-publish logs include existing catalog component names as `catalog-existing ...`, not only catalog counts. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
