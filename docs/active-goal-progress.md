...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen Studio semantic service surface-count evidence. `appgen.studio-semantic-service-audit.v1` now reports service, required/observed/missing service-format, surface, required-surface, surface-format, semantic-surface-format, and panel counts so Studio bridge release evidence proves breadth without expanding nested payloads. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
