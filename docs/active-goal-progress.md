...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen LSP completion coverage text evidence. `appgen.lsp-service-text-renderer.v1` now proves human LSP service logs include explicit `completion-missing ...` markers for missing completion coverage sources alongside source-of-truth, navigation, formatting, hover, and rename safety markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
