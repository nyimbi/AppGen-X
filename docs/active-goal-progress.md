...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Split PBC completion coverage into explicit contract families. The LSP completion coverage now proves PBC catalog keys, aggregate contracts, API contracts, event contracts, and command-style contracts separately so the implementation matches the docs/tooling.md completion promise for APIs, events, and commands. | Pending focused completion coverage pytest, doctor completion probe, Python compile, diff check, and restricted legacy-name scan excluding active PBC paths. |
