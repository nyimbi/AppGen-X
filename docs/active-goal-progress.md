...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened VS Code extension executable coverage. `appgen.vscode-extension-audit.v1` now proves command activation events, command palette membership, and the `appgen.command` CLI configuration property in addition to language metadata, providers, CLI command contracts, and webview renderers. | Pending focused verification. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
