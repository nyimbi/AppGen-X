...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Split graph rendering and explain behavior into first-class tooling gates. `appgen.tooling-audit.v1` now includes `graph_rendering_contracts` and `explain_cli_contracts`, and phase 3 requires graph kind/rendering breadth plus explain text/JSON format coverage and graph/explain text renderer evidence. | Pending verification: py_compile, focused implementation-phase pytest, focused graph CLI, graph-suite, and explain CLI pytest, top-level tooling-audit pytest, direct tooling-audit probe for the graph rendering and explain gates, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
