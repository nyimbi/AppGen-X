...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Split migration safety and text evidence into a first-class tooling gate. `appgen.tooling-audit.v1` now includes `migration_safety_text_contracts`, and phase 6 requires migration detection breadth, supported backend CLI evidence, approval posture, safe alternatives, and migration text renderer safety markers. | Pending verification: py_compile, focused implementation-phase pytest, focused migration CLI and migration text renderer pytest, top-level tooling-audit pytest, direct tooling-audit probe for the migration safety gate, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
