...
| --- | --- | --- | --- |
| 2026-05-28 | `30ce97a` | Expanded the AppGen DSL into a compact typed enterprise language. The grammar now covers API, event, job, report, menu, reusable component, package, and test contracts; view and contract handlers; keyword-valued dotted metadata; and richer rule expressions while preserving the 32-keyword budget. Parser/schema conversion now exposes typed enterprise contract collections and validates handler/contract/package targets. | Py compile passed for DSL/schema/generator/tests; focused DSL platform surface and DSL grammar/linter/release regressions passed (`6 passed`); `git diff --check` passed for touched DSL/docs/generated-parser paths; scoped restricted legacy-name diff scan returned clean. |
