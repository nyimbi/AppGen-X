...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen missing-input CLI failure evidence. `appgen.missing-input-exit-audit.v1` now proves missing file, semantic-baseline, and component-catalog path failures emit no stdout payload, so usage/configuration failures cannot be mistaken for partial tooling reports. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
