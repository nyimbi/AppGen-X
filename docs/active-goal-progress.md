...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Expanded the LSP code-action CLI audit beyond missing-operation and lookup fixes. `appgen lsp --apply-code-action` now has executable CLI evidence for secret-literal replacement and invalid runtime/stream/backend picker removal, proving safety quick fixes used by IDEs and agents. | Pending focused code-action CLI pytest, direct CLI helper probe, Python compile, diff check, and restricted legacy-name scan excluding active PBC paths. |
