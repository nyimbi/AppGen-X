...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen explain CLI mode-count evidence. `appgen.explain-cli-audit.v1` now reports total case count, text-mode count, JSON-mode count, and report-format-covered count so release evidence proves all symbol, diagnostic, and handler explain modes remain covered. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
