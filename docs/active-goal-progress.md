...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added generator evidence to the semantic drift gate. `appgen.semantic-drift-audit.v1` now runs `appgen.generate-report.v1` and verifies its validation semantic-model digest matches the canonical semantic model used by CLI, LSP, Studio, graph, release-verifier, and test surfaces. | Pending focused verification and commit. |
