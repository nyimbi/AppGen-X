...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen Studio semantic service contract evidence. `appgen.studio-semantic-service-audit.v1` now reports required, observed, and missing service contract formats so the documented `appgen.studio-semantic-service.v1` bridge is directly proven alongside its LSP, designer-sync, graph-suite, and natural-language planner dependencies. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
