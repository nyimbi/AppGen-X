...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced designer edit operation coverage in text output. `appgen designer-sync` now prints the required visual edit operation names from `appgen.designer-visual-edit-matrix.v1`, so text-mode release logs show which designer lanes are covered. | Pending focused verification. |
