...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened non-goal policy audit coverage. Natural-language plans now explicitly reject attempts to skip linting, semantic validation, or release evidence before any DSL patch is produced, extending `appgen.non-goal-policy-audit.v1` beyond generated-code bypass prompts. | Pending focused verification. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
