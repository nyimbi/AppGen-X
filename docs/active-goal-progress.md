...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen migration coverage text evidence. `appgen.migration-plan-text-renderer.v1` now proves human migration logs include explicit `migration-missing ...` markers for missing coverage families alongside detected-family, destructive-change, safe-alternative, and diagnostic markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
