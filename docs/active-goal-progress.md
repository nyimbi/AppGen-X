...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen LSP hover text evidence. `appgen.lsp-service-text-renderer.v1` now proves editor-service text logs include hover content as `hover ...`, not only hover item counts, alongside completion coverage, navigation, formatting, and rename safety markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
