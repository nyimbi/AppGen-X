...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen lint source-file text evidence. `appgen.lint-text-renderer.v1` now proves lint text logs include source file names as `source-file ...`, not only source mode and file counts, alongside ordered stage counts, migration-preview metadata, and diagnostics. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
