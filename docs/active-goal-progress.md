...
| --- | --- | --- | --- |
| 2026-05-25 | `current` | Strengthened the event-processing guidance so developers get an explicit opinionated stack, choice budget, workload-default table, and rule that ordinary generated apps use the AppGen-X event contract while runtime/broker details remain platform-owned. | Pending verification. |
