...
| --- | --- | --- | --- |
| 2026-05-30 | PENDING | Split natural-language planner evidence into first-class operation and CLI/agent handoff gates. `appgen.tooling-audit.v1` now includes `natural_language_operation_contracts` and `natural_language_cli_agent_contracts`, and phase 6 requires operation coverage, rejection diagnostics, token-budget evidence, text handoff markers, and zero blocking cases instead of relying only on the broad planner aggregate. | Pending verification. |
