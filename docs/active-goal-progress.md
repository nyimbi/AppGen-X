...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added human-readable planning command summaries. `appgen migration-plan` now prints backend, change counts, destructive/approval state, detected migration families, individual change kinds, and diagnostics by default; `appgen nl-plan` now prints intent, operation count, patch size, generated test count, operation kinds, lint status, and migration-preview state while preserving both JSON contracts. | Pending focused verification. |
