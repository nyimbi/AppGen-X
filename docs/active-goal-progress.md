...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen natural-language planner test-plan text evidence. `appgen.nl-plan-cli-audit.v1` now proves accepted `appgen nl-plan` text output includes generated verification steps as `test-plan ...`, not only test counts, alongside lint, migration-preview, and token-budget markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
