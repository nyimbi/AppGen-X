...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen PBC publish catalog text evidence without entering PBC implementation paths. `appgen.pbc-publish-text-renderer.v1` now proves publish text logs include target catalog paths as `catalog_path ...` and catalog patch entries as `catalog-patch ...`, not only mode and side-effect flags. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
