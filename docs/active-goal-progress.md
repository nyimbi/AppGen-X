...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Aligned lint text rendering with published report lanes. `appgen lint` text output now renders stage counts from `stage_names` instead of duplicating lane order in the CLI renderer, keeping human output synchronized with `appgen.lint-report.v1`. | Pending focused verification. |
