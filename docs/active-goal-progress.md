...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added catalog-backed LSP reference resolution. `textDocument/references` now returns read-only `catalog://pbc/...` index locations for registered PBC keys and API/event contract tokens alongside ordinary workspace occurrences, extending the catalog navigation path from hover, symbols, and definitions into reference search. | Pending focused verification and commit. |
