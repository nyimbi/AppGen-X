...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened semantic drift text renderer evidence. `appgen.semantic-drift-text-renderer.v1` now reports summary-line, surface-line, gap-line, evidence-line, check-line, passing/failing check-line, and digest-line counts so shared-model drift logs prove surface coverage, evidence references, named gaps, check outcomes, and digest identity. | Pending verification: py_compile, focused semantic drift text renderer pytest, direct semantic drift text renderer probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
