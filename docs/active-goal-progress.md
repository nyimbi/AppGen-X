...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Expanded missing-input CLI evidence across non-PBC file commands. `appgen.missing-input-exit-audit.v1` now proves missing path errors return exit code `2` without tracebacks for lint, previous semantic baselines, format, validate, graph, graph-suite, explain, generate, migration-plan, nl-plan, lsp, verify, package, designer-sync, and drift. | Pending focused verification. |
