...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen tooling-audit text policy markers. `appgen.tooling-audit-text-renderer.v1` now proves human release logs include the explicitly documented `appgen.non-goal-policy-audit.v1` and `appgen.tooling-doc-anchor-audit.v1` format markers without running the full tooling audit. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
