...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added catalog-backed LSP workspace symbol search. `workspace/symbol` now returns registered PBC catalog entries and API/event contract symbols with `catalog://pbc/...` locations when queries match catalog names, labels, mesh metadata, descriptions, or contract names, proving the docs/tooling.md catalog metadata requirement. | Pending focused workspace-symbol pytest, JSON-RPC audit pytest, direct workspace-symbol probes, Python compile, diff check, and restricted legacy-name scan excluding active PBC paths. |
