...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced unsafe rename state in LSP text output. `appgen lsp` now prints blocked status, blocker count, and migration approval requirement when a rename is requested, and `appgen.lsp-rename-cli-audit.v1` verifies the blocked text contract. | Pending focused verification. |
