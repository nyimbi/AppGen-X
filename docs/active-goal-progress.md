...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Expanded LSP code-action CLI proof for package and smoke-test quick fixes. `appgen.lsp-code-action-cli-audit.v1` now verifies `add_package_for_app_target` and `create_smoke_test_declaration` through `appgen lsp --apply-code-action`, aligning the agent-facing CLI evidence with the required code-action list in `docs/tooling.md`. | Pending focused verification and commit. |
