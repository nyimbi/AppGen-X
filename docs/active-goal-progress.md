...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added release-verifier text renderer evidence. `appgen.release-verifier-text-renderer.v1` proves human-readable verify/package handoff logs expose the release verifier envelope, evidence bundle, graph-suite marker, per-target status, blocking gaps, and artifact paths without invoking PBC package verification. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused release-verifier text renderer pytest; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
