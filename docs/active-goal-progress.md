...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened CLI usage-failure audit evidence. Missing-input, missing-required-option, and invalid-choice audits now report case ids, failing case ids/counts, stdout-empty counters, and command-family coverage for missing-input paths so release triage identifies the exact broken CLI surface. | Pending verification: py_compile, focused CLI usage-failure pytest, direct usage-failure audit probes, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
