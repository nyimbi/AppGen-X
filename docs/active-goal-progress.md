...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Split package manifest handoff and release text evidence into first-class tooling gates. `appgen.tooling-audit.v1` now includes `package_manifest_handoff_contracts` and `release_text_evidence_contracts`, and phase 6 requires package manifest breadth plus release text marker evidence instead of relying only on the broader package/release verifier aggregate. | Pending verification: py_compile, focused implementation-phase pytest, focused package CLI/text pytest, top-level tooling-audit pytest, direct tooling-audit probe for the package manifest and release text gates, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
