...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened VS Code language metadata audit coverage. The extension audit now records and gates the documented `.appgen`, `.ag`, and `.ags` file extensions plus `onLanguage:appgen` activation metadata, so editor file association support is part of the machine-readable tooling release gate. | Pending focused verification. |
