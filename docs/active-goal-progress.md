...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen explain CLI detail evidence. `appgen.explain-cli-audit.v1` now reports resolved symbol id/kind/parent/reference count, diagnostic title/docs URL, and handler match/edge evidence so explain release evidence carries the navigational details promised by `docs/tooling.md`. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
