...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added default text summaries for LSP CLI evidence. `appgen lsp <file>` now reports semantic-model format, diagnostics, completions, code actions, document/workspace symbols, source-of-truth, hover evidence, and rename status; `appgen lsp --apply-code-action <id>` now reports action id, change status, edit count, lint status, title, available actions, and diagnostics without requiring JSON mode. | Pending focused verification. |
