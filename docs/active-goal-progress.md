...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthen semantic model and symbol coverage contract counts. The shared semantic model now reports required/present/missing top-level field counts, symbol counts, symbol-kind counts, diagnostic counts, and missing top-level fields; symbol coverage now reports required/detected/missing kind counts. | Pending commit. Verification passed with `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for semantic-model and symbol-coverage contract tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
