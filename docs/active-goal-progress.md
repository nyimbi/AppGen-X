...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen formatter organize evidence. `appgen.format-write-audit.v1` now reports `organize_table_body_order` with the named table-body categories produced by `--organize`, making formatter release evidence directly match the documented identity, business key, relationship, editable, calculated, audit, directive ordering. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
