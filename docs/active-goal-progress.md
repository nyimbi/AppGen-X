...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced migration safe alternatives in text output. `appgen migration-plan` now prints safe alternatives for changes that carry them, so destructive migration release logs expose the same mitigation guidance as `appgen.migration-plan.v1`. | Pending focused verification. |
