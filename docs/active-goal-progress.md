...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Promoted frontend Studio bridge audits to first-class tooling gates. `appgen.tooling-audit.v1` now includes explicit `frontend_semantic_service_bridge` and `frontend_interaction_audit_bridge` checks, and phase 5 requires browser-smoke semantic-service and interaction-audit bridge evidence instead of relying only on nested Studio details. | Pending verification: py_compile, focused Studio semantic-service audit pytest, implementation-phase pytest, direct tooling-audit probe for the two frontend checks, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
