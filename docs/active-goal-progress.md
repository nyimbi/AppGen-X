...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen diagnostic catalog registry text evidence. `appgen.diagnostics-text-renderer.v1` now proves diagnostic catalog text logs include required registry codes as `required-code ...` and covered fixture codes as `covered-fixture-code ...`, not only counts and fixture-audit details. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
