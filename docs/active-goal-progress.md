...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen CLI help surface-count evidence. `appgen.cli-help-surface-audit.v1` now reports required/documented/listed subcommand counts plus option-surface, required-option, and missing-option counts so CLI discoverability evidence can be compared without scraping nested help payloads. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
