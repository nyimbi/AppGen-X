...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen validate/generate artifact handoff evidence. `appgen.validate-generate-cli-audit.v1` now proves successful generation returns resolved targets, output directory, semantic-model format, validation-report format, manifest existence, manifest app name, artifact count, and artifact path existence. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
