...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced validation target mismatch details in text output. `appgen validate` now prints missing and unknown target names for `target_compatibility` failures, so text-mode CI and agent logs expose the same target mismatch detail as `appgen.validate-report.v1`. | Pending focused verification. |
