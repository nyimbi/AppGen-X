...
| --- | --- | --- | --- |
| 2026-06-01 | `this commit` | Tightened multi-file semantic source attribution for nested enterprise symbols. Directory source sets no longer fall back to loose token matches for parent-owned declarations; table fields, view sections, component bindings, handlers, flow states, agent skills, permissions, and deployment units must resolve inside their owning parent block. The semantic source-set audit now proves nested ownership for `table.Invoice.total`, `view.InvoiceForm.Main`, `view.InvoiceForm.customer.name`, `view.InvoiceForm.Save`, `flow.SubmitInvoice.draft`, `agent.InvoiceAssistant.read`, `agent.InvoiceAssistant.Invoice:read`, `agent.InvoiceAssistant.Explain`, and `deploy.Production.SubmitInvoice`. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct semantic source-set probe returned `True () () ()` and expected nested owner paths for field/view/flow/agent/deploy symbols; focused semantic source-set pytest passed 1 test in 4.42s; direct contract-schema audit returned `True 0 0 ()`; direct aggregate tooling-audit probe returned `appgen.tooling-audit.v1 True 61 61 []` and source-set detail `True 9 0 0 0`; direct docs/schema parity probe returned `required_count 2100`, `doc_format_count 2100`, `missing_required_in_docs_count 0`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive, active PBC paths, PBC tests, and `docs/pbc_writing_guide.md` returned no matches. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Hardened multi-file semantic source sets from CRUD-only proof to enterprise workspace proof. `appgen.semantic-source-set-cli-audit.v1` now builds a nine-file workspace and verifies app, table, view, flow, rule, LLM provider, agent, deployment, deployment-unit, and package declarations are discovered across files with zero missing symbols. Source-file attribution now handles nested deployment units strictly inside their owning `deploy` block, preventing a repeated operation name in another file from stealing ownership. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused semantic source-set plus full docs/tooling surface pytest passed 2 tests in 12:46 with existing PBC datetime/dependency warnings; direct semantic source-set audit returned `True () () ()` for ok, missing symbol-file ids, files without symbols, and missing text markers; direct contract-schema audit returned `True 0 0 ()`; direct docs/schema parity probe returned `required_count 2100`, `doc_format_count 2100`, `missing_required_in_docs_count 0`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive, active PBC paths, PBC tests, and `docs/pbc_writing_guide.md` returned no matches. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Propagated runnable generation-job semantics into generated applications. Generated Studio modules now emit queued runnable `appgen.generation-job.v1` manifests with stage, lifecycle, quality-gate, required-artifact, evidence-format, command, and blocking-gap counters; generated `appgen.generation-job-queue.v1` payloads now report queue health, runnable/blocked job counts, required command coverage, and release-gate consumable stop conditions. Package-level smoke evidence now verifies that generated apps inherit the executable generation queue contract. | `python3 -m py_compile src/pyAppGen/gen.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py tests/test_main.py`; focused Studio generation/package pytest passed 2 tests in 38.50s with existing PBC datetime/dependency warnings; direct contract-schema audit returned `True 0 0 ()`; direct aggregate tooling-audit probe returned `appgen.tooling-audit.v1 True 61 61 []` and generation-run detail `True 5 1266 4 3`; direct docs/schema parity probe returned `required_count 2100`, `doc_format_count 2100`, `missing_required_in_docs_count 0`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py src/pyAppGen/studio.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py tests/test_main.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive, active PBC paths, PBC tests, and `docs/pbc_writing_guide.md` returned no matches. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Added an executable Studio generation runner behind the lifecycle queue. `run_generation_job` now validates DSL, generates application artifacts, emits release verifier evidence, writes release-evidence/package manifests, and feeds those counts back into `studio_generation_queue_lifecycle`; the gate now proves not only that a queue item is runnable, but that the platform can execute validation, source generation, and packaging evidence production end to end. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py`; focused Studio generation pytest passed 1 test in 13.57s with existing PBC datetime warnings; direct contract-schema audit returned `True 0 0 ()`; direct aggregate tooling-audit probe returned `appgen.tooling-audit.v1 True 61 61 []` and Studio generation-run detail `True 5 1266 4 3`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted Studio generation jobs from planned records to executable lifecycle contracts. `appgen.package-generation-job.v1` now exposes runnable queue state, lifecycle stages through generated/verified/packaged, required commands, quality gates, required artifacts, evidence formats, counters, and blocking gaps; `appgen.package-generation-queue.v1` aggregates runnable/blocked job, command, status, stage, artifact, evidence, and blocking-gap counters. The aggregate tooling audit now has 61 required gates including `studio_generation_queue_lifecycle`, which fails on planned-only generation records or missing run/open/rerun commands. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py`; focused Studio generation/semantic pytest passed 2 tests in 8.09s; direct Studio generation queue audit returned `appgen.studio-generation-smoke-audit.v1 True ()`, schema validation `True True`, and counts `5 4 5 5`; direct contract-schema audit returned `ok True`, `sample_validation_failing_count 0`, `missing_required_schema_count 0`; direct aggregate tooling-audit probe returned `appgen.tooling-audit.v1 True 61 61 []`; direct docs/schema parity probe returned `required_count 2100`, `doc_format_count 2100`, `missing_required_in_docs_count 0`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive, active PBC paths, PBC tests, and `docs/pbc_writing_guide.md` returned no matches. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Hardened the Studio data-service workbench from lane/existence checks to implementation-ready catalog evidence. `appgen.frontend-data-service-catalog-audit.v1` now schema-validates named data-service capabilities, required source/query/publish/embedded database/resilience/security lanes, missing capability/lane/audit-field counters, and zero weak generation terms. The frontend catalog no longer describes failover output as a stub; it now names a failover coordinator, health probe, promotion workflow, and operational runbook package, and the browser-smoke script asserts that rendered generated-artifact text. The aggregate tooling audit now has 60 required gates including `frontend_data_service_catalog_depth`. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py`; `npm run build` passed in `appgen-frontend`; focused Studio and implementation-phase pytest passed 2 tests in 3.14s; focused contract-schema pytest passed 1 test in 99.52s with existing PBC/dependency warnings; direct contract-schema audit returned `ok True`, `sample_validation_failing_count 0`, `missing_required_schema_count 0`; direct aggregate tooling-audit probe returned `ok True passed 60 required 60 bad []`; direct LSP transport aggregate detail probe returned `prepare_rename_response_count 1`; direct docs/schema parity probe returned `required_count 2100`, `doc_format_count 2100`, `missing_required_in_docs_count 0`; `git diff --check -- appgen-frontend/src/dataServiceCatalog.ts appgen-frontend/scripts/browser-smoke.mjs docs/tooling.md src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive, active PBC paths, PBC tests, and `docs/pbc_writing_guide.md` returned no matches. `npm run test:browser` rebuilt successfully but Chrome exited before DOM dump in this host, so runtime browser smoke remains environment-limited here. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Made LSP prepare-rename a first-class IDE contract and removed the schema-validation performance trap that blocked full tooling evidence. The language server now advertises `renameProvider.prepareProvider`, handles `textDocument/prepareRename`, returns exact code-identifier ranges with placeholder, symbol metadata, lexical scope, and occurrence counts, rejects comments/string literals/unknown symbols before workspace edits are offered, exports `appgen.lsp-prepare-rename.v1`, covers JSON-RPC and stdio transport, and documents the editor lifecycle. Contract schema generation is now cached so sample validation does not rebuild the full 2,099-format catalog for every payload. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP prepare-rename/lifecycle pytest passed 5 tests in 7.27s; direct contract-schema audit returned `ok True`, `sample_validation_failing_count 0`, `missing_required_schema_count 0`; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`; direct doc-anchor integrity probe returned `ok True` and `missing_sections ()`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive, active PBC paths, PBC tests, and `docs/pbc_writing_guide.md` returned no matches. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Implemented the IDE language-server save/close lifecycle as real JSON-RPC behavior instead of an open/change-only session. The LSP now advertises structured text sync with open/close, full-document changes, and save text; `textDocument/didSave` refreshes diagnostics from the saved buffer, `textDocument/didClose` evicts the buffer and publishes empty diagnostics, and the JSON-RPC/stdio audits now gate 13 method contracts plus a 16-step editor workflow covering save diagnostics and close diagnostic clearing. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP lifecycle pytest passed 3 tests; direct LSP JSON-RPC audit returned `ok True methods 13 13 workflow 16 16 missing () ()`; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`; direct docs/schema parity probe returned `2098 2098 () ()`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Hardened Studio browser-smoke interaction count evidence so `appgen.studio-browser-smoke-ci-contract.v1` requires the browser smoke script to assert rendered interaction-count text derived from `appgen.frontend-interaction-audit.v1` required scenario count. The stale `9 checks` smoke expectation is now `10 checks`, matching the ten required interaction scenarios exposed by the frontend audit and status rail. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py`; focused Studio semantic-service pytest passed 1 test; `npm run build` passed in `appgen-frontend`; direct browser-smoke CI probe returned `ok True`, expected `10 checks`, present `True`, and `browser_smoke_interaction_count_text: True`; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`; direct docs/schema parity probe returned `2098 2098 [] []`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; `git diff --check -- appgen-frontend/scripts/browser-smoke.mjs docs/tooling.md src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Hardened the browser-facing Studio semantic-service contract so `appgen.frontend-semantic-service-audit.v1` is emitted by the frontend with the same named required/observed/missing fields that the Python tooling audit reports for services, surfaces, and surface contracts. The Studio/Monaco proof now fails if the browser-side audit function omits service lists, required service lists, missing service lists, surface lists, required surface lists, missing surface lists, surface contract lists, required surface contract lists, missing surface contract lists, or their missing-count fields. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py`; focused Studio semantic-service pytest passed 1 test; `npm run build` passed in `appgen-frontend`; direct Studio audit probe returned `ok True`, `frontend audit fields missing ()`, and missing counts `0 0 0 0`; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`; direct docs/schema parity probe returned `2098 2098 [] []`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; `git diff --check -- appgen-frontend/src/semanticServiceContract.ts docs/tooling.md src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Hardened VS Code extension readiness from view-existence checks to named activity-bar action coverage. `appgen.vscode-extension-audit.v1` now proves the Workspace, Reports, and Agent Handoffs views expose 12 concrete tree actions for validation, designer sync, graph preview, generation, packaging, doctor, tooling audit, release verification, migration planning, natural-language planning, semantic preview, and PBC catalog browsing. This closes a proof-depth gap where an empty or hollow activity bar could satisfy the previous audit. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused VS Code extension pytest passed 1 test; direct VS Code audit probe returned `ok True` with `tree 12 12 ()`; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`; direct docs/schema parity probe returned `2098 2098 [] []`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the PBC-family runtime contract backlog into the schema-backed tooling surface without touching active PBC implementation files. `appgen contract-schema` now exports and sample-validates 44 additional contracts for generated PBC runtime manifests, package discovery and metadata validation, registration plans, service/schema contracts, implementation and release audits, catalog/topology reports, natural-language selection, source packages, source artifact evidence, lifecycle documentation, runtime test coverage, and table-stakes evidence. The documented contract surface rises to 2,098 formats, and runtime inventory is now closed except for the intentional unsupported-schema sentinel. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `2098 2098 [] []`; direct runtime inventory probe returned `runtime 2099 schema 2098 unpromoted 1 ('appgen.missing-contract.v1',)`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; direct sample-validation probe returned `total 2098 bad 0`; focused contract-schema pytest passed 2 tests in 4:40 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the remaining non-PBC runtime contract backlog into the schema-backed tooling surface while leaving active PBC contracts and `appgen.missing-contract.v1` untouched. `appgen contract-schema` now exports and sample-validates 315 additional contracts for mobile offline/background flows, monitoring, native packaging, natural-language evolution, package management, platform release evidence, operations workbenches, reports, resilience, schema tooling, security, source intake, UI customization, visual runtime verification, wizarding, and database/search deployment plans. The documented contract surface rises to 2,054 formats and the runtime inventory backlog is now limited to PBC-family contracts plus the unsupported-schema sentinel. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `2054 2054 [] []`; direct sample-validation probe returned `total 2054 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; direct runtime inventory probe returned `runtime 2099 schema 2054 unpromoted 45`; focused contract-schema pytest passed 2 tests in 4:05 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the mobile/native/IDE runtime family into the schema-backed tooling surface while preserving `appgen.missing-contract.v1` as the unsupported-schema sentinel. `appgen contract-schema` now exports and sample-validates 259 additional contracts for generated mobile device APIs, native bridge errors, permission workflows, simulator replays, privacy/store manifests, generated native form and runtime operation manifests, generated language/runtime workbench contracts, platform parity, property editors, visual runtime assets, wizard manifests, generation job queues, handler cross-calls, HTTPS/i18n/identity gates, IDE capability matrices, object-inspector workflows, integration workbenches, capability-superiority evidence, livebinding operations, local database maintenance, manufacturing/media gates, menus, microservice packaging, migration plans, and mobile background delivery. The documented contract surface rises to 1,739 formats while active PBC paths remain untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `1739 1739 [] []`; direct sample-validation probe returned `total 1739 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; focused contract-schema pytest passed 2 tests in 3:14 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the generated visual/data/mobile runtime family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 220 additional contracts for generated component wiring manifests, cross-target scene authoring, animation timelines, visual effects, shader/material editing, custom designers, generated data connection/service manifests, dataset and relationship lookup replay, data-tooling operations, design-time package management, device API replay matrices, form-stream round-trips, handler source round-trips, generated object-inspector workflows, generated binding workbenches, local database maintenance, and mobile lifecycle/deep-link delivery. The documented contract surface rises to 1,480 formats while active PBC paths remain untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `1480 1480 [] []`; direct sample-validation probe returned `total 1480 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; focused contract-schema pytest passed 2 tests in 2:54 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the generated frontend/component runtime family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 120 additional contracts for frontend parity and quality gates, generated app shell chrome, generated binding authoring/runtime/replay evidence, generated component behavior and design surfaces, drag/drop handler wiring, editor-family manifests, event dispatch, package lifecycle and dependency evidence, palette registration, parity runtime manifests, render/runtime/serialization contracts, target adapters, and component usability workbenches. The documented contract surface rises to 1,260 formats while active PBC paths remain untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `1260 1260 [] []`; direct sample-validation probe returned `total 1260 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; focused contract-schema pytest passed 2 tests in 2:37 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the design/data runtime family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 240 additional contracts for component templates, composition packages, context menus, coverage gates, cross-target 3D scene editing, animation timelines, visual effects, style resources, shader/material editing, custom designers, data connection failover, dataset design, lookup editors, offline replay, query previews, relationship navigation, replication, published resources, schema adapters, service contracts, SQL safety, stored procedure workflows, database-backed form column guards, deployment scaling, desktop sync snapshots, device API components, diagnostics, documentation, ERP coverage, event-processing defaults, finance gates, and form-designer evidence. The documented contract surface rises to 1,140 formats while active PBC paths remain untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `1140 1140 [] []`; direct sample-validation probe returned `total 1140 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; focused contract-schema pytest passed 2 tests in 2:20 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the application/component runtime family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 140 additional contracts for accessibility and agent workbenches, application lifecycle plans, assistant/chatbot/CI/collaboration/compliance evidence, component drag/drop, event binding and dispatch, component family replay, component module manifests, and component package installation, dependency, lifecycle, palette, preview, rollback, sandbox, signature, uninstall, update, render, runtime, and serialization evidence. The documented contract surface rises to 900 formats while active PBC paths remain untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `900 900 [] []`; direct sample-validation probe returned `total 900 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; focused contract-schema pytest passed 2 tests in 2:05 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the generated runtime/module manifest family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 120 additional contracts for visual runtime generated-test smoke evidence, component-family and package-module smoke evidence, generated file/test manifests, replay matrices, designer and extension modules, seed, backup, data access, schema import, database operations, data exchange, integration, productivity, lifecycle, emerging, voice, text-quality, notification, agentic, and view-composition module runtime evidence. The documented contract surface rises to 760 formats while keeping active PBC paths untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `760 760 [] []`; direct sample-validation probe returned `total 760 bad 0`; direct doc-anchor integrity probe returned `ok True runtime_gap 0 test_gap 0`; focused contract-schema pytest passed 2 tests in 2:00 with existing PBC datetime/dependency warnings; direct aggregate tooling-audit probe returned `ok True passed 59 required 59 bad []`. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-06-01 | `this commit` | Promoted the native Pascal/runtime workbench family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 45 contracts for unit generation, compile pipelines, semantic validation, incremental compilation and invalidation, diagnostic mapping, package dependency and target matrices, runtime type metadata, event binding and handler wiring, resource streaming/hash/fidelity, form stream schemas, debug symbols, debug session/watch replay, compile package transactions, runtime memory/toolchain/session/design-edit replay, actionable operations, authoring scenario replay, runtime readiness, runtime module replay, and the aggregate runtime workbench. The documented contract surface rises to 303 formats while keeping active PBC paths untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `303 303 [] []`; direct sample-validation probe returned `total 303 bad 0`; direct aggregate audit probe returned `ok True passed 59 required 59 bad []`; focused contract-schema pytest passed 2 tests in 1:49 with existing PBC datetime/dependency warnings; aggregate tooling-audit pytest passed 1 test in 11:15 with existing PBC datetime/dependency warnings; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the visual binding designer runtime family into the schema-backed tooling surface in one batch. `appgen contract-schema` now exports and sample-validates 34 binding contracts covering expression validation, authoring sessions, conflict validation/resolution, graph validation/editing/JSON round-trip, staged transactions, preview/runtime parity, lookup and master-detail wiring, scope contexts, bulk edits, diagnostics, scheduling, dependency execution, expression sandboxing/editor replay, runtime failure recovery, cursor sync, offline replay, accessibility routes, propagation, design-runtime replay, designer transaction replay, lifecycle release replay, and designer family coverage. The documented contract surface rises to 258 formats while active PBC paths remain untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `258 258 [] []`; direct sample-validation probe returned `total 258 bad 0`; direct aggregate audit probe returned `ok True passed 59 required 59 bad []`; focused contract-schema pytest passed 2 tests in 1:53 with existing PBC datetime/dependency warnings; aggregate tooling-audit pytest passed 1 test in 11:29 with existing PBC datetime/dependency warnings. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the top-level package support family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 29 contracts for base-feature readiness, config editor catalog/update/production evidence, distribution templates, package artifacts, seed fixtures, DSL-quality artifacts/linting, ERP starter/template contracts, ideas document/release evidence, generated-app excellence, goal audits, and roadmap release gates. The documented contract surface rises to 224 formats while keeping active PBC paths untouched and using lightweight representative samples for heavyweight generated-app smoke envelopes. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `224 224 [] []`; direct sample-validation probe returned `total 224 bad 0`; focused contract-schema pytest passed 2 tests in 2:21 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 13:20 with existing PBC datetime warnings; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py docs/active-goal-progress.md`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the ACP composition and compact generation family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 8 contracts for opinionated ACP event-processing policy, developer guidance, stream profile selection, event-choice resolution, application-composition topology, ACP capability coverage, compact generation briefs, and full-app small-model natural-language generation gates. The documented contract surface rises to 195 formats. Also hardened DSL rule parsing across generated parser context variants after the existing natural-language package test exposed `RuleItemContext.IDENT()` arity drift. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct docs/schema parity probe returned `195 195 [] []`; direct sample-validation probe returned `total 195 bad 0`; focused contract-schema pytest passed 2 tests in 1:39 with existing PBC datetime warnings; focused natural-language package pytest passed 1 test in 20.28s with existing PBC datetime warnings after the parser fix; aggregate tooling-audit pytest passed 1 test in 12:30 with existing PBC datetime warnings; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; restricted legacy-name scan excluding archive and active PBC paths. Focused PBC catalog package pytest was not used as slice evidence because current active PBC work makes `pbc_implementation_contracts` fail, and PBC files remain off-limits. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the agentic development-vector family into the schema-backed tooling surface. `appgen contract-schema` now exports and sample-validates 8 contracts for package agentic DSL declarations, provider readiness, tool policies, execution matrices, Claude Code/OpenAI Codex/OpenCode backend workflows, release gates, and generated app agentic smoke evidence. The documented contract surface rises to 187 formats while keeping active PBC paths untouched. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused contract-schema pytest passed 2 tests in 1:55 with existing PBC datetime warnings; direct docs/schema parity probe returned `187 187 [] []`; direct contract-schema audit probe returned `ok=True` with zero sample validation failures or missing samples; focused agentic package audit pytest passed 1 test in 15.82s with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 10:36 with existing PBC datetime warnings; `git diff --check -- docs/tooling.md src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the top-level package release-audit family into the schema-backed tooling surface and fixed the generated mobile device runtime evidence path that the live form-designer release audit exposed. `appgen contract-schema` now exports and sample-validates 15 package release audit envelopes for DSL quality, Studio, form designer, visual modeling, security, source intake, config editor, distribution, reporting, operations, integrations, agentic systems, targets, natural-language evolution, and ERP templates. The documented schema surface rises to 179 formats while keeping active PBC paths untouched and avoiding heavyweight package verifier execution inside the schema audit. Generated device component operation steps now emit `check_permission`, and generated mobile replay aggregation tolerates optional phase lists without crashing. | `python3 -m py_compile src/pyAppGen/gen.py src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused contract-schema pytest passed 2 tests in 1:36 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 11:13 with existing PBC datetime warnings before the generated-runtime crash fix; direct docs/schema parity probe returned `179 179 [] []`; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths; direct `form_designer_generation_smoke_audit()` probe returned `appgen.form-designer-generation-smoke-audit.v1 True approved 0` and `generated_mobile_device_runtime` passed all required checks after adding `check_permission`. Full package form-designer pytest was started but did not return promptly after the direct smoke passed; full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Added a runtime contract inventory tooling surface. `appgen runtime-contracts` now emits `appgen.runtime-contract-inventory.v1`, the CLI audit emits `appgen.runtime-contract-inventory-cli-audit.v1`, and the aggregate tooling audit gates JSON/text inventory evidence for top-level package runtime formats while explicitly skipping active PBC paths. The schema/docs sample surface is now 164 formats with the package runtime contract backlog visible for batch promotion. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/__main__.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; focused runtime/schema/help pytest passed 4 tests in 1:43 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 10:55 with existing PBC datetime warnings; direct docs/schema parity probe returned `164 164 [] []`; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted Studio bridge and formatter write runtime payloads into schema-backed contracts. `appgen contract-schema` now validates formatter write/check/organize audit evidence, Studio diagnostics/quick-fixes, graph/explain, natural-language evolution, and browser-smoke CI contracts from live runtime payloads, raising the documented contract-sample surface to 162 formats. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused schema/Studio/formatter pytest passed 4 tests in 1:33 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 11:54 with existing PBC datetime warnings; direct docs/schema parity probe returned `162 162 [] []`; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the DSL authoring and language-service family into schema-backed contracts. `appgen contract-schema` now validates quick-fix application, formatting, outline/navigation, code actions, authoring score, ergonomics, experience, authoring release gate, and combined language-service payloads from live authoring outputs, raising the documented contract-sample surface to 157 formats. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused DSL authoring/schema pytest passed 3 tests in 1:29 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 9:48 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted individual Studio/designer projections into schema-backed handoff contracts. `appgen contract-schema` now validates the DSL editor, component palette, form, database, workflow, PBC composition, package/deployment, graph/explain, NL planner, designer-sync CLI audit, and single visual-edit result formats from live designer-sync output, raising the documented contract-sample surface to 148 formats. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused designer/schema pytest passed 5 tests in 1:24 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 9:49 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the full graph payload family into schema-backed contracts. `appgen contract-schema` now validates `appgen.graph-report.v1` plus ER, lookup, workflow, handler, PBC, security, agent, deployment, and package graph payload schemas from live graph-suite output, raising the documented contract-sample surface to 137 formats. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused graph/schema pytest passed 5 tests in 1:25 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 9:30 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Exported the package/component/PBC audit envelope schemas as reusable contracts. `appgen contract-schema` now validates representative payloads for release evidence files, invalid package target audits, component publish CLI audits, PBC package load reports, and PBC publish CLI audits, raising the documented contract-sample surface to 127 formats. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused schema/package/PBC pytest passed 4 tests in 1:36 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 9:38 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted the PBC catalog list into the schema-backed contract surface. `appgen contract-schema` now includes `appgen.pbc-verifier-catalog.v1`, validates a live PBC catalog sample, and `docs/tooling.md` documents the JSON contract agents should use to discover selectable composable capabilities without scraping text or hard-coding built-ins. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused schema/PBC CLI pytest passed 3 tests in 1:38 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 11:56 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. Push attempted and rejected because local `main` is behind `origin/main`. |
| 2026-05-31 | `this commit` | Exported target-specific verifier schemas. `appgen contract-schema` now includes `appgen.web-verifier.v1`, `appgen.mobile-verifier.v1`, `appgen.desktop-verifier.v1`, `appgen.pbc-verifier.v1`, and `appgen.deployment-verifier.v1`, and the schema audit validates live all-target release verifier samples instead of only the aggregate release report. | `python3 -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused release/schema pytest passed 3 tests in 1:38 with existing PBC datetime warnings after fixing the sample to use all target reports; aggregate tooling-audit pytest passed 1 test in 12:40 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted governance evidence into direct CLI commands. `appgen module-boundaries`, `appgen non-goals`, `appgen implementation-phases`, and `appgen tooling-docs` now expose architecture, policy, phase, docs-anchor, and section-coverage release contracts without requiring contributors or agents to expand the full aggregate tooling audit. `appgen tooling-docs` adds `appgen.tooling-docs-audit.v1` to the contract schema catalog. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/__main__.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; focused governance/help/schema pytest passed 4 tests in 1:24 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 9:55 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Promoted roadmap and test-strategy evidence into first-class CLI commands. `appgen test-strategy <file>` now emits `appgen.test-strategy-cli-audit.v1` directly, `appgen contributor-tasks` exposes the executable contributor task breakdown, and `appgen priority-order` exposes the evidence-backed implementation roadmap for agents and humans without expanding the full tooling audit. | `python3 -m py_compile src/pyAppGen/dsl.py src/pyAppGen/__main__.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; focused CLI/help/schema pytest passed 5 tests in 2:00 with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 9:28 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Expanded contract-schema coverage to the main tooling report surface. `appgen contract-schema` now covers 22 required machine contracts, including formatter, validate/generate, graph-suite, explain, LSP, designer-sync, diagnostic catalog/fixture, parser-golden, semantic-drift, migration, NL plan, release verifier, component publish, doctor, tooling audit, schema catalog, and validation report envelopes. `appgen.contract-schema-cli-audit.v1` now validates representative live payloads for every required schema format instead of only listing schemas. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct sample-validation probe returned `bad 0 total 22`; focused contract-schema/validation pytest passed 3 tests in 25.29s with existing PBC datetime warnings; focused schema CLI tests passed 2 tests in 30.77s after adding explicit `appgen.diagnostic.v1` test coverage; aggregate tooling-audit pytest passed 1 test in 7:40 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive, `.git`, and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Made contract tooling self-describing. `appgen contract-schema` now includes `appgen.contract-schema-catalog.v1` and `appgen.contract-validation-report.v1` schemas, and `appgen.contract-validation-cli-audit.v1` proves the validator can validate its own validation report in addition to semantic payloads and controlled failure cases. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; focused contract schema/validation/phase pytest passed 4 tests in 5.28s; aggregate tooling-audit pytest passed 1 test in 5:56 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive, `.git`, and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `this commit` | Enforced machine contract payloads from the CLI. `appgen contract-validate` now validates JSON tooling payloads against the exported contract-schema catalog, infers schema formats from payloads, emits `appgen.contract-validation-report.v1`, rejects missing required fields, unknown schemas, and malformed JSON, and `contract_validation_cli_contracts` gates JSON/text modes in the aggregate tooling audit. Diagnostic schemas now include the existing `hint` severity emitted by semantic diagnostics. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; focused contract-validation/schema pytest passed 3 tests in 1.59s; focused implementation-phase pytest passed 1 test; aggregate tooling-audit pytest passed 1 test in 5:30 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive, `.git`, and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `d3a1a89b` | Exposed contract schemas as CLI tooling. `appgen contract-schema` now publishes `appgen.contract-schema-catalog.v1` JSON Schema 2020-12 contracts for diagnostics, lint reports, semantic models, migration plans, natural-language plans, release verifier reports, and tooling audits; `appgen.contract-schema-cli-audit.v1` gates catalog JSON, selected-schema JSON, missing-schema JSON, and text modes. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; direct tooling-audit probe returned `True 56 56`; focused contract-schema, phase, and aggregate tooling-audit pytest passed 4 tests in 5:19 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive, `.git`, and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `f5f5eebe` | Accepted semantic baselines for migration planning. `appgen migration-plan` now accepts raw DSL or serialized `appgen.semantic-model.v1` inputs, emits explicit migration-input text evidence, and `appgen.migration-semantic-input-cli-audit.v1` gates source path preservation, semantic input counts, rename/additive detection, coverage format, and non-JSON text output. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused migration semantic-baseline/text/CLI pytest passed 4 tests in 1.34s after one stale expectation fix; focused semantic audit/text pytest passed 2 tests in 3.77s; aggregate tooling-audit pytest passed 1 test in 5:26 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding archive, `.git`, and active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `daf874e1` | Gated graph/explain text by name. `appgen.graph-explain-text-renderer.v1` now publishes required and emitted graph kinds, graph formats, graph check ids, symbol ids, diagnostic codes, diagnostic docs URLs, and handler edges; `explain_cli_contracts` fails when named graph/explain release text evidence disappears instead of only changing fragment counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused graph/explain text-renderer pytest passed 1 test in 2.90s; aggregate tooling-audit pytest passed 1 test in 4:54 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `34d773c4` | Gated validate/generate text by name. `appgen.validate-generate-text-renderer.v1` now publishes required and emitted requested targets, app targets, generated targets, validation check ids, passing/failing check ids, unknown/missing target ids, artifact paths, manifest paths, blocking-gap ids, diagnostic codes, and diagnostic severities; `validate_target_contracts` and `generate_artifact_policy_contracts` fail when named release text evidence disappears instead of only changing marker counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused validate/generate text-renderer pytest passed 1 test in 2.57s; aggregate tooling-audit pytest passed 1 test in 4:23 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `b33e021b` | Gated formatter text markers by name. `appgen.format-text-renderer.v1` now publishes required and emitted write paths, write-requested values, written values, organize values, idempotence states, diagnostic codes, and diagnostic severities; `formatter_write_organize_contracts` fails when named formatter mutation or diagnostic text evidence disappears instead of only changing marker counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused formatter text-renderer pytest passed 1 test in 2.46s; aggregate tooling-audit pytest passed 1 test in 4:18 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `db2a8bbe` | Gated component publish text by name. `appgen.component-publish-text-renderer.v1` now publishes required and emitted catalog sources, side-effect values, write-performed values, patch formats, and existing catalog component names; `component_publish_catalog_contracts` fails when named component publication text evidence disappears instead of only changing marker counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused component publish text-renderer pytest passed 1 test in 2.57s; aggregate tooling-audit pytest passed 1 test in 4:17 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `08a6e431` | Gated semantic drift text by name. `appgen.semantic-drift-text-renderer.v1` now publishes required and emitted surface names, blocking-gap ids, evidence keys, check ids, passing-check ids, and failing-check ids; `semantic_drift_surface_contracts` fails when named shared-model drift release evidence disappears instead of only changing line counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused semantic drift text-renderer pytest passed 1 test in 2.57s; aggregate tooling-audit pytest passed 1 test in 4:26 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `94a14359` | Gated lint text markers by name. `appgen.lint-text-renderer.v1` now publishes required and emitted source files, stage names, migration families, diagnostic codes, and diagnostic severities; `lint_directory_and_strict_profiles` fails when named lint release-log evidence disappears instead of only changing marker counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused lint text-renderer pytest passed 1 test in 2.33s; aggregate tooling-audit pytest passed 1 test in 4:18 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `8a61da5e` | Gated diagnostic text codes by name. `appgen.diagnostics-text-renderer.v1` now publishes required and emitted registry codes, covered fixture codes, fixture-audit covered codes, fixture-audit missing codes, and blocking-gap ids; `diagnostic_catalog_fixture_contracts` fails when named diagnostic codes or fixture gaps disappear from release text logs. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused diagnostics text-renderer pytest passed 1 test in 2.42s; aggregate tooling-audit pytest passed 1 test in 4:12 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `00677f3c` | Gated roadmap evidence by name. Implementation phases now publish required/observed/missing phase ids and required/observed/missing exit criteria by phase; contributor task contracts publish required groups and required task names; priority-order contracts publish required priority ids. The aggregate gates fail on deleted or renamed roadmap, contributor, or priority obligations instead of count-only drift. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase pytest passed 1 test in 2.49s; aggregate tooling-audit pytest passed 1 test in 4:28 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-31 | `786d2440` | Gated doctor named checks. `appgen.doctor-report.v1` and `appgen.doctor-text-renderer.v1` now publish required, observed/emitted, and missing doctor check ids plus required and emitted detail formats by check; `doctor_cli_text_contracts` fails on named readiness-check or embedded audit-marker gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused doctor report/text renderer pytest passed 2 tests in 5.49s; aggregate tooling-audit pytest passed 1 test in 4:30 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `745d2255` | Gated package invalid-target cases. `appgen.package-invalid-target-audit.v1` now publishes required, observed, and missing invalid-target case ids, invalid-choice-message cases, traceback-free cases, expected exit codes by case, and missing expected-exit cases; `package_and_release_verifiers` fails on named `package` or `verify` unsupported-target failure-path gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused package invalid-target audit pytest passed 1 test in 2.33s; aggregate tooling-audit pytest passed 1 test in 4:21 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `7175b437` | Gated designer-sync CLI scenarios. `appgen.designer-sync-cli-audit.v1` now publishes required, observed, missing, and failing scenario ids, required projection ids, missing projection ids, required changed surfaces, required diff fragments, invalid-case ids, and traceback-free case ids; `ide_visual_designer_round_trip` fails on named designer-sync scenario, projection, diff, changed-surface, or invalid-payload rejection gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused designer-sync CLI audit pytest passed 1 test in 5.16s; aggregate tooling-audit pytest passed 1 test in 5:08 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `ebf2ac73` | Gated in-process quick-fix apply cases. `appgen.lsp-code-action-apply-audit.v1` now publishes required, observed, and missing quick-fix case ids, expected text by case, expected-text matched cases, applied-edit cases, changed cases, cleanup cases, and lint-passing cases; `lsp_quick_fix_coverage_contracts` fails on named library-side quick-fix application gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused quick-fix apply audit pytest passed 1 test in 3.02s; aggregate tooling-audit pytest passed 1 test in 4:26 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `96ee2c25` | Gated LSP quick-fix CLI cases. `appgen.lsp-code-action-cli-audit.v1` now publishes required, observed, and missing quick-fix case ids, expected text by case, forbidden cleanup text by case, applied-edit cases, changed cases, lint-format cases, and lint-passing cases; `lsp_quick_fix_cli_contracts` fails on named quick-fix patch, cleanup, lint, or changed-status handoff gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused quick-fix CLI audit pytest passed 1 test in 3.60s; aggregate tooling-audit pytest passed 1 test in 4:12 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `8bf1b145` | Gated natural-language planner CLI cases. `appgen.nl-plan-cli-audit.v1` now publishes required, observed, and missing accepted JSON case ids, expected operation kinds by case, expected payload formats by case, and per-case lint, migration, test-plan, and token-budget evidence; `natural_language_cli_agent_contracts` fails on named natural-language edit operation or agent handoff evidence gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused NL planner CLI audit pytest passed 1 test in 6.12s; aggregate tooling-audit pytest passed 1 test in 4:20 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `5f1adb21` | Gated migration CLI backend cases. `appgen.migration-cli-audit.v1` now publishes required, observed, and missing backend case ids, expected versus observed backend names by case, required versus missing change kinds by case, expected versus observed payload formats by case, approval-required case ids, and rename-hint counts by case; `migration_safety_text_contracts` fails on named backend, migration-family, approval, payload, or rename-hint evidence gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused migration CLI audit pytest passed 1 test in 2.67s; aggregate tooling-audit pytest passed 1 test in 4:22 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `a0ee54a5` | Gated explain CLI case contracts. `appgen.explain-cli-audit.v1` now publishes required, observed, and missing explain CLI case ids, expected versus observed text/JSON output modes by case, required report-format cases, required text markers, and required navigation-detail cases; `explain_cli_contracts` fails on missing named symbol, diagnostic, or handler explain examples, markers, modes, or navigation payloads. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused explain CLI audit pytest passed 1 test in 2.97s; aggregate tooling-audit pytest passed 1 test in 4:21 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `71fd638a` | Gated graph CLI case formats. `appgen.graph-cli-format-audit.v1` now publishes required, observed, and missing graph CLI case ids, expected versus observed output formats by case, expected versus observed JSON payload formats by case, and required versus missing Mermaid/DOT text markers by case; `graph_rendering_contracts` fails on missing named graph examples or format/payload/text-marker drift. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused graph CLI audit pytest passed 1 test in 3.15s; aggregate tooling-audit pytest passed 1 test in 4:15 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `00bf1a02` | Gated validate/generate case formats. `appgen.validate-generate-cli-audit.v1` now publishes required, observed, and missing validation/generation case ids plus expected versus observed payload formats by case; `validate_target_contracts` and `generate_artifact_policy_contracts` fail on named scenario or report-envelope drift. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused validate/generate CLI audit pytest passed 1 test in 10.12s with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 4:25 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `9ddc7bb7` | Gated test-strategy CLI formats. `appgen.test-strategy-cli-audit.v1` now publishes required, observed, and missing case ids plus expected versus observed payload formats by case; `parser_golden_and_drift_gates` fails on missing diagnostics, parser-golden, drift, or doctor cases and format drift. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused test-strategy CLI audit pytest passed 1 test in 11.57s with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 4:25 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `4565f4e0` | Gated CLI failure-path evidence. Missing-input, missing-required-option, and invalid-choice audits now publish required, observed, and missing case ids, missing command-family names, and missing expected-message cases; `cli_usage_failure_contracts` fails on any named failure-path evidence gap. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused CLI failure-path pytest passed 3 tests in 2.43s; aggregate tooling-audit pytest passed 1 test in 4:21 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `76c5a87c` | Gated component publish evidence markers. `appgen.component-publish-cli-audit.v1` now publishes required, observed, and missing JSON/text/failure-path case ids, required and missing text markers, and required versus observed missing-catalog blocking gaps; `component_publish_catalog_contracts` fails on missing named component publication evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused component publish CLI audit pytest passed 1 test in 2.16s; aggregate tooling-audit pytest passed 1 test in 4:22 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `f0fc8103` | Gated DSL language CLI markers. `appgen.dsl-language-cli-audit.v1` now publishes required, observed, and missing JSON/text command case ids, expected versus observed payload formats, and required versus missing text markers; `dsl_language_cli_contracts` fails on missing command modes, payload-format drift, or lost human log markers. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused DSL language CLI audit pytest passed 1 test in 3.26s; aggregate tooling-audit pytest passed 1 test in 4:19 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `48fbadf1` | Gated named package handoff artifacts. `appgen.package-verify-cli-audit.v1` now publishes required, emitted, and missing handoff artifact names by target, and `package_manifest_handoff_contracts` fails if any named web, mobile, desktop, PBC, or deployment handoff artifact is absent. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused package handoff pytest passed 1 test in 3.66s; aggregate tooling-audit pytest passed 1 test in 4:18 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `bf96cd6b` | Exposed named release text handoff markers. `appgen.release-verifier-text-renderer.v1` now publishes required, emitted, and missing release markers, graph markers, target statuses, blocking gaps, and artifact markers; `release_text_evidence_contracts` fails if any named human handoff marker is missing. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused release-verifier text renderer pytest passed 1 test in 2.27s; aggregate tooling-audit pytest passed 1 test in 4:34 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `8101e987` | Exposed package readiness matrix evidence. `appgen.package-verify-cli-audit.v1` now publishes web, mobile, desktop, PBC, and deployment readiness checks, aggregate readiness counts, and exact `target.check_name` gaps; `package_manifest_handoff_contracts` requires all 29 readiness checks to pass. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused package handoff audit pytest passed 1 test in 3.57s; aggregate tooling-audit pytest passed 1 test in 4:23 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `6c4b8ff3` | Exposed named tooling-audit text log markers. `appgen.tooling-audit-text-renderer.v1` now reports required, emitted, and missing check ids, section anchors, embedded detail formats, and blocking-gap ids; `tooling_audit_text_renderer` fails if any named human-release-log marker is missing. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused tooling-audit text-renderer pytest passed 1 test in 3.07s; aggregate tooling-audit pytest passed 1 test in 4:21 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `c5b0e42f` | Exposed tooling contract provenance counts. `appgen.tooling-doc-anchor-audit.v1` now publishes per-format docs/runtime/test reference counts, runtime/test covered format counts, missing-reference gap counts, minimum runtime/test reference counts, and a per-format matrix; `tooling_doc_anchor_integrity` requires all documented `appgen.*.v1` formats to have runtime and test provenance. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused tooling doc-anchor audit pytest passed 1 test in 2.12s; aggregate tooling-audit pytest passed 1 test in 4:47 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `311778c9` | Gated the documented LSP method matrix. `appgen.lsp-json-rpc-audit.v1` now reports every method listed in the `docs/tooling.md` LSP capabilities table with provider/notification source, advertised status, exercised status, and backing check id; `lsp_transport_rpc_contracts` requires all 11 methods to pass with zero missing method contracts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 4.55s; aggregate tooling-audit pytest passed 1 test in 4:17 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `df58c607` | Gated completion-context filtering in aggregate LSP evidence. `appgen.lsp-json-rpc-audit.v1` now reports top-level, table, view, flow, composition, deploy, package, and agent completion contexts with missing context counts, missing expected-label counts, and forbidden label leak counts; `lsp_navigation_completion_contracts` requires all eight contexts to pass with zero label misses or leaks. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 4.94s; aggregate tooling-audit pytest passed 1 test in 4:31 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `a2f6a693` | Gated catalog reference index depth. `appgen.lsp-json-rpc-audit.v1` now reports editable workspace and read-only catalog reference counts for PBC-key references and event-contract references, and `lsp_navigation_completion_contracts` requires both reference families to include workspace and catalog locations. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 4.68s; aggregate tooling-audit pytest passed 1 test in 4:28 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `3bdd55a3` | Gated context-aware definition targets. `appgen.lsp-json-rpc-audit.v1` now reports named definition contexts for PBC includes, API/event targets, deployment health targets, deployment resource targets, and deployment environment targets, including expected/observed declaration lines, passing counts, and missing context names; `lsp_navigation_completion_contracts` requires all contexts to resolve. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 4.87s; aggregate tooling-audit pytest passed 1 test in 4:21 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `b22fc371` | Gated catalog-backed workspace symbol discovery. `appgen.lsp-json-rpc-audit.v1` now reports catalog workspace-symbol query names, passing/missing query counts, PBC and contract result counts, catalog URI sets, PBC keys, and contract names, and `lsp_navigation_completion_contracts` requires catalog PBC and contract discovery evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 4.60s; aggregate tooling-audit pytest passed 1 test in 4:14 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `50b0e69b` | Gated named LSP hover surfaces. `appgen.lsp-json-rpc-audit.v1` now reports required, observed, and missing hover surfaces for PBC catalog metadata, diagnostic explanations, relationship fields, lookup paths, and handler targets, and `lsp_navigation_completion_contracts` requires all five surfaces with zero named misses. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 6.94s; aggregate tooling-audit pytest passed 1 test in 4:43 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `7c757315` | Gated lexical reference-scope evidence. `appgen.lsp-json-rpc-audit.v1` now reports scoped reference match counts, expected code-reference line counts, excluded comment/string match counts, and expected/excluded line sets, while `lsp_navigation_completion_contracts` requires that evidence plus separate definition/reference text markers. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP JSON-RPC audit pytest passed 1 test in 4.61s; focused LSP text renderer pytest passed 1 test in 2.07s; aggregate tooling-audit pytest passed 1 test in 4:47 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `54104094` | Made implementation-phase completion evidence reviewable. `appgen.tooling-implementation-phase-audit.v1` now reports passing exit-criterion ids, passing ids by phase, and per-criterion evidence-format maps so release dashboards and agents can prove which phase criteria passed and which machine-readable reports support them without expanding every nested criterion. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase audit pytest passed 1 test in 2.08s; aggregate tooling-audit pytest passed 1 test in 5:03 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `632764e5` | Exposed named Studio frontend bridge failures. The aggregate `frontend_semantic_service_bridge` and `frontend_interaction_audit_bridge` gates now surface required, observed, and missing names for frontend services, surfaces, surface contracts, scenarios, audit inputs, and helpers, and require those named-missing tuples to be empty instead of relying on count-only evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused Studio semantic-service audit pytest passed 1 test in 4.47s; aggregate tooling-audit pytest passed 1 test in 4:34 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `1134de25` | Exposed package handoff coverage gaps. `appgen.package-verify-cli-audit.v1` now reports failing-case names, case ids, expected targets, manifest target coverage, missing manifest targets, manifest formats, handoff counts by target, release-evidence report counts, missing release reports, and release graph kind/format gaps. The aggregate package manifest gate now requires the stronger target, manifest, release report, and graph-bundle evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused package CLI audit pytest passed 2 tests in 4.37s; aggregate tooling-audit pytest passed 1 test in 4:16 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `6f50bdcc` | Exposed graph-suite and explain audit detail counts. `appgen.graph-suite-cli-audit.v1` now reports expected/present rendering counts, complete rendering kind counts, missing format counts, and named text-fragment ids. `appgen.explain-cli-audit.v1` now reports failing-case names, case ids, text/JSON report-format counts, navigation-detail case names, and symbol/diagnostic/handler navigation-detail counts. Aggregate graph and explain gates require the stronger evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused explain and graph-suite audit pytest passed 2 tests in 3.21s; aggregate tooling-audit pytest passed 1 test in 4:16 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `3e00b0b3` | Exposed LSP quick-fix and rename failure identities. `appgen.lsp-code-action-cli-audit.v1` now reports failing-case names, case ids, expected-text match counts, forbidden-text removal counts, and lint-format counts. `appgen.lsp-rename-cli-audit.v1` now reports named scenario ids, failing-scenario names/counts, and safe JSON, blocked JSON, and blocked text scenario counts. Aggregate language-server and quick-fix gates require the stronger evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP code-action and rename audit pytest passed 2 tests in 10.25s; aggregate tooling-audit pytest passed 1 test in 4:12 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `a322ec24` | Broadened non-PBC tooling evidence for lint-directory and component publication contracts. `appgen.lint-directory-cli-audit.v1` now exposes named scenario ids, failing-scenario counts, named failing scenarios, stage-profile ids, passing/failing stage-profile counts, and required/observed/missing stage and severity names. `appgen.component-publish-cli-audit.v1` now exposes case ids and side-effect markers for the rejected missing-catalog path, and aggregate gates require the stronger counters. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused component publish and lint-directory audit pytest passed 2 tests in 1.64s; direct tooling audit probe proved `True True 0 0 True 0 False`; aggregate tooling-audit pytest passed 1 test in 6:13 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `8c2f9d3a` | Exposed validate/generate filesystem handoff counters. `appgen.validate-generate-cli-audit.v1` now reports existing-manifest cases, artifact-path cases, artifact-path-missing counts, blocked-output absence cases, payload formats, and generated blocking-gap names, and the aggregate generation policy gate requires those counters. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused validate/generate CLI audit pytest passed 1 test in 9.98s with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 4:32 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `62605017` | Exposed natural-language planner and test-strategy gap counters. `appgen.nl-plan-cli-audit.v1` now reports required and missing accepted operation kinds, accepted failing cases, text marker counts, blocking case counts, text test-plan/token-note line counts, and rejection status. `appgen.test-strategy-cli-audit.v1` now reports failing cases, case ids, required/observed/missing surfaces, payload formats, and missing-surface counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused NL planner CLI and test-strategy CLI pytest passed 2 tests in 15.44s with existing PBC datetime warnings; aggregate tooling-audit pytest passed 1 test in 4:31 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `461f2413` | Exposed service and CLI help audit counters. LSP text logs now emit a `service_counts ...` line with completion-source, reference, symbol, code-action, formatting, and rename-edit counts, and `appgen.cli-help-surface-audit.v1` plus aggregate gates now expose listed-subcommand and failing-entrypoint-dispatch counters. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP service text renderer and CLI help alias pytest passed 2 tests in 3.74s, then 2 tests in 2.16s after aggregate detail wiring; aggregate tooling-audit pytest passed 1 test in 4:37 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `c623034f` | Exposed broader tooling audit failure evidence in one slice. `appgen.package-invalid-target-audit.v1` now covers both package and verify invalid-target paths; `appgen.migration-cli-audit.v1` reports failing cases, observed/missing backends, required/observed/missing change kinds, approval counts, and rename-hint counts; tooling-audit text now includes implementation-phase exit-criterion totals. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused package invalid-target, migration CLI, and tooling-audit text renderer pytest passed 3 tests in 2.38s, then focused migration/text rerun passed 2 tests in 0.82s after aggregate integration fix; aggregate tooling-audit pytest passed 1 test in 4:23 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `7b324f0f` | Strengthened natural-language planner operation coverage evidence. `appgen.nl-plan-contract-audit.v1` now reports observed operation kinds, missing required operation kinds/counts, and blocking-gap count, and the aggregate `natural_language_operation_contracts` gate exposes those fields so agents can verify every documented NL edit family without expanding all cases. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused NL planner contract and CLI audit pytest passed 2 tests in 8.31s; direct contract probe proved `appgen.nl-plan-contract-audit.v1 True 13 0 0 ()`; aggregate tooling-audit pytest passed 1 test in 5:40 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `157f1a12` | Added per-phase implementation audit counters. `appgen.tooling-implementation-phase-audit.v1` now reports exit-criterion, passing, and missing counts by phase in addition to aggregate counts and missing criteria, so release triage can review phase completion without expanding every nested criterion. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase audit pytest passed 1 test in 1.96s; aggregate tooling-audit pytest passed 1 test in 5:38 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `0b0d011c` | Expanded designer-sync text renderer evidence to the full documented visual edit matrix. Human-readable release logs now prove database field, form component, workflow transition, PBC composition include, package creation, deployment-unit creation, and invalid binding rejection case ids, with required operation names aligned to `designer_visual_edit_matrix_dsl`. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused designer-sync renderer and CLI audit pytest passed 2 tests in 2.91s; direct renderer probe proved `appgen.designer-sync-text-renderer.v1 True 7 ()`; aggregate tooling-audit pytest passed 1 test in 5:30 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `f56ae794` | Promoted multi-document LSP workspace behavior into the JSON-RPC release audit. The audit now opens multiple DSL buffers and proves cross-document definition, references, completion, workspace symbol search, and rename through `workspace_document_scan_and_rename`, matching the `docs/tooling.md` promise that editor requests scan open documents individually instead of concatenating source files. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused JSON-RPC audit and multi-document LSP pytest passed 3 tests in 3.46s; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 19 19 True ()`; aggregate tooling-audit pytest passed 1 test in 5:38 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `40457c50` | Exposed handler-target hover depth for event wiring. Handler target hover now emits `appgen.lsp-handler-target-hover.v1` with owner kind/name, event, target, target kind, target symbol id, resolution status, and handler-graph edge metadata. The JSON-RPC release audit now includes `hover_handler_target_depth`, and `docs/tooling.md` documents the audited payload. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused hover and JSON-RPC pytest passed 2 tests in 7.66s; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 18 18 True ()`; aggregate tooling-audit pytest passed 1 test in 5:36 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `55283e9b` | Exposed LSP hover depth for database-backed relationships and lookup paths. Field hover now emits `appgen.lsp-relationship-hover.v1` with target table, target field, cardinality, and alias metadata; lookup-path hover emits `appgen.lsp-lookup-hover.v1` with the resolved table context and semantic chain. The JSON-RPC release audit now includes `hover_relationship_lookup_depth`, and `docs/tooling.md` documents the audited payloads. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused hover and JSON-RPC pytest passed; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 17 17 True ()`; focused plus aggregate tooling pytest passed 3 tests in 5:54 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `f626cb58` | Expanded LSP completion evidence to every documented cursor context. The `completion_context_filtering` audit now covers top-level, table, view, flow, composition, deployment, package, and agent contexts, and table completions include directive and field-group spread labels so the audited surface matches `docs/tooling.md`. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused completion coverage, context filtering, and JSON-RPC audit pytest passed 3 tests in 4.69s; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 16 16 ()`; top-level tooling-audit pytest passed 1 test in 5:58 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `2f6d3e23` | Made LSP completions context-aware instead of broad global lists. The language server now classifies top-level, table, view, flow, composition, deployment, package, and agent cursor contexts, filters completion families accordingly, exposes context metadata through JSON-RPC, and release-audits top-level/composition/deploy/agent filtering through `completion_context_filtering`. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused completion, JSON-RPC, and stdio pytest passed 3 tests in 4.36s; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 16 16 True ()`; top-level tooling-audit pytest passed 1 test in 5:02 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `a8884bb4` | Kept LSP reference search scoped to lexical code. Reference search now skips quoted strings, line comments, hash comments, and block comments while preserving workspace references and catalog references; the JSON-RPC audit now includes `lexical_reference_scope` so release evidence proves handler/declaration references without treating authoring evidence as symbol usage. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused reference-scope and JSON-RPC audit pytest passed 2 tests in 1.14s; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 15 15 True ()`; top-level tooling-audit pytest passed 1 test in 4:13 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `e9eeca4c` | Made LSP definition and hover resolution context-aware for enterprise symbols. Definition navigation now routes PBC include references, API/event contract targets, and deployment health/resource/env references to their intended declarations even when fields or operations share the same token; semantic symbol ranges now locate declarations in their owning blocks instead of the first same-named token. The JSON-RPC audit includes an `enterprise_definition_context` check. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused definition-context and JSON-RPC audit pytest passed 2 tests in 2.92s; direct JSON-RPC audit proved `appgen.lsp-json-rpc-audit.v1 True 14 14 True ()`; top-level tooling-audit pytest passed 1 test in 4:32 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `93332fa4` | Strengthened the framed LSP stdio transport contract for `didChange`. The stdio audit now changes a valid open buffer into an invalid database-backed form binding and requires the changed `publishDiagnostics` response to carry semantic error evidence, so the transport gate proves cache mutation and re-linting instead of only counting diagnostic notifications. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused stdio transport pytest passed 1 test in 4.73s; direct stdio audit proved `appgen.lsp-stdio-transport-audit.v1 True 2 True 2 1 ('AGX0402', 'AGX0000')`; top-level tooling-audit pytest passed 1 test in 9:47 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `e88dfd41` | Expanded safe rename coverage to the remaining enterprise tooling surfaces promised by `docs/tooling.md`. PBC key renames now scope candidate edits to PBC declarations, composition includes, composition connection endpoints, and deployment unit/health/resource/env targets; event contract renames update event declarations plus API/composition references while preserving same-named operations and handlers; package renames update package declarations only; deployment-unit renames update unit/health/resource/env targets while preserving operation declarations. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused enterprise/field/CLI rename pytest passed 3 tests in 13.67s; direct rename CLI audit proved `appgen.lsp-rename-cli-audit.v1 True 11 11 True True True True True`; top-level tooling-audit pytest passed 1 test in 9:49 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `873838c2` | Restored EOF insert semantics for LSP quick fixes after the cursor-aware rename helper changed end-of-document position handling. Required append-style quick fixes no longer duplicate the source document, so the quick-fix application family is back to 15/15 lint-clean cases and the aggregate tooling audit is green again. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused quick-fix and rename pytest passed 4 tests in 6.47s; direct quick-fix CLI audit proved `True 15 15 ()`; top-level tooling-audit pytest passed 1 test in 4:09 with existing PBC datetime warnings; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository suite remains deferred for active PBC isolation and velocity. |
| 2026-05-30 | `aae6b177` | Added table-scoped field rename candidates. Field rename now uses cursor context to select the owning table field, updates the field declaration, calculated expressions in that table, and database-backed view bindings/components for views over that table, while preserving same-named fields on other tables, handler targets, lookup-path leaves, operation names, comments, and strings. Per-change migration approval flags now block inferred field rename candidates so review-visible previews are not silently accepted. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP rename pytest passed 6 tests in 6.90s; direct rename CLI audit proved `appgen.lsp-rename-cli-audit.v1 True 7 7 True field_declarations_and_bindings 4 True True True True True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full top-level tooling audit currently fails on existing quick-fix application gaps (`lsp_quick_fix_application`, `lsp_quick_fix_coverage_contracts`, `lsp_quick_fix_cli_contracts`) and remains the next delivery target. |
| 2026-05-30 | `106401fc` | Added symbol-kind-aware view rename candidates. View renames now scope candidate edits to view declarations and menu item navigation targets while preserving same-named fields, operations, comments, and string literals; blocked safety previews still expose the candidate edit for review. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP rename pytest passed 5 tests in 15.04s; top-level tooling-audit pytest passed 1 test in 9:31 with existing PBC datetime warnings; direct tooling probe proved `appgen.tooling-audit.v1 True 55` and `appgen.lsp-rename-cli-audit.v1 True 6 6 True view_declarations_and_targets 2 True True True True True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `bca95be1` | Added symbol-kind-aware table rename candidates. Table renames now scope candidate edits to table declarations, database-backed view subjects, relationship targets, and report sources while preserving same-named fields, comments, and string literals; migration-risk blocking still applies until an explicit migration decision is available. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP rename pytest passed 4 tests in 6.14s; top-level tooling-audit pytest passed 1 test in 4:40 with existing PBC datetime warnings; direct tooling probe proved `appgen.tooling-audit.v1 True 55` and `appgen.lsp-rename-cli-audit.v1 True 5 5 True table_declarations_and_targets 4 True True True True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `47af5db4` | Tightened LSP rename from lexical-safe replacement to symbol-kind-aware operation/flow refactors. Operation and workflow renames now update declarations, handler/transition targets, and deployment references while preserving same-named fields, comments, and string literals, and the CLI audit exposes the scoped rename evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP rename pytest passed 3 tests in 4.84s; direct rename probe proved `ok=True`, `blocked=False`, `operation_declarations_and_targets`, 12 scoped occurrences, and zero stale `ReverseInvoice` references; top-level tooling-audit pytest passed 1 test in 3:57 with existing PBC datetime warnings; direct tooling probe proved `appgen.tooling-audit.v1 True 55` and `appgen.lsp-rename-cli-audit.v1 True 4 4 flow_declarations_and_targets 2 operation_declarations_and_targets 4 True True True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `37c8c3ba` | Hardened LSP rename from broad token substitution into lexical identifier replacement. Rename now preserves comments and string literals, reports `lexical_scope` and occurrence counts, rejects reserved-keyword targets, and extends `appgen.lsp-rename-cli-audit.v1` with an explicit lexical-safety scenario. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP rename pytest passed 3 tests in 5.62s; top-level tooling-audit pytest passed 1 test in 4:42 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 55` and `appgen.lsp-rename-cli-audit.v1 True 4 4 code_identifiers True 2 True True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `be80f0fa` | Added concrete subsection coverage evidence for `docs/tooling.md`. `appgen.tooling-audit.v1` now requires `tooling_section_coverage_contracts` to prove all 18 major `##` sections and all 40 concrete `###` subsections map to executable audit gates, with separate stale mapping counts for sections and subsections. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; top-level tooling-audit pytest passed 1 test in 4:07 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 55` and `tooling_section_coverage_contracts True appgen.tooling-section-coverage-audit.v1 18 18 0 40 40 0 0 0 ()`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `21491e7e` | Added full major-section coverage evidence for `docs/tooling.md`. `appgen.tooling-audit.v1` now includes `tooling_section_coverage_contracts`, proving all 18 major `##` sections map to top-level executable audit gates and stale gate mappings fail release evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; top-level tooling-audit pytest passed 1 test in 8:30 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 55` and `tooling_section_coverage_contracts True appgen.tooling-section-coverage-audit.v1 18 18 0 0 ()`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `a2c28e6e` | Added priority-order release evidence. `appgen.tooling-audit.v1` now includes `priority_order_contracts`, proving the ten documented Priority Order items remain in order and each maps to executable tooling evidence before downstream tooling is counted complete. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase plus aggregate tooling pytest passed 2 tests in 4:22 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 54` and `priority_order_contracts True appgen.priority-order-contract-audit.v1 10 10 0 10 True ()`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `48b9e541` | Added contributor task breakdown coverage evidence. `appgen.tooling-audit.v1` now includes `contributor_task_breakdown_contracts`, proving all 22 good-first, intermediate, and advanced contributor tasks map to executable tooling evidence instead of remaining prose-only onboarding advice. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase plus aggregate tooling pytest passed 2 tests in 3:57 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 53` and `contributor_task_breakdown_contracts True appgen.contributor-task-contract-audit.v1 3 ('good_first', 'intermediate', 'advanced') 22 22 0 ()`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `007eeb0e` | Added implementation-phase documentation alignment evidence. `appgen.tooling-audit.v1` now includes `implementation_phase_doc_alignment_contracts`, proving the seven documented phase headings, runtime phase ids, titles, and representative exit criteria stay aligned with `appgen.tooling-implementation-phase-audit.v1`. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase plus aggregate tooling pytest passed 2 tests in 4:21 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 52` and `implementation_phase_doc_alignment_contracts True appgen.implementation-phase-doc-alignment.v1 7 7 0 0 0 7 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `b92723bf` | Added a first-class Test Strategy family contract gate. `appgen.tooling-audit.v1` now includes `test_strategy_family_contracts`, and phase 0 requires `appgen.test-family-contract-audit.v1` evidence proving all 11 documented test families map to executable parser, semantic, diagnostic, formatter, CLI, LSP, graph, migration, natural-language, verifier, and drift coverage. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused phase and test-strategy pytest passed 2 tests; top-level tooling-audit pytest passed 1 test in 3:58 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 51`, `test_strategy_family_contracts True appgen.test-family-contract-audit.v1 11 11 0 ()`, and all family names: parser, semantic, diagnostic, formatter, CLI, LSP, graph, migration, natural-language, verifier, drift; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `a310e51b` | Promoted CLI help and alias evidence into a first-class tooling gate. `appgen.tooling-audit.v1` now includes `cli_help_alias_contracts`, and phase 3 requires help discoverability, subcommand option coverage, module dispatch, and `apg` alias evidence independently of generic usage-failure handling. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused phase and CLI help/alias pytest passed 2 tests; top-level tooling-audit pytest passed 1 test in 4:18 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 50` and `cli_help_alias_contracts True appgen.cli-help-surface-audit.v1 24 0 0 25 25 0 2 2 pyAppGen.__main__:main True True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `42eb041a` | Split LSP quick-fix evidence into first-class coverage, CLI parity, and text-log gates. `appgen.tooling-audit.v1` now exposes separate `lsp_quick_fix_coverage_contracts`, `lsp_quick_fix_cli_contracts`, and `lsp_quick_fix_text_contracts`, and phase 4 requires quick-fix family coverage plus CLI/text contracts rather than relying on the broad application aggregate. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused phase/code-action/text pytest passed 5 tests; top-level tooling-audit pytest passed 1 test in 3:58 with existing PBC datetime warnings; direct probe proved `appgen.tooling-audit.v1 True 49`, quick-fix coverage `15 15 0`, CLI parity `15 15 0`, and text evidence `1 1 1 1 1 False`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full repository test suite remains deferred for active PBC path isolation and velocity. |
| 2026-05-30 | `67c671f8` | Split validation and generation evidence into first-class tooling gates. `appgen.tooling-audit.v1` now includes `validate_target_contracts` and `generate_artifact_policy_contracts`, and phase 3 requires target validation coverage plus generated artifact/manifest handoff and warning/error blocking policy evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused validation/generation/phase pytest passed 5 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True 46`, validation `3 2 2 1`, and generation `4 2 2 1 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `12d78b3b` | Split component and PBC publication evidence into first-class tooling gates. `appgen.tooling-audit.v1` now includes `component_publish_catalog_contracts` and `pbc_publish_side_effect_contracts`, and phase exits require component catalog patch behavior plus side-effect-free PBC publish CLI/text evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused component/PBC/phase pytest passed 7 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True 44`, component publish `3 3 appgen.component-catalog-patch.v1 True False 1`, and PBC publish `2 2 1 True 4`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `1a082f4f` | Split formatter, parser-golden, semantic-drift, and doctor evidence into first-class tooling gates. `appgen.tooling-audit.v1` now includes `formatter_write_organize_contracts`, `parser_golden_fixture_contracts`, `semantic_drift_surface_contracts`, and `doctor_cli_text_contracts`, with phase exits requiring formatter write/organize behavior plus parser fixture, drift surface, and doctor CLI/text evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused formatter/parser/drift/doctor/phase pytest passed 9 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True 42`, formatter `9 9 5 5 7`, parser `53 53 5 0`, drift `8 0 8`, and doctor `18 0 8`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `a6cde9bf` | Split diagnostics and linter evidence into first-class tooling gates. `appgen.tooling-audit.v1` now includes `diagnostic_catalog_fixture_contracts` and `lint_cli_directory_contracts`, and phase 2 requires diagnostic shape/fixture coverage plus strict lint, directory input, stage separation, and file-scoped diagnostic evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused diagnostics/lint/phase pytest passed 6 tests, then focused lint/phase rerun passed 2 tests after aligning the text renderer stage-line contract; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True 38`, diagnostics `28 28 28 28 0 0`, and lint CLI `8 8 3 True True 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `ee8babd2` | Split natural-language planner evidence into first-class operation and CLI/agent handoff gates. `appgen.tooling-audit.v1` now includes `natural_language_operation_contracts` and `natural_language_cli_agent_contracts`, and phase 6 requires operation coverage, rejection diagnostics, token-budget evidence, text handoff markers, and zero blocking cases instead of relying only on the broad planner aggregate. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase, NL contract, and NL CLI pytest passed 3 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True 36`, NL operation contract `14 14 13 13 14 ()`, and NL CLI/agent contract `15 13 13 13 3 3 ()`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `517c2aea` | Split migration safety and text evidence into a first-class tooling gate. `appgen.tooling-audit.v1` now includes `migration_safety_text_contracts`, and phase 6 requires migration detection breadth, supported backend CLI evidence, approval posture, safe alternatives, and migration text renderer safety markers. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase, migration CLI, and migration text renderer pytest passed 3 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True True 14 14 () appgen.migration-cli-audit.v1 3 3 5 appgen.migration-plan-text-renderer.v1 2 1 False 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `42ea7cbc` | Split graph rendering and explain behavior into first-class tooling gates. `appgen.tooling-audit.v1` now includes `graph_rendering_contracts` and `explain_cli_contracts`, and phase 3 requires graph kind/rendering breadth plus explain text/JSON format coverage and graph/explain text renderer evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase, graph CLI, graph-suite, explain CLI, and graph/explain text renderer pytest passed 5 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True True appgen.graph-suite-report.v1 0 0 appgen.graph-cli-format-audit.v1 10 0 0 True appgen.explain-cli-audit.v1 6 0 appgen.graph-explain-text-renderer.v1 0 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `1465e069` | Split language-server transport and navigation/completion behavior into first-class tooling gates. `appgen.tooling-audit.v1` now includes `lsp_transport_rpc_contracts` and `lsp_navigation_completion_contracts`, and phase 4 requires JSON-RPC/stdio transport evidence plus completion, symbol, navigation, and LSP text renderer evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase, LSP JSON-RPC, stdio, text renderer, and symbol coverage pytest passed 5 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True True appgen.lsp-json-rpc-audit.v1 9 8 appgen.lsp-stdio-transport-audit.v1 4 () True appgen.completion-coverage.v1 0 0 0 2 False 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `451a46af` | Split package manifest handoff and release text evidence into first-class tooling gates. `appgen.tooling-audit.v1` now includes `package_manifest_handoff_contracts` and `release_text_evidence_contracts`, and phase 6 requires package manifest breadth plus release text marker evidence instead of relying only on the broader package/release verifier aggregate. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase, package CLI, and release text renderer pytest passed 5 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True True appgen.package-verify-cli-audit.v1 5 5 26 appgen.graph-suite-report.v1 True appgen.release-verifier-text-renderer.v1 2 3 2 2 False 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `6294f841` | Split CLI usage and controlled-failure behavior into a first-class tooling gate. `appgen.tooling-audit.v1` now includes `cli_usage_failure_contracts`, and phase 3 requires internal-error, missing-input, missing-required-option, and invalid-choice exit contracts instead of relying only on the broader validation/generation aggregate. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase pytest passed 1 test; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True True appgen.internal-error-exit-audit.v1 2 appgen.missing-input-exit-audit.v1 0 appgen.missing-required-option-exit-audit.v1 0 appgen.invalid-choice-exit-audit.v1 8 8 appgen.cli-help-surface-audit.v1 () 0 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `e586d6d1` | Promoted frontend Studio bridge audits to first-class tooling gates. `appgen.tooling-audit.v1` now includes explicit `frontend_semantic_service_bridge` and `frontend_interaction_audit_bridge` checks, and phase 5 requires browser-smoke semantic-service and interaction-audit bridge evidence instead of relying only on nested Studio details. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused Studio semantic-service and implementation-phase pytest passed 2 tests; top-level tooling-audit pytest passed 1 test; direct probe proved `appgen.tooling-audit.v1 True True appgen.frontend-semantic-service-audit.v1 0 0 0 True appgen.frontend-interaction-audit.v1 8 0 0 0 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `29c4c3f1` | Added first-class DSL language-quality CLI contracts. `appgen dsl-quality`, `appgen dsl-antlr`, `appgen dsl-authoring-gate <file>`, and `appgen dsl-language-service <file>` now expose grammar/parser sync, keyword-budget, authoring-gate, and language-service evidence directly, and `appgen.dsl-language-cli-audit.v1` is wired into the top-level tooling audit and CLI help surface. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py src/pyAppGen/__main__.py src/pyAppGen/gen.py tests/test_dsl_tooling_contracts.py`; focused pytest for DSL language CLI audit, CLI help surface, and missing-input audit passed 3 tests; direct probe proving `appgen.dsl-language-cli-audit.v1 True 8 8 0 33` and `appgen.tooling-audit.v1 True True appgen.dsl-language-cli-audit.v1 8 8 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `9349dc94` | Added top-level DSL language-quality gating to the tooling audit. `appgen.tooling-audit.v1` now embeds `appgen.dsl-language-quality.v1`, and phase 0 exit criteria require grammar/parser sync, required enterprise grammar rules, keyword budget, authoring aliases, and progressive learning-path evidence before tooling completion can pass. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused implementation-phase and tooling-audit pytest passed 2 tests; direct probe proving `appgen.dsl-language-quality.v1 True True True 32` and `appgen.tooling-audit.v1 True True appgen.dsl-language-quality.v1 21 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full external CLI traversal remains deferred for active PBC path isolation. |
| 2026-05-30 | `81d79cbe` | Strengthened LSP service text renderer evidence. `appgen.lsp-service-text-renderer.v1` now reports summary-line, source-line, completion-line, completion-missing-line, navigation-line, formatting-line, rename-line, rename-blocker-line, hover-summary-line, and hover-line counts so editor-service text logs prove diagnostics/completions/actions/symbols, source-of-truth, navigation, formatting, completion gaps, rename blockers, and hover evidence remain visible. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused LSP service text renderer pytest; direct probe proving `appgen.lsp-service-text-renderer.v1 True 12 0 11 1 1 2 1 2 1 1 1 1 2`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
| 2026-05-30 | `b45334b6` | Strengthened format and designer-sync text renderer evidence. `appgen.format-text-renderer.v1` now reports summary/write-path/diagnostic/write-flag/idempotence/organize line counts, and `appgen.designer-sync-text-renderer.v1` now reports summary/surface/visual-edit/dsl-diff/matrix/operation/case/check line counts. The designer-sync test also now asserts designer counters instead of migration counters, with migration counters asserted in the migration renderer test. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused format/designer/migration text renderer pytest; direct probes proving `appgen.format-text-renderer.v1 True 3 0 3 1 1 1 1 0 1 1 1`, `appgen.designer-sync-text-renderer.v1 True 12 0 12 1 1 1 2 1 1 3 2 2 0`, and `appgen.migration-plan-text-renderer.v1 True 1 1 1 1 3 2 1 1 0 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `0e9ee8b2` | Strengthened migration-plan text renderer evidence. `appgen.migration-plan-text-renderer.v1` now reports summary-line, coverage-line, detected-family-line, missing-family-line, change-line, safe-alternative-line, diagnostic-line, warning-line, error-line, approval-line, and destructive-summary-line counts so migration release logs prove safety posture, coverage breadth, destructive-change count, and mitigation guidance. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused migration-plan text renderer pytest; direct probe proving `appgen.migration-plan-text-renderer.v1 True 10 0 10 1 1 1 1 3 2 1 1 0 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `7d9e9d2d` | Strengthened semantic drift text renderer evidence. `appgen.semantic-drift-text-renderer.v1` now reports summary-line, surface-line, gap-line, evidence-line, check-line, passing/failing check-line, and digest-line counts so shared-model drift logs prove surface coverage, evidence references, named gaps, check outcomes, and digest identity. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused semantic drift text renderer pytest; direct probe proving `appgen.semantic-drift-text-renderer.v1 True 8 0 8 1 1 1 3 2 1 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `a759c666` | Strengthened diagnostics text renderer evidence. `appgen.diagnostics-text-renderer.v1` now reports summary-line, required-code-line, covered-fixture-line, covered-code-line, missing-code-line, and blocking-gap-line counts so diagnostic text logs prove catalog coverage, fixture coverage, missing diagnostic coverage, and blocking fixture gaps. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused diagnostics text renderer pytest; direct probe proving `appgen.diagnostics-text-renderer.v1 True 8 0 12 2 3 3 2 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `4faf3822` | Strengthened lint text renderer evidence. `appgen.lint-text-renderer.v1` now reports source-file-line, stage-line, migration-line, migration-preview-line, migration-detected-line, diagnostic-line, error-line, and warning-line counts so lint text logs prove source coverage, stage coverage, migration preview evidence, and diagnostic severity visibility. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused lint text renderer pytest; direct probe proving `appgen.lint-text-renderer.v1 True 9 0 7 2 1 2 1 1 2 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `b908c7e9` | Strengthened validate/generate text renderer evidence. `appgen.validate-generate-text-renderer.v1` now reports summary-line, validation check-line, passing/failing check-line, target-detail-line, artifact-line, manifest-line, gap-line, diagnostic-line, warning-line, and error-line counts so validation/generation human logs prove target mismatch, artifact handoff, gaps, and diagnostics remain visible. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused validate/generate text renderer pytest; direct probe proving `appgen.validate-generate-text-renderer.v1 True 12 0 10 2 2 1 1 2 1 1 1 2 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `e40da14e` | Strengthened component publication text renderer evidence. `appgen.component-publish-text-renderer.v1` now reports summary-line, catalog-line, side-effect-line, patch-contract-line, and existing-catalog-line counts so non-PBC component publication logs prove catalog context and side-effect-free patch posture remain visible. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused component publication text renderer pytest; direct probe proving `appgen.component-publish-text-renderer.v1 True 8 0 3 1 2 1 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `eaf35e2e` | Strengthened release verifier text renderer evidence. `appgen.release-verifier-text-renderer.v1` now reports release-line, graph-line, target-status-line, passing/failing target-line, blocking-gap-line, and artifact-line counts so package handoff logs prove target status, graph-suite context, gaps, and written artifact paths remain visible. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused release verifier text renderer pytest; direct probe proving `appgen.release-verifier-text-renderer.v1 True 9 0 9 2 3 2 1 1 1 2`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `e73962f3` | Strengthened tooling-audit text renderer evidence. `appgen.tooling-audit-text-renderer.v1` now reports check-line, passing/failing check-line, detail-format-line, section-line, blocking-gap-line, and implementation-phase-line counts so human release logs prove they preserved checks, embedded audit formats, sections, and failure markers. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused tooling-audit text renderer pytest; direct probe proving `appgen.tooling-audit-text-renderer.v1 True 12 0 23 10 10 0 10 8 1 2`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `4bb1963b` | Strengthened CLI usage-failure audit evidence. Missing-input, missing-required-option, and invalid-choice audits now report case ids, failing case ids/counts, stdout-empty counters, and command-family coverage for missing-input paths so release triage identifies the exact broken CLI surface. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused CLI usage-failure pytest; direct probes proving `appgen.missing-input-exit-audit.v1 True 17 17 0 14 17 17 17`, `appgen.missing-required-option-exit-audit.v1 True 4 4 0 4 4 4`, and `appgen.invalid-choice-exit-audit.v1 True 8 8 0 8 8 8`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `3cb9e9d1` | Strengthened CLI help surface audit dispatch evidence. `appgen.cli-help-surface-audit.v1` now reports command-alias counts, module/repo-alias entrypoint dispatch counts, failing option surfaces, and top-level help byte counts so agent-facing command discoverability failures identify the exact surface. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused CLI help surface pytest; direct probe proving `appgen.cli-help-surface-audit.v1 True 2 2 21 21 0 0 0 506`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `f7c93304` | Expanded graph CLI and graph-suite audit breadth. `appgen.graph-cli-format-audit.v1` now covers every required graph kind and reports failing cases, missing required graph kinds, per-format case counts, payload-format coverage, and text-marker coverage; `appgen.graph-suite-cli-audit.v1` now reports missing required kinds and missing text fragments. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused graph CLI and graph-suite pytest; direct probes proving `appgen.graph-cli-format-audit.v1 True 10 10 0 9 0 3 3 4 3 3 7` and `appgen.graph-suite-cli-audit.v1 True 9 0 3 0 3 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `04732af5` | Strengthened validate/generate CLI audit outcome evidence. `appgen.validate-generate-cli-audit.v1` now reports failing case counts, case ids, generated success/blocked cases, validation rejection cases, manifest case counts, artifact handoff case counts, and blocking-gap case counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for validate/generate CLI audit and doc-anchor audit; direct probe proving `appgen.validate-generate-cli-audit.v1 True 7 7 0 4 3 2 2 2 2 1 2`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `f627eff3` | Strengthened formatter write audit gap evidence. `appgen.format-write-audit.v1` now reports scenario ids, failing scenario counts, failing scenario ids, and blocking-gap counts across dirty-check, clean-check, organize, JSON-write, and text-write paths. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for formatter write audit and doc-anchor audit; direct probe proving `appgen.format-write-audit.v1 True 5 5 0 5 0 2 2 7`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `208d6e83` | Strengthened Studio semantic-service audit gap evidence. `appgen.studio-semantic-service-audit.v1` now reports check/pass/fail counts, missing required Studio surfaces, surface-format gaps, semantic-surface-format gaps, and blocking-gap counts so IDE bridge failures identify the exact missing panel or contract. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for Studio semantic-service audit and doc-anchor audit; direct probe proving `appgen.studio-semantic-service-audit.v1 True 12 12 0 10 0 0 0 0 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `22c86b81` | Strengthened LSP JSON-RPC and stdio audit evidence. `appgen.lsp-json-rpc-audit.v1` now reports failing-check, missing-provider, passing-request, request-id, and blocking-gap evidence; `appgen.lsp-stdio-transport-audit.v1` now reports total/notification message counts, expected/missing response ids, and completion/workspace/shutdown response counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for LSP JSON-RPC audit, LSP stdio transport audit, and doc-anchor audit; direct probes proving `appgen.lsp-json-rpc-audit.v1 True 13 13 0 9 0 8 8 0` and `appgen.lsp-stdio-transport-audit.v1 True 7 4 3 4 0 2 1 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `74f301b5` | Strengthened LSP code-action audit failure evidence. `appgen.lsp-code-action-apply-audit.v1` and `appgen.lsp-code-action-cli-audit.v1` now report failing-case, lint-failing, blocking-gap, and additional diagnostic/changed-case counts so quick-fix breadth and failures are visible without expanding every case. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for in-process code-action audit, CLI code-action audit, and doc-anchor audit; direct probes proving `appgen.lsp-code-action-apply-audit.v1 True 15 15 0 15 0 15 0 13 0` and `appgen.lsp-code-action-cli-audit.v1 True 15 15 0 15 0 15 0 15 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `1f641d33` | Strengthened release verifier aggregate evidence. `appgen.release-verifier-report.v1` now reports target, check, passing/failing-check, report, diagnostic, graph-kind, graph-format, evidence-artifact, and written-artifact counts; target verifier payloads now report check, passing-check, and blocking-gap counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for release verifier evidence, package evidence writing, and doc-anchor audit; direct probe proving `appgen.release-verifier-report.v1 True 5 5 5 0 5 1 9 3 5 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `5f66c786` | Strengthened parser-golden aggregate evidence. `appgen.parser-golden-audit.v1` now reports required/covered/missing construct counts, fixture pass/fail counts, parsed-valid counts, rejected-invalid counts, and blocking-gap counts so grammar coverage can be reviewed without recomputing from arrays. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for parser-golden audit, parser-golden CLI text/JSON, and doc-anchor audit; direct probe proving `appgen.parser-golden-audit.v1 True 53 53 0 5 5 0 2 3 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `52f0a335` | Strengthened implementation-phase audit traceability. `appgen.tooling-implementation-phase-audit.v1` now reports phase ids, exit-criterion ids, missing-exit-criterion counts, and missing criteria grouped by phase so tooling completion gaps are actionable without expanding every nested phase. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for implementation-phase audit and doc-anchor audit; direct probe proving `appgen.tooling-implementation-phase-audit.v1 True 7 7 20 0 7 20`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `88ee2a58` | Strengthened migration-plan aggregate evidence. `appgen.migration-plan.v1` now reports allowed-backend, change, destructive-change, diagnostic, and rename-hint counts so agents can triage migration risk without expanding every change object. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for migration-plan aggregate counters, migration coverage with rename hints, and doc-anchor audit; direct probe proving `appgen.migration-plan.v1 True 3 13 5 1 2`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `1519acc3` | Strengthened explain CLI aggregate evidence. `appgen.explain-cli-audit.v1` now reports passing-case, exit-failure, missing-report-format, symbol-case, diagnostic-case, and handler-case counts in addition to total/text/JSON/report-format case counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for explain CLI audit, graph/explain text renderer, and doc-anchor audit; direct probe proving `appgen.explain-cli-audit.v1 True 6 6 0 3 3 6 0 2 2 2`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `5c64d9d4` | Strengthened graph-suite aggregate evidence. `appgen.graph-suite-report.v1` now reports required/present/missing graph-kind counts, format/report/rendering/expected-rendering/missing-rendering counts, diagnostic/check/passing/blocking counts, and exact missing kind/rendering lists. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for graph-suite report, graph-suite CLI audit, and doc-anchor audit; direct probe proving `appgen.graph-suite-report.v1 True 9 9 0 3 9 27 27 0 0 3 3 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `2b89797a` | Strengthened validate/generate aggregate evidence. `appgen.validate-report.v1` now reports requested/app target counts, check/passing/blocking check counts, blocking check ids, diagnostic counts, and target-diagnostic counts; `appgen.generate-report.v1` now reports artifact, diagnostic, and blocking-gap counts plus manifest existence across success and blocked paths. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for validate, target rejection, generate, and doc-anchor audits; direct probe proving validation `appgen.validate-report.v1 True 2 2 6 6 0 1 0`, missing target `appgen.validate-report.v1 False 1 1 1`, generated `appgen.generate-report.v1 True True 1266 True 1 0`, and blocked `appgen.generate-report.v1 False False 0 1 1`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `53c14b9f` | Strengthened lint report aggregate evidence. `appgen.lint-report.v1` now reports top-level stage, severity, file, diagnostic, and fix counts plus `fixes_available` for single-file, empty directory, and multi-source reports. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for single-file lint, directory lint, and doc-anchor audit; direct probe proving single-file `appgen.lint-report.v1 False 3 4 1 1 0 False` and directory `appgen.lint-report.v1 False directory 3 4 2 3 0 False`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `fd51fde9` | Strengthened formatter contract breadth counters. `appgen.formatter-contract-audit.v1` now reports failed-check, idempotent-report, changed-report, diagnostic, diagnostic-error, diagnostic-severity, and output byte counts in addition to check, comment-check, ordering-check, and report counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for formatter contract and doc-anchor audit; direct probe proving `appgen.formatter-contract-audit.v1 True 9 9 0 2 2 2 4 0 619` with severity counts `error=0`, `hint=4`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `90a9152e` | Strengthened module-boundary audit counts. `appgen.module-boundary-audit.v1` now reports passing-boundary, missing-boundary, missing-callable, passing-core-runtime, and core-runtime-gap counts, and each boundary reports callable and missing-callable counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for module-boundary audit and doc-anchor audit; direct probe proving `appgen.module-boundary-audit.v1 True 12 12 0 25 0 4 4 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `ee390ca1` | Strengthened diagnostic catalog and fixture audit breadth evidence. `appgen.diagnostic-catalog.v1` now reports range, diagnostic, required-code, covered-fixture-code, shape-field, catalog-field, catalog-shape-gap, and missing-fixture counts; `appgen.diagnostic-fixture-audit.v1` now reports required/covered/missing code counts, fixture/passing/blocking counts, shape/severity gap counts, and exercised report-format counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for diagnostic catalog/fixture audit and doc-anchor audit; direct probe proving `appgen.diagnostic-catalog.v1 True 28 28 28 0 0` and `appgen.diagnostic-fixture-audit.v1 True 28 29 0 28 28 0 0 0 4`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `cf70d086` | Strengthened CLI help-surface gap details. `appgen.cli-help-surface-audit.v1` now reports missing documented subcommands, option help exit failures, per-command missing-option details, option-surface names, and passing option-surface counts so CLI discoverability failures are actionable for agents and release gates. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for top-level help/alias contract and doc-anchor audit; direct probe proving `appgen.cli-help-surface-audit.v1 True 20 0 0 21 21 0 0 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `d85ba89c` | Strengthened VS Code extension audit evidence and aligned doctor text markers. `appgen.vscode-extension-audit.v1` now reports explicit missing counts for required commands, activation events, command palette entries, provider markers, CLI contract markers, and webview renderers, and the doctor text-renderer contract now uses the same VS Code audit format emitted by real doctor reports. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for VS Code extension contract, doctor text renderer, and doc-anchor audit; direct probe proving `appgen.vscode-extension-audit.v1 True 10 10 0 0 0 0 0 0` and `appgen.doctor-text-renderer.v1 True 9 0 8`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `1ceb8919` | Expanded doctor text-renderer evidence. `appgen.doctor-text-renderer.v1` now proves documented doctor log markers for parser-golden, completion coverage, semantic symbol coverage, LSP symbol coverage, CLI alias, module boundary, designer-sync, and extension-surface audits instead of only a three-check sample. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused doctor text-renderer pytest; direct renderer probe proving `appgen.doctor-text-renderer.v1 True 9 0 8 8`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `29c3d5f5` | Added frontend interaction-audit bridge evidence. The Studio browser-smoke CI contract now embeds `appgen.frontend-interaction-audit.v1`, and the Python Studio tooling audit verifies frontend interaction scenarios, audit inputs, helper coverage, and bridge gating alongside the semantic-service evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py tests/test_main.py`; focused pytest for Studio semantic service and package Studio browser-smoke CI contracts; focused doc-anchor pytest; direct browser-smoke CI contract probe proving `appgen.frontend-interaction-audit.v1 True 8 0 0 0`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `1a1e2f48` | Added frontend semantic-service bridge evidence. The Studio browser-smoke CI contract now embeds `appgen.frontend-semantic-service-audit.v1`, and the Python Studio tooling audit verifies frontend service evidence, required semantic surface ids, surface contract formats, and panel rendering hooks. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py src/pyAppGen/studio.py tests/test_dsl_tooling_contracts.py tests/test_main.py`; focused pytest for Studio semantic service, doc-anchor, and package Studio browser-smoke CI contracts; direct frontend semantic-service audit probe; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `c8411d03` | Added LSP symbol-surface coverage. `appgen.lsp-service.v1` now embeds `appgen.lsp-symbol-coverage.v1`, and doctor/tooling audit language-server gates use the full symbol fixture to prove every required semantic symbol kind projects into both document-symbol and workspace-symbol LSP surfaces. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for LSP symbol coverage, LSP service, and doctor/generate JSON contracts; direct probe of `appgen.lsp-symbol-coverage.v1` and service count wiring; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `c2d666b6` | Added executable `apg` alias proof. The repo now includes a local `./apg` command shim, and `appgen.cli-help-surface-audit.v1` / `appgen.cli-alias-contract.v1` now prove both the module entrypoint and repo-local alias dispatch tooling commands successfully. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for top-level help and `apg` alias contract; direct `./apg --help`; direct `./apg lint` JSON smoke; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `51af7adb` | Strengthened LSP service and completion coverage counts. `appgen.lsp-service.v1` now reports compact service counts for diagnostics, completions, completion coverage, hover content, references, symbols, actions, formatting, workspace symbols, and rename edits; `appgen.completion-coverage.v1` now reports required/detected/missing source counts plus label counts by source. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for LSP service and completion coverage tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-30 | `adad8bc8` | Strengthened semantic model and symbol coverage contract counts. The shared semantic model now reports required/present/missing top-level field counts, symbol counts, symbol-kind counts, diagnostic counts, and missing top-level fields; symbol coverage now reports required/detected/missing kind counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for semantic-model and symbol-coverage contract tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `3a6d475d` | Strengthened policy, phase, and package failure-path audit breadth counts. Non-goal policy guards, implementation-phase evidence, and package invalid-target audits now report case/phase/criterion/fix/rejection/failure-path counts so release gates expose measurable breadth. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for implementation-phase, non-goal policy, and package invalid-target audit tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `d0c40b8a` | Strengthened formatter, natural-language contract, LSP stdio, and quick-fix apply audit breadth counts. Formatter, in-process natural-language planning, framed LSP transport, and in-process LSP code-action application audits now report check/case/action/transport/token-budget and passing counts so library, CLI, and editor paths all expose measurable evidence. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for formatter contract, natural-language contract, LSP code-action apply, and LSP stdio transport audit tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `7638f413` | Strengthened agent-facing IDE and planning audit breadth counts. LSP JSON-RPC, test-strategy, designer-sync, and natural-language planner audits now report check/case/scenario/provider/surface/projection/operation-kind counts so IDE and coding-agent workflows prove breadth without expanding nested payloads. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for 4 LSP/designer/test-strategy/NL planner audit tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Focused tests emitted existing PBC catalog deprecation warnings through import side effects; no PBC files were edited or staged. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `eb992d1e` | Strengthened CLI write and failure-path audit breadth counts. Format-write, missing-input, missing-required-option, invalid-choice, and internal-error audits now report scenario/case/mode, passing, expected-message, stdout-empty, traceback-free, write-mode, check-mode, and organize-category counts instead of only boolean success. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for 5 CLI write/failure audit tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `efbafd02` | Strengthened CLI audit breadth counts across non-PBC tooling contracts. Lint directory, validate/generate, graph, graph-suite, migration, package/verify, LSP code-action, and LSP rename CLI audits now report case/scenario, passing, target/backend/action/manifest/handoff, and coverage counts so release evidence proves breadth instead of only boolean success. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; direct non-PBC helper probe for lint-directory, LSP code-action CLI, LSP rename CLI, validate/generate CLI, graph CLI, graph-suite CLI, and migration CLI audit contracts; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit and package/verify audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `27073956` | Completed text renderer count evidence across the remaining non-PBC renderer contracts. Parser-golden, component-publish, graph/explain, migration-plan, release-verifier, LSP service, LSP code-action, and tooling-audit text renderer contracts now report required-fragment, missing-fragment, output-line, and marker-line counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for 8 renderer contracts; direct renderer helper probe proving parser-golden `4/0/4/4`, component-publish `8/0/3/3`, graph/explain `14/0/17/11`, migration-plan `10/0/10/10`, release-verifier `9/0/9/9`, LSP service `12/0/12/11`, LSP code-action `6/0/6/6`, tooling-audit `12/0/23/13`, all `ok=True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `c7dc7b85` | Strengthened text renderer count evidence across multiple tooling contracts. The diagnostics, lint, semantic drift, doctor, validate/generate, format, and designer-sync text renderer contracts now report required-fragment, missing-fragment, output-line, and marker-line counts, with doctor preserving explicit check-line and detail-format-line counts. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for 7 renderer contracts; direct renderer helper probe proving diagnostics `8/0/12/12`, lint `9/0/9/7`, semantic drift `8/0/8/8`, doctor `4/0/4/3` with 3 check lines and 3 detail-format lines, validate/generate `12/0/12/10`, format `3/0/3/3`, designer-sync `12/0/12/12`, all `ok=True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `46b79fd4` | Strengthened Studio semantic service surface-count evidence. `appgen.studio-semantic-service-audit.v1` now reports service, required/observed/missing service-format, surface, required-surface, surface-format, semantic-surface-format, and panel counts so Studio bridge release evidence proves breadth without expanding nested payloads. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused Studio semantic service audit pytest; direct `appgen.studio-semantic-service-audit.v1` helper probe proving 4 services, 5 required service formats, 5 observed service formats, 0 missing service formats, 10 surfaces, 10 required surfaces, 10 surface formats, 7 semantic surface formats, 11 panel counters, and `ok=True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `8376dee1` | Strengthened explain CLI mode-count evidence. `appgen.explain-cli-audit.v1` now reports total case count, text-mode count, JSON-mode count, and report-format-covered count so release evidence proves all symbol, diagnostic, and handler explain modes remain covered. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused explain CLI audit pytest; direct `appgen.explain-cli-audit.v1` helper probe proving 6 total cases, 3 text cases, 3 JSON cases, 6 report-format-covered cases, and `ok=True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `91c79f81` | Strengthened CLI help surface-count evidence. `appgen.cli-help-surface-audit.v1` now reports required/documented/listed subcommand counts plus option-surface, required-option, and missing-option counts so CLI discoverability evidence can be compared without scraping nested help payloads. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused CLI help-surface audit pytest; direct `appgen.cli-help-surface-audit.v1` helper probe proving 20 required subcommands, 20 documented subcommands, 20 help-listed subcommands, 21 option help surfaces, 56 required options, 0 missing options, and `ok=True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
| 2026-05-29 | `a6e5b940` | Strengthened VS Code extension surface-count evidence. `appgen.vscode-extension-audit.v1` now reports required command, command-palette, activation-event, provider-marker, CLI-contract-marker, and webview-marker counts so extension release evidence captures editor surface breadth rather than only boolean checks. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused VS Code extension audit pytest; direct `appgen.vscode-extension-audit.v1` helper probe proving 10 required commands, 10 command-palette entries, 12 provider markers, 8 CLI contract markers, 5 webview markers, and `ok=True`; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
