...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened LSP service text renderer evidence. `appgen.lsp-service-text-renderer.v1` now reports summary-line, source-line, completion-line, completion-missing-line, navigation-line, formatting-line, rename-line, rename-blocker-line, hover-summary-line, and hover-line counts so editor-service text logs prove diagnostics/completions/actions/symbols, source-of-truth, navigation, formatting, completion gaps, rename blockers, and hover evidence remain visible. | Pending verification: py_compile, focused LSP service text renderer pytest, direct LSP service text renderer probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
