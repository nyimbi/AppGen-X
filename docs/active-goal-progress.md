...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened tooling-audit text renderer evidence. `appgen.tooling-audit-text-renderer.v1` now reports check-line, passing/failing check-line, detail-format-line, section-line, blocking-gap-line, and implementation-phase-line counts so human release logs prove they preserved checks, embedded audit formats, sections, and failure markers. | Pending verification: py_compile, focused tooling-audit text renderer pytest, direct text renderer audit probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
