...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Added first-class DSL language-quality CLI contracts. `appgen dsl-quality`, `appgen dsl-antlr`, `appgen dsl-authoring-gate <file>`, and `appgen dsl-language-service <file>` now expose grammar/parser sync, keyword-budget, authoring-gate, and language-service evidence directly, and `appgen.dsl-language-cli-audit.v1` is wired into the top-level tooling audit and CLI help surface. | Pending verification: py_compile, focused DSL language CLI audit pytest, CLI help-surface pytest, direct CLI/audit probes, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
