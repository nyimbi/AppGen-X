...
| --- | --- | --- | --- |
| 2026-05-29 | `4c7d0336` | Extended natural-language planning CLI audit text evidence. `appgen.nl-plan-cli-audit.v1` now proves an accepted non-JSON request exposes the `appgen.nl-plan.v1` envelope, nested lint and migration-preview format markers, backend metadata, and token-budget notes marker in human-readable output. | `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused NL-plan CLI text and audit pytest; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
