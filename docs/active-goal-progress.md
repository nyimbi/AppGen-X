...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen lint directory determinism evidence. `appgen.lint-directory-cli-audit.v1` now reports `file_order_sorted` and `file_relative_order`, proving recursive directory lint discovery is deterministic instead of only checking file count. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
