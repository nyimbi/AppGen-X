...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Expanded the natural-language planner CLI audit to cover every supported DSL edit operation. `appgen.nl-plan-cli-audit.v1` now exercises the same operation family as the in-process contract audit through `appgen nl-plan --json`, proving token-efficient agent-facing paths for tables, fields, relationships, forms, handlers, workflows, PBC composition, packages/deployment, and agent permissions. | Pending focused verification and commit. |
