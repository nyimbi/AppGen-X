...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened format and designer-sync text renderer evidence. `appgen.format-text-renderer.v1` now reports summary/write-path/diagnostic/write-flag/idempotence/organize line counts, and `appgen.designer-sync-text-renderer.v1` now reports summary/surface/visual-edit/dsl-diff/matrix/operation/case/check line counts. The designer-sync test also now asserts designer counters instead of migration counters, with migration counters asserted in the migration renderer test. | Pending verification: py_compile, focused format/designer/migration text renderer pytest, direct format/designer/migration renderer probes, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
