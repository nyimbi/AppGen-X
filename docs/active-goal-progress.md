...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Added lint text renderer evidence. `appgen.lint-text-renderer.v1` proves lint text logs expose source mode, file count, ordered syntax/semantic/policy stage counts, migration preview metadata, detected migration families, and diagnostic lines without parsing JSON or touching active PBC implementation files. | Pending focused verification. Full tooling audit intentionally deferred this turn to avoid active PBC path traversal. |
