...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen module-boundary surface-count evidence. `appgen.module-boundary-audit.v1` now reports `boundary_count`, `callable_count`, and `core_runtime_count`, so responsibility-boundary release evidence captures observed callable surface breadth rather than only missing boundary lists. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
