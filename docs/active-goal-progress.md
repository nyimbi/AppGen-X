...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened LSP CLI text coverage. `appgen lsp` now has executable text-mode proof for diagnostic, completion, code-action, document-symbol, workspace-symbol, source-of-truth, and hover evidence counts tied to the JSON language-service payload. | Pending focused verification. |
