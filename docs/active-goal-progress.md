...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen policy, phase, and package failure-path audit breadth counts. Non-goal policy guards, implementation-phase evidence, and package invalid-target audits now report case/phase/criterion/fix/rejection/failure-path counts so release gates expose measurable breadth. | Pending commit. Verification passed with `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for implementation-phase, non-goal policy, and package invalid-target audit tests; `git diff --check`; restricted legacy-name scan excluding active PBC paths. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
