...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen validate/generate artifact text evidence. `appgen.validate-generate-text-renderer.v1` now proves generation text logs include artifact byte metadata as `artifact ... bytes=...` when artifact summaries provide sizes, alongside manifest, target, semantic-model, blocking-gap, and diagnostic markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
