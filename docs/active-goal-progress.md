...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Strengthened component publication text renderer evidence. `appgen.component-publish-text-renderer.v1` now reports summary-line, catalog-line, side-effect-line, patch-contract-line, and existing-catalog-line counts so non-PBC component publication logs prove catalog context and side-effect-free patch posture remain visible. | Pending verification: py_compile, focused component publication text renderer pytest, direct component text renderer audit probe, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full tooling audit CLI traversal remains deferred to avoid active PBC path traversal. |
