...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Strengthened web package manifest handoff evidence. `appgen.package-manifest.v1` for web targets now carries explicit verifier booleans for build contract, routes, valid form bindings, handler resolution, smoke tests, and `web.smoke` entrypoint instead of relying only on generic handoff artifact names. | Pending focused verification. |
