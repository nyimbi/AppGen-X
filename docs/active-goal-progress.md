...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen CLI write and failure-path audit breadth counts. Format-write, missing-input, missing-required-option, invalid-choice, and internal-error audits now report scenario/case/mode, passing, expected-message, stdout-empty, traceback-free, write-mode, check-mode, and organize-category counts instead of only boolean success. | Pending commit. Verification passed with `./.venv/bin/python -m py_compile src/pyAppGen/dsl.py tests/test_dsl_tooling_contracts.py`; focused pytest for 5 CLI write/failure audit tests; `git diff --check`. Full tooling audit intentionally deferred to avoid active PBC path traversal. |
