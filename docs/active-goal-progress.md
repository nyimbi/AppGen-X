...
| --- | --- | --- | --- |
| 2026-05-29 | `pending` | Surfaced natural-language planning token-budget evidence in text output. `appgen nl-plan` now prints token-budget note counts so agent-facing release logs expose the same small-model planning guidance carried by `appgen.nl-plan.v1`. | Pending focused verification. |
