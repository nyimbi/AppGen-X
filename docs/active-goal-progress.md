...
| --- | --- | --- | --- |
| 2026-05-30 | `PENDING` | Split CLI usage and controlled-failure behavior into a first-class tooling gate. `appgen.tooling-audit.v1` now includes `cli_usage_failure_contracts`, and phase 3 requires internal-error, missing-input, missing-required-option, and invalid-choice exit contracts instead of relying only on the broader validation/generation aggregate. | Pending verification: py_compile, focused implementation-phase pytest, focused top-level tooling-audit pytest, direct tooling-audit probe for the CLI usage failure gate, `git diff --check`, and restricted legacy-name scan excluding active PBC paths. Full external tooling traversal remains deferred for active PBC path isolation. |
