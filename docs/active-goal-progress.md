...
| --- | --- | --- | --- |
| 2026-05-29 | `PENDING` | Strengthen semantic drift blocking-gap text evidence. `appgen.semantic-drift-text-renderer.v1` now proves semantic drift text logs include named blocking gaps as `gap ...`, not only aggregate gap counts alongside surface, evidence, digest, and check markers. | Pending focused non-PBC verification; broad tooling audit intentionally deferred while active PBC implementation paths are off-limits. |
